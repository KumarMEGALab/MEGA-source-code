...
type
  ...
  private
    ...
    ColWidthSet: Boolean;
    function DataToAbs(index: Integer): Integer;
    ...
    procedure MapSiteIndex(var x0, x1: integer);
    procedure MapSeqIndex(var y0, y1: integer);
  ...
  public
    ...
    procedure ResetGrid;
    procedure ResetSize;
    procedure Select(x0,y0,x1,y1: integer);
    ...
    procedure CacheSelectionData;
    procedure SetSelectionFromCachedData;
...
uses
  RegExpr, MPleaseWait, math, mcolorcodes, dateutils, LCLProc;
...
begin
  ...
  try
    ...
    begin
      seq := DNASeqs[i];
      for j := (Length(seq) div 3)-1 downto 0 do
        if System.Copy(seq, j*3+1, 3) = '---' then
          System.Delete(seq, j*3+1, 3);
      if SeqList[i].NoOfSites > 0 then
        for j := 1 to SeqList[i].NoOfSites do
          if SeqList[i][j] = '-' then
            System.Insert('---', seq, (j-1)*3+1);
      SeqList[i].SeqData := seq;
      if MilliSecondsBetween(Now, updateTime) > 200 then
      begin
...
procedure TAlignGrid.MapSiteIndex(var x0, x1: integer);  // maybe fwn
begin
  if Selection.Left = 0 then
    x0 := 1
  else
    x0 :=  Selection.Left-FixedCols+1;
  if (Selection.Right > 1) and (Selection.Right >= MaxNoOfSites+FixedCols) then
    x1 := MaxNoOfSites
  else
    x1 := Selection.Right-FixedCols+1;
  if x1 = 0 then x0 := 0;
end;

procedure TAlignGrid.MapSeqIndex(var y0, y1: integer);
begin
  if Selection.Top = 0 then
    y0 := 0
  else
    y0 := Selection.Top-FixedRows;
  if (NoOfSeqs > 0) and (Selection.Bottom >= NoOfSeqs+FixedRows) then
    y1 := NoOfSeqs-1
  else
    y1 := Selection.Bottom-FixedRows;
  if y1 = 0 then y0 := 0;
end;
...
function TAlignGrid.BlockSelected:boolean; //FWN
begin
  result := false;
  if not (goRangeSelect in Options) then exit;

  if (Selection.Left >= 0) and (Selection.Left <= DataToAbs(0)) then exit;
...
begin
  ...
  //Result := (Selection.Left >= 0) and (Selection.Left <= DataToAbs(FixedCols)) and (Selection.Top >= 1) and (Selection.Bottom <= NoOfSeqs);
end;

function TAlignGrid.GetNoOfSelectedSites: integer;  //fwn
begin

  if Selection.Left <= DataToAbs(0) then
    result := Selection.Right-Selection.Left-DataToAbs(0)
  else
...
begin
  result := false;
  if Empty then exit;
  Result := (Selection.Top <= 1) and (Selection.Left >= DataToAbs(1)) and (Selection.Right <= DataToAbs(MaxNoOfSites));
end;
...
procedure TAlignGrid.Select(x0,y0,x1,y1: integer);  //fwn
var
  s: TGridRect;
begin
  if x0 <= DataToAbs(0) then
    x0 := DataToAbs(1);
  if y0 < 0 then
    y0 := 0;
  if x1 > ColCount then
...
begin
  ...
  if MaxNoOfSites = 0 then
    ...
    s.Right := DataToAbs(MaxNoOfSites);
  if NoOfSeqs = 0 then
    s.Bottom := FixedRows
  else
    s.Bottom := RowCount-1;
...
begin
  ...
  begin
{
    if SeqList[i].FullSeqName <> '' then
      str := SeqList[i].FullSeqName
    else
    begin
      while Pos(' ', str) > 0 do
        str[Pos(' ', str)] := '_';
...
procedure TAlignGrid.CacheSelectionData;
begin
  FSelectionData.Left := Selection.Left;
  FSelectionData.Top := Selection.Top;
  FSelectionData.Right := Selection.Right;
  FSelectionData.Bottom := Selection.Bottom;
  FColumnAndRow.X := Col;
  FColumnAndRow.Y := Row;
end;

procedure TAlignGrid.SetSelectionFromCachedData;
begin
  Select(FSelectionData.Left, FSelectionData.Top - 1, FSelectionData.Right, FSelectionData.Bottom - 1);
  //Col := FColumnAndRow.X;
  //Row := FColumnAndRow.Y;
end;
        ...
        str := Trim(System.Copy(sl[i], 2, Length(sl[i])-1));
{
        if Pos(' ', str) > 0 then
        begin
          seq.FullSeqName := System.Copy(str, Pos(' ', str)+1, Length(str)-Pos(' ', str));
...
function TAlignGrid.DataToAbs(index: Integer): Integer;
begin
  //result := index + Cols.Count-1; // -1 because data is expected to start at 1
  result := index + Cols.VisibleCount - 1;
end;
