...
type
  ...
  TTabularTreeExport = class(TObject)
    ...
    public
      ...
      function GenerateLittleBootstrapsExport(treeIndex: Integer): TStringList;
      function ExportLittleBootstrapsTree(treeIndex: Integer; filename: String): Boolean;
  end;
...
function TTabularTreeExport.GenerateLittleBootstrapsExport(treeIndex: Integer): TStringList;
const
  IdFormatString = '%6d';
  SIdFormatString = '%6s';
...
begin
  Initialize(FTreeList[treeIndex], FTreeList.OTUNameList, FTreeList.isRooted);
  Result := TStringList.Create;
  if Assigned(FTreeList.InternalNodeLbls) then
    NodeLabels := FTreeList.InternalNodeLbls;
  ...
  if FTreeList[treeIndex].isBLen then
    TempString += Format(SFloatFormatString, ['BLen']) + #9;
  if FtreeList[treeIndex].isStats then
  begin
    TempString += Format(SFloatFormatString, ['BCL']) + #9;
    if FTreeList[treeIndex].isSE then
      TempString += Format(SFloatFormatString, ['SE(BCL)']) + #9;
  end;
...
function TTabularTreeExport.ExportLittleBootstrapsTree(treeIndex: Integer;filename: String): Boolean;
var
  aList: TStringList = nil;
begin
  try
    aList := GenerateLittleBootstrapsExport(treeIndex);
