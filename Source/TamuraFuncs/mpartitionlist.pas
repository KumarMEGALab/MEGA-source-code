...
type
  TPartitionArray = array[Word] of TPartition;
  ...
  private
    ...
    function AddWithBLen(tree: PArrayOfNodeData; blen: PArrayOfDouble; OnlyNew: boolean; Value: double; F: double):boolean;
  ...
  public
    ...
    function GetPartitionStringsForTreeData(tree: TTreeData): TStringList;
...
uses
  ...
  MD_MegaMain,
  {$ENDIF}
  MGlobalSettings, MegaUtils, StringUtils;
...
begin
  ...
  GetMem(PartArray, SizeOf(Pointer)*(FNoOfOTUs-3));
...
begin
  Result := AddWithBLen(tree.NodeArray, tree.BLenArray, False, Value, 1.0);
end;
...
function TPartitionList.AddWithBLen(tree: PArrayOfNodeData; blen: PArrayOfDouble; OnlyNew: boolean; Value: double; F: double):boolean;
var i,j,n : integer;
    flag : boolean;
    p : TPartition;
    t : TTopology;
...
function TPartitionList.ToInfoString: AnsiString;
var
  MyString: AnsiString;
  MyCount: Integer;
  i: Integer;
begin
  ...
  begin
    ...
    for i := 0 to MyCount - 1 do
      MyString := MyString + '[Information_' + IntToStr(i) + '] ' + Information[i] + #13#10;
  end;

  MyString := MyString + '[ValueName] ' + ValueName + #13#10;
  MyString := MyString + '[Value2Name] ' + Value2Name + #13#10;
  MyString := MyString + '[FreqName] ' + FreqName + #13#10;
  MyString := MyString + '[StatsName] ' + StatsName + #13#10;

  MyString := MyString + '[MaxStats]' + FloatToStrF(MaxStats, ffFixed, 4, 4) + #13#10;
  MyString := MyString + '[TotalFrequency]' + FloatToStrF(TotalFrequency, ffFixed, 4, 4) + #13#10;

  //if DistanceMatrix <> nil then
    //MyString := MyString + DistanceMatrixToString(DistanceMatrix, NoOfOtus);

   if lstPartition <> nil then
   begin
     MyCount := lstPartition.Count;
     for i := 0 to MyCount do
       MyString := MyString + TPartition(lstPartition[i]).ToString();
   end;

   if lstTopology <> nil then
   begin
     MyCount := lstTopology.Count;
     for i := 0 to MyCount do
       MyString := MyString + TTopology(lstTopology[i]).DescString();
   end;

  MyString := MyString + '[isRooted] ' + BoolToStr(isRooted) + #13#10;
  MyString := MyString + '[isBLen] ' + BoolToStr(isBLen) + #13#10;

  MyString := MyString + '[NoOfOTUs] ' + IntToStr(NoOfOTUs) + #13#10;
  MyString := MyString + '[NoOfTrees] ' + IntToStr(NoOfTrees) + #13#10;
  MyString := MyString + '[MaxNoOfTrees] ' + IntToStr(MaxNoOfTrees) + #13#10;
  MyString := MyString + '[NoOfPartitions] ' + IntToStr(NoOfPartitions) + #13#10;
...
begin
  ...
  MyString := MyString + '[Freq] ' + FloatToStrf(Freq,ffFixed, 5, 5) + #13#10;
  MyString := MyString + '[Value] ' + FloatToStrf(Value,ffFixed, 5, 5) + #13#10;
  MyString := MyString + '[Value2] ' + FloatToStrf(Value2,ffFixed, 5, 5) + #13#10;

  MyString := MyString + '[Dt] ' + IntToStr(Dt) + #13#10;
  MyString := MyString + '[NoOfOTUs] ' + IntToStr(NoOfOTUs) + #13#10;

  MyCount := NoOfOtus - 4;
  for i := 0 to MyCount - 1 do
    MyString := MyString + '[Partition_' + IntToStr(i) + '] ' + IntToStr(Partition[i]) + #13#10;
