...
begin
  ...
  begin
    ...
    FSavedSettingsFile := GetPrivateFile(mfMuscleDnaJsonSaved, False);
  end
  ...
  begin
    ...
    FSavedSettingsFile := GetPrivateFile(mfMuscleCodonsJsonSaved, False);
  end
  else if FDataType = snProtein then
  begin
    FDefaultSettingsFile := GetPrivateFile(mfMuscleAminoJsonDefault, True);
    FSavedSettingsFile := GetPrivateFile(mfMuscleAminoJsonSaved, False);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    FSavedSettingsFile := GetPrivateFile(mfClustalDnaJsonSaved, False);
  end
  else if FDataType = snCoding then
  begin
    FDefaultSettingsFile := GetPrivateFile(mfClustalCodonsJsonDefault, True);
    FSavedSettingsFile := GetPrivateFile(mfClustalCodonsJsonSaved, False);
  end
  else if FDataType = snProtein then
  begin
    FDefaultSettingsFile := GetPrivateFile(mfClustalAminoJsonDefault, True);
    FSavedSettingsFile := GetPrivateFile(mfClustalAminoJsonSaved, False);
  end
  else
  begin
    Assert(False, 'Missing handler for data type. This is a bug!');
