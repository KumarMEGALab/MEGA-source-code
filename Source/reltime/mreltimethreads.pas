...
begin
  ...
  try
    ...
    FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Computing CorrTest');
    ...
    {$ENDIF}
    FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Preparing results');
    OnProgress(100);
  except
    on E: EAbort do
    begin
...
begin
  ...
  try
    try
      ...
      FAnalysisInfo.ARP.UpdateRunStatusInfo('status', 'Running CorrTest');
...
begin
  ...
  try
    try
      ...
      RelTimeComputer.ProgressProc := Self.OnProgress;
      FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Computing RelTime divergence times');
      ...
      begin
        ...
        begin
          ...
          RelTimeComputer.GenerateSampledTimesClockTreeExport(ClockExportList,FAnalysisInfo.MyOriTreeList.OTUNameList, FAnalysisInfo.MyOriTreeList.InternalNodeLbls, sampleTimes);
        end
        else
        begin
          if FAnalysisInfo.CalibrationTimes.UsesCalibrationDensities then
        ...
        RelTimeTree := RelTimeComputer.ComputeRelTimeBLens(FAnalysisInfo.MyOriTreeList[0],
                                                           ClockExportList,
                                                           FAnalysisInfo.MyOtuNames,
                                                           FAnalysisInfo.MyOriTreeList.InternalNodeLbls,
                                                           MaxRateRatio);
      if not Assigned(RelTimeTree) then
        if ReltimeComputer.Log.Text <> EmptyStr then
          raise Exception.Create(ReltimeComputer.Log.Text)
...
begin
  ...
  try
    ...
    MLTreeAnalyzer.ModelInfoList.Add(TModelInfo.Create); // put the non-clock tree stuff at index 1 so we don't have to set the index for TTreeViewForm
    MLTreeAnalyzer.GetModelInfo(MLTreeAnalyzer.ModelInfoList[0]);
    if FAnalysisInfo.BootReps > 0 then
      FAnalysisInfo.ARP.UpdateRunStatusInfo('status', 'Constructing bootstrap timetrees')
    else
      FAnalysisInfo.ARP.UpdateRunStatusInfo('status', 'Constructing timetree');
    ...
    siteCoverageThread := TSiteCoverageThread.CreateFromTreeData(FAnalysisInfo, ClockTree);
    ...
    if IsDeveloper then
      MLTreeAnalyzer.MakeClockTree(ClockTree, ClockTreeExport, FAnalysisInfo.MyOtuNames, FAnalysisInfo.MyOriTreeList.InternalNodeLbls, FAnalysisInfo.MaxRateRatio)
    else
      MLTreeAnalyzer.MakeClockTree(ClockTree, ClockTreeExport, FAnalysisInfo.MyOtuNames, FAnalysisInfo.MyOriTreeList.InternalNodeLbls, DEFAULT_MAX_RATE_RATIO);
    ...
    begin
      ...
      MLTreeAnalyzer.GenerateSampledTimesClockTreeExport(ClockTreeExport, FAnalysisInfo.MyOtuNames, FAnalysisInfo.MyOriTreeList.InternalNodeLbls, SampleTimes);
    end;
...
begin
  try
    try
      ...
      FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Computing distances');
      ...
      FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Computing LS tree');
...
var
  siteCoverageThread: TSiteCoverageThread;
...
begin
  ...
  if IsDeveloper then
    ...
    MaxRateRatio := DEFAULT_MAX_RATE_RATIO;
  TempNodeLabels := FAnalysisInfo.MyOriTreeList.InternalNodeLbls;
  try
    try
      ...
      begin
        ...
        RelTimeComputer.ProgressProc := Self.OnProgress;
        RelTimeComputer.PropagateConstraints := FAnalysisInfo.PropagateConstraints;
        siteCoverageThread := TSiteCoverageThread.CreateFromTreeData(FAnalysisInfo, FAnalysisInfo.MyOriTreeList[0]);
        siteCoverageThread.Start;
        siteCoverageThread.WaitFor;
        siteCoverageThread.Free;
        FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Computing RelTime divergence times');
        ...
        FAnalysisInfo.AnalysisSummary.AddCalculatedValue('CorrTest P-value', CorrelationTest.PValueString);
        {$ENDIF}
        FAnalysisInfo.ReltimeNexusExport := RelTimeComputer.GenerateNexusReltimeExport(FAnalysisInfo.MyOtuNames, HasCalibrations);
        OLSAnalysisInfo := nil;
        FAnalysisInfo.MyOriTreeList.Insert(0, RelTimeTree);
...
begin
  try
    ...
    FAnalysisInfo.CalibrationTimes.PrepareDivTimeArrays(FMinTimes, FMaxTimes, ClockTree.NoOfOTUs, False);
    if IsDeveloper then
    begin
    ...
    if IsDeveloper then
      MLTreeAnalyzer.GenerateCalibratedClockTreeExport(ClockTreeExport, FAnalysisInfo.MyOtuNames, FAnalysisInfo.MyOriTreeList.InternalNodeLbls, FMinTimes, FMaxTimes)
    else
      MLTreeAnalyzer.GenerateCalibratedClockTreeExport(ClockTreeExport, FAnalysisInfo.MyOtuNames, FAnalysisInfo.MyOriTreeList.InternalNodeLbls, FOrigMinTimes, FOrigMaxTimes);
...
//  paraDigits = 2;
//var
  //CoverageComputer: TSiteCoverageComputer;

  //UserTree: TTreeData;
  //ClockModelInfo: TModelinfo;
  //ClockTreeExport: TStringList;
  //TempNodeLabels: array of AnsiString;
begin
  //CoverageComputer := nil;
  //ClockTree := nil;
  //UserTree := nil;
  //ClockModelInfo := nil;
  //ClockTreeExport := nil;
  //SetLength(TempNodeLabels, 0);
  ...
  try
    try
      ...
      // now make a clock tree from the ML optimized user tree
      //FAnalysisInfo.MyOriTreeList.ValueName := 'LogL';
      //MLTreeAnalyzer.ModelList.Add(TModelInfo.Create); // put the non-clock tree stuff at index 1 so we don't have to set the index for TTreeViewForm
      //MLTreeAnalyzer.GetModelInfo(MLTreeAnalyzer.ModelList[0]);
      ...
      //CoverageComputer := TSiteCoverageComputer.Create;
      //CoverageComputer.Progress := FAnalysisInfo.ARP;
      //CoverageComputer.ComputeSiteCoverage(FAnalysisInfo, ClockTree);
      //UserTree.AssignDataCoverage(ClockTree); { same topology so no need to recompute}
      //if FAnalysisInfo.BootReps > 0 then
      //  FAnalysisInfo.ARP.UpdateRunStatusInfo('status', 'Constructing bootstrap timetrees')
      //else
      FAnalysisInfo.ARP.UpdateRunStatusInfo('Status', 'Anchoring timetree');
      FAnalysisInfo.ARP.Progress := 1;
      //FAnalysisInfo.MySeqStrings := nil; { this gets freed by MLAnalyzer}
      Calibrations.PrepareDivTimeArrays(FMinTimes, FMaxTimes, FAnalysisInfo.NoOfSeqs, False);
      if IsDeveloper then
      ...
      if not FIsSuccess then
        raise Exception.Create('Unable to calculate divergence times. Please check the provided calibration constraints');
      //MLTreeAnalyzer.GenerateCalibratedClockTreeExport(ClockTreeExport, FAnalysisInfo.MyOtuNames, FAnalysisInfo.MyOriTreeList.InternalNodeLbls);
      //
      //FAnalysisInfo.ClockTreeExport := ClockTreeExport;

      //MLTreeAnalyzer.ModelList.Insert(0,TModelInfo.Create);
      //MLTreeAnalyzer.GetModelInfo(MLTreeAnalyzer.ModelList[0]); // copy the active model info (for clock tree) into the newly created model info

      //FAnalysisInfo.LogLikelihoodWithClock  := FloatToStrF(MLTreeAnalyzer.ModelList[0].LogL, ffFixed, 12, lnLDigits);
      //FAnalysisInfo.LogLikelihoodWithoutClock := FloatToStrF(MLTreeAnalyzer.ModelList[1].LogL, ffFixed, 12, lnLDigits);
      //FAnalysisInfo.InvarWithClock := FloatToStrF(MLTreeAnalyzer.ModelList[0].Invar, ffFixed, 12, paraDigits);
      //FAnalysisInfo.GammaWithClock := FloatToStrF(MLTreeAnalyzer.ModelList[0].Gamma, ffFixed, 12, paraDigits);
      //FAnalysisInfo.InvarWithoutClock := FloatToStrF(MLTreeAnalyzer.ModelList[1].Invar, ffFixed, 12, paraDigits);
      //FAnalysisInfo.GammaWithoutClock := FloatToStrF(MLTreeAnalyzer.ModelList[1].Gamma, ffFixed, 12, paraDigits);
      //FAnalysisInfo.FullModelName := MLTreeAnalyzer.ModelList[0].FullName;
      //FAnalysisInfo.ARP.Progress := 100;
    except
      On E : Exception do
      begin
    ...
    //if Assigned(CoverageComputer) then
    //  CoverageComputer.Free;
  end;
end;
