...
uses
  ...
  Buttons, ActnList, ImgList, MTimeTreeWizard, MegaConsts,
  mimageform;
  ...
  case FProcessStep of
    ttpsLoadTree:
      begin
        ...
        DefaultButtons.GetBitmap(0, LoadTreeBtn.Picture.Bitmap);
        StatusBar1.SimpleText := UpperCase('Ready to load a tree file');
        if LoadSeqDataAction.Enabled then
        begin
          DefaultButtons.GetBitmap(0, LoadSeqDataBtn.Picture.Bitmap);
    ...
    ttpsLoadSeqData:
      begin
        ...
        Check.Invalidate;
        StatusBar1.SimpleText := UpperCase('Ready to load a sequence data file');
      end;
    ttpsDoOutgroup:
      begin
        ...
        DefaultButtons.GetBitmap(1, SpecifyOutgroupTEBtn.Picture.Bitmap);
        DefaultButtons.GetBitmap(2, SpecifyOutgroupBtn.Picture.Bitmap);
        StatusBar1.SimpleText := UpperCase('Ready to select the outgroup taxa');
      end;
    ttpsDoCalibrations:
      begin
        ...
        DefaultButtons.GetBitmap(3, SpecifyCalibrationsBtn.Picture.Bitmap);
        SpecifyOutgroupTaxaAction.Enabled := CanSetOutgroup;
        SpecifyOutgroupTEAction.Enabled := CanSetOutgroup;
        SkipAction.Enabled := True;
        SkipButtons.GetBitmap(0, SkipBtn.Picture.Bitmap);
        StatusBar1.SimpleText := UpperCase('Ready to specify divergence time calibration constraints');
      end;
    ttpsDoSettings:
      begin
        ...
        DefaultButtons.GetBitmap(4, AnalysisOptionsBtn.Picture.Bitmap);
        StatusBar1.SimpleText := UpperCase('Ready to select analysis settings');
      end;
    ttpsLaunchAnalysis:
      begin
        ...
        DefaultButtons.GetBitmap(5, LaunchBtn.Picture.Bitmap);
        Self.BringToFront;
        StatusBar1.SimpleText := UpperCase('Ready to launch analysis');
      end;
  end;
  {$IFDEF DARWIN}
  if Visible then
...
begin
  if not SpecifyOutgroupTEAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(1, SpecifyOutgroupTEBtn.Picture.Bitmap);
end;
...
var
  reduceBy: Integer;
begin
  Caption := 'Evolutionary Probabilities';
...
begin
  DisabledButtons.GetBitmap(0, LoadSeqDataBtn.Picture.Bitmap);
  DisabledButtons.GetBitmap(0, LoadTreeBtn.Picture.Bitmap);
  DisabledButtons.GetBitmap(1, SpecifyOutgroupTEBtn.Picture.Bitmap);
  DisabledButtons.GetBitmap(2, SpecifyOutgroupBtn.Picture.Bitmap);
  DisabledButtons.GetBitmap(3, SpecifyCalibrationsBtn.Picture.Bitmap);
  DisabledButtons.GetBitmap(4, AnalysisOptionsBtn.Picture.Bitmap);
  DisabledButtons.GetBitmap(5, LaunchBtn.Picture.Bitmap);
  SkipButtons.GetBitmap(2, SkipBtn.Picture.Bitmap);
end;
...
var
  reduceBy: Integer;
begin
  Caption := 'Codon-by-Codon Selection (Datamonkey)';
...
begin
  ...
  begin
    ...
    begin
      Caption := VER_MEGA_WIN_CAPTION_PREFIX + ': Tip Dating Wizard';
...
begin
  if not LoadTreeAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(0, LoadTreeBtn.Picture.Bitmap);
...
begin
  if not SpecifyCalibrationsAction.Enabled then
    Exit;
  SkipButtons.GetBitmap(1, SkipBtn.Picture.Bitmap);
end;
...
begin
  if not SpecifyCalibrationsAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(3, SpecifyCalibrationsBtn.Picture.Bitmap);
end;
...
begin
  if not SpecifyOutgroupTaxaAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(2, SpecifyOutgroupBtn.Picture.Bitmap);
end;
...
begin
  if not AnalysisPreferencesAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(4, AnalysisOptionsBtn.Picture.Bitmap);
end;
...
begin
  if not LaunchAnalysisAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(5, LaunchBtn.Picture.Bitmap);
end;
...
begin
  if not LoadSeqDataAction.Enabled then
    Exit;
  HoverButtons.GetBitmap(0, LoadSeqDataBtn.Picture.Bitmap);
end;
...
begin
  ...
  begin
    ...
    Wizard.CancelCalibrationChanges;
