...
type
  ...
  { TCorrelationTestResult }

  TCorrelationTestResult = class
    public
      PValue: String;
      Score: Extended;
      constructor Create;
      ...
      procedure AddFigureLegend(aLegend: TStringList);
  end;
  ...
  TReltimeComputer = class(TObject)
    private
      ...
      procedure UpdateNodeLabels(const CalibTimes: TCalibrations; var NodeLbls: TStringList);
    ...
    public
      ...
      function GenerateNexusReltimeExport(NamesList: TStringList; HasCalibrations: Boolean; HasDataCoverage: Boolean=True): TStringList;
...
uses
  ...
  MegaUtils, Math, StringUtils, mnexuswriter, MTreeDataAdapter;

{ TCorrelationTestResult }

constructor TCorrelationTestResult.Create;
begin

end;

destructor TCorrelationTestResult.Destroy;
begin
  inherited Destroy;
end;

function TCorrelationTestResult.SaveToFile(filename: String): Boolean;
begin
  Result := False;
  Assert(False, 'not implemented');
end;

procedure TCorrelationTestResult.AddFigureLegend(aLegend: TStringList);
begin
  Assert(False, 'not implemented');
end;
...
begin
  ...
  try
    try
      ...
      CalibrationTimes.PrepareDivTimeArrays(FMinTimes, FMaxTimes, TreeData.NoOfOTUs, False);
      Result := AnchorClockTree(FMinTimes, FMaxTimes);
      if not Assigned(ExportList) then
        ExportList := TStringList.Create
      else
        ExportList.Clear;
      UpdateNodeLabels(CalibrationTimes, NodeLabels);
...
begin
  ...
  try
    try
      ...
      CalibrationTimes.PrepareDivTimeArrays(FMinTimes, FMaxTimes, TreeData.NoOfOTUs, True);
      Result := AnchorClockTree(FMinTimes, FMaxTimes);
      if not Assigned(ExportList) then
        ExportList := TStringList.Create;
      ExportList.Clear;
      ...
      for i := Low(FNodes) to High(FNodes) do
        NodeLabels.Add(GetNodeLabel(CalibrationTimes, FNodes[i].Index));
...
begin
  FExportBuilder.Initialize(self, FNodes, names, labels, FIsBlensOnly, minTimes, maxTimes);
end;

procedure TReltimeComputer.GenerateSpreadsheetExport(const names, labels: TStringList);
begin
  FExportBuilder.Initialize(self, FNodes, names, labels, FIsBlensOnly);
end;

procedure TReltimeComputer.GenerateSpreadsheetExport(const names, labels: TStringList; const sampleTimes: array of Extended);
begin
  FExportBuilder.Initialize(self, FNodes, names, labels, FIsBlensOnly, sampleTimes);
end;
...
begin
  ...
  if IsDeveloper and (Length(FOriginalReltimes) > 0) then
    TempString := TempString + Format(SFloatFormatString, ['Orig_RelTime']) + #9;
  ...
  if IsDeveloper then
    TempString := TempString + Format(SFloatFormatString, ['BLen']) + #9;
  ...
  begin
    ...
    if IsDeveloper and (Length(FOriginalReltimes) > 0) then
      TempString := TempString + Format(SFloatFormatString, ['-']) + #9; // Orig_RelTime
    ...
    if IsDeveloper then
      TempString := TempString + Format(FloatFormatString, [FNodes[i].BLen]) + #9;  // BLen
  ...
  begin
    NodeLabel := GetInternalNodeLabel(MapNodeIndexToTreeDataIndex(FNodes[i].Index));
    TempString := MakePrettyTaxonNameString(NodeLabel, TaxonColumnWidth) + #9;
    ...
    if IsDeveloper and (Length(FOriginalReltimes) > 0) then
      TempString := TempString + Format(FloatFormatString, [FOriginalReltimes[i]]) + #9;
...
function TReltimeComputer.GenerateNexusReltimeExport(NamesList: TStringList; HasCalibrations: Boolean; HasDataCoverage: Boolean=True): TStringList;
var
  Writer: TNexusWriter;
begin
  Writer := nil;
...
begin
  try
    ...
    eb.Initialize(Self, FNodes, aNames, aLabels, FIsBlensOnly, minTimes, maxTimes);
...
begin
  try
    eb := TReltimeExportBuilder.Create;
    eb.Initialize(Self, FNodes, aNames, aLabels, FIsBlensOnly);
    if Assigned(aCaption) then
      eb.AddCaptionAsWorksheet(aCaption);
...
begin
  ...
  begin
    ...
    if IsDeveloper and (Length(FOriginalReltimes) > 0) then
      TempString := TempString + Format(SFloatFormatString, ['-']) + #9; // Orig Reltime
    ...
    if IsDeveloper then
    begin
      TempFloatStr := Trim(FormatDoubleSafe(FNodes[i].BLen, Precision, ValueDigits));
      TempString := TempString + Format(SFloatFormatString, [TempFloatStr]) + #9; // BLen
    end;
  ...
  begin
    ...
    if IsDeveloper and (Length(FOriginalReltimes) > 0) then
    begin
      TempFloatStr := Trim(FormatDoubleSafe(FOriginalReltimes[i], Precision, ValueDigits));
      TempString := TempString + Format(SFloatFormatString, [TempFloatStr]) + #9; // Orig Reltime
    end;
...
begin
  if IsSamplingTime then
    FExportBuilder.Initialize(self, FNodes, aNames, aLabels, FIsBlensOnly, FOrigSamplingTimes)
  else if Length(FMinTimes) > 0 then
    FExportBuilder.Initialize(self, FNodes, aNames, aLabels, FIsBlensOnly, FOrigMinTimes, FOrigMaxTimes)
  else
    FExportBuilder.Initialize(self, FNodes, aNames, aLabels, FIsBlensOnly);
end;
...
begin
  if UseTimeFactor then
    Result := NewickStringRecursiveWithTimeFactor(FRoot) + ';'
  else
    Result := NewickStringRecursive(FRoot) + ';';
end;

procedure TReltimeComputer.UpdateNodeLabels(const CalibTimes: TCalibrations; var NodeLbls: TStringList);
var
  i: Integer;
  NodeId: Integer;
...
begin
  ...
  for i := 0 to Length(FNodes) - 1 do
    NodeLbls.Add('-');
  for i := 0 to CalibTimes.Count - 1 do
  begin
    aCalib := CalibTimes.GetCalibration(i);
    if aCalib.IsSamplingTime then
      continue;
    NodeId := aCalib.NodeID;
    aLabel := aCalib.NodeLabel;
    if Trim(aLabel) <> EmptyStr then { don't replace any labels specified in the newick file with an empty string}
    begin
      if NodeId < FNoOfOtus then
        NodeLbls[NodeId + FNoOfOtus] := aLabel
      else
        NodeLbls[NodeId] := aLabel;
    end;
  end;
end;
