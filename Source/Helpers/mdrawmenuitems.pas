...
const
  ...
  MAINMENUACTIVEBGCOLOR = $003641c7;
...
begin
  ...
  begin
    ...
    if isTopItem then
      ...
      x := aRect.Left + MegaForm.NewIcons.Width + 2*MENU_ITEM_MARGIN;
    y := aRect.Top + 2;
    with ACanvas do
    begin
      if (odSelected in AState) or (odFocused in AState) then
...
procedure TMenuRenderer.MeasureMenuItem(Sender: TObject; ACanvas: TCanvas; var AWidth, AHeight: Integer);
const
  MARGIN = 8;
  PADDING = 4;
var
...
begin
  ...
  begin
    ...
    if not IsTopItem then
      aWidth := aWidth + MegaForm.NewIcons.Width;
    if aText = '-' then
      aHeight := MARGIN
    else
