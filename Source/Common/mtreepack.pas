...
type
  ...
  public
    ...
    function GetMLInitialTreesMethodCitation(IsAmino: Boolean; DistModel: TDistType): String;
...
uses
  Sysutils, MegaUtils,
  MegaAnalysisPrefStrings, TypInfo;
...
begin
  ...
  case FOperation of
      ...
      dtdoMPTree,
      dtdoMLIQTree,
      dtdoMLTree:                    AddType(ttInferTree);
...
//  if MyValue = InitialTreeByDefaultStr then AddType();

//  if (MyValue = UserSpecifyFromFile) then AddType(ttUserTreeFromFile);
  ...
  if MyValue = SPRFastStr then // GS - for MEGA 5.1
  begin
    AddType(ttSPRFast);
    AddType(ttDtSearch);
  end;

  if MyValue = SPRExtensiveStr then // GS - for MEGA 5.1
  begin
    AddType(ttSPRExtensive);
    AddType(ttDtSearch);
  end;

  if MyValue = SPRStr then // For parsimony
  begin
    AddType(ttSPR);
    AddType(ttDtSearch);
  end;

  if MyValue = TBRStr then // For parsimony
  begin
    AddType(ttTBR);
    AddType(ttDtSearch);
  end;
  ...
  if (MyValue = BootTestStr) or (MyValue = LittleBootTestStr) then AddType(ttBootstrap);
...
/// <summary>Get the citation string for the initial tree method used for
/// Maximum Likelihood phylogeny inference.</summary>
function TTreePack.GetMLInitialTreesMethodCitation(IsAmino: Boolean; DistModel: TDistType): String;

  function getModelString:String;
  begin
     if not IsAmino then
     begin
       if DistModel = gdJukesCantor then
         Result := 'the Jukes-Cantor model'
       else if DistModel = gdTamura then
         Result := 'the Tamura 3 parameter model'
       else if DistModel = gdTamuraNei then
         Result := 'the Tamura-Nei model'
       else
         Result := 'the Maximum Composite Likelihood (MCL) approach';
     end
     else
     begin
       if DistModel = gdJones then
         Result := 'the JTT model'
       else if DistModel = gdPoisson then
         Result := 'the Poisson model'
       else if DistModel = gdEqualInput then
         Result := 'the equal input model'
       else if DistModel = gdDayhoff then
         Result := 'the Dayhoff model'
       else
         Result := 'the JTT model';
     end;
  end;
...
begin
  if doesContain(ttUserTree) then
    Result := 'A user-specified tree was used as an initial tree in the heuristic search.'
  else
  begin
    if FMLInitialTreesMethod = NJInitTreeMethod then
      result := 'Initial tree(s) for the heuristic search were obtained by applying the ' +
                'Neighbor-Joining method to a matrix of pairwise distances ' +
                'estimated using ' + getModelString + '.'
    else if FMLInitialTreesMethod = BioNJInitTreeMethod then
      result := 'Initial tree(s) for the heuristic search were obtained by applying the ' +
                'BioNJ method to a matrix of pairwise distances estimated using ' + getModelString + '.'
    else if FMLInitialTreesMethod = MPInitTreeMethod then
      result := 'Initial tree(s) for the heuristic search were obtained automatically ' +
                'by applying the Maximum Parsimony method.'
    else if FMLInitialTreesMethod = DefaultInitTreeMethod then
      result := 'Initial tree(s) for the heuristic search were obtained automatically ' +
              'by applying Neighbor-Join and BioNJ algorithms to a matrix of pairwise distances ' +
              'estimated using ' + getModelString + ', and then selecting the topology with ' +
...
begin
  ...
  MyStringList.Add('IsAncesteral=' + BoolToStr(IsAncesteral));
