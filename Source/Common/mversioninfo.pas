...
type
  ...
  public
    ...
    procedure UpdateInfo(aMajor, aMinor, aRelease, aBuildCode: Integer; aAppName, aOs, aArch, aRelType: String);
    ...
    property ReleaseType: String read GetReleaseType;
  end;
...
var
  ...
  os, arch, releaseType, appName: String;
begin
  Result := False;
  try
    try
      ...
      if d.Count > 0 then
        ...
        begin
          ...
          aInfo.UpdateInfo(major, minor, release, build, appName, os, arch, releaseType);
          FReleases.Add(aInfo);
        end;
...
function TMegaVersionInfo.GetReleaseType: String;
begin
   case MegaReleaseType of
     mrtAlpha: Result := 'alpha';
     mrtBeta: Result := 'beta';
     mrtStable: Result := 'stable';
     mrtDeveloper: Result := 'developer';
   end;
end;
...
procedure TMegaVersionInfo.UpdateInfo(aMajor, aMinor, aRelease, aBuildCode: Integer; aAppName, aOs, aArch, aRelType: String);
begin
  FMajor := aMajor;
  FMinor := aMinor;
  FRelease := aRelease;
...
begin
  Result := False;
  if (GetReleaseType = 'stable') and (aReleaseType <> 'stable') then
    Exit;
  if (GetReleaseType = 'beta') and ((aReleaseType = 'alpha') or (aReleaseType = 'developer')) then
    Exit;
  if (GetReleaseType = 'alpha') and (aReleaseType <> 'alpha') then
    Exit;
