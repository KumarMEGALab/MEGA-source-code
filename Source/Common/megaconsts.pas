...
type
  ...
  TTreeboxType = (tttOriTree, tttBootTree, tttReltimeTree, tttEditBox);
  TCalibrationDensityDistribution = (cddNormal, cddExponential, cddUniform, cddLognormal, cddNone);
  TCalibrationCategory = (ccDensityNormal, ccDensityExponential, ccDensityUniform, ccMinTimeOnly, ccMaxTimeOnly, ccFixedTime, ccFixedRate, ccDensityLogNormal, ccNone);
  TCalibrateClockMode = (ccmEvoRate, ccmDivTime);
  ...
  TTimeTreeAnalysis = (ttaReltime, ttaCorrtest, ttaEP);
  ...
  TTtProcessStep = (ttpsLoadTree, ttpsLoadSeqData, ttpsDoOutgroup, ttpsDoCalibrations, ttpsDoIQSettings, ttpsSelectIQTreeOption, ttpsDoSettings, ttpsLaunchAnalysis, ttpsDone, ttpsCancelAnalysis);
...
const
  ...
  PROG_UPDATE_INTERVAL = 150; { number of milliseconds between progress updates}
  SUBTASK_PROG_UPDATE_INTERVAL = 150;
...
const  //Moved from MSeqDataViewerForm in refactoring process by Nick
  stCurSite      = 0;
  stMarkedSites  = 1;
  stCounts       = 2;
  stDomainName   = 3;
  ...
  MSDX_SESSION_VERSION = 1013; //The version of Mega Data Session file that this build uses
  MAS_SESSION_VERSION = 1011; // the current version of alignment session files
  LAST_BAD_MAS_SESSION_VERSION = 111; // this version and earlier are broken for later versions of MEGA
  MTS_SESSION_VERSION = 1007; // the current version of tree session files
  ...
  {$ELSE}
    {$IFDEF MSWINDOWS}
      ...
      {$ENDIF}
    {$ELSE}
      TARGET_PLATFORM = UNKNOWN_TARGET;
    {$ENDIF}
  {$ENDIF}
...
const  // Strings in the Menu preferences
  ...
  {$IFDEF RELTIME_ONLY}
  UserPref_AllowCollectUsageData = 'Allow RelTime to collect anonymous usage data.';
  {$ELSE}
...
type
  TGlenDevType = (gdtReltime, gdtRtdt, gdtRtdtInferDates, gdtLoadTreeFile, gdtMLUserTree, gdtSiteCoverage, gdtIQTree);
...
var
  ...
  InputDataOptionsFiletype: String = '#nucleotide_sequence_li a';

  IsGlenDevMode: Boolean = {$IFDEF DEBUG}False{$ELSE}False{$ENDIF};
  GlenDevType: TGlenDevType = gdtMLUserTree;
  {$IFNDEF VISUAL_BUILD} {$IFNDEF MSWINDOWS}
  PeakMemoryUsedByMega: Integer;
...
type
  TExportType = (EXInvalid, EXnone, EXtext, EXtextSave, EXcsvDisp, EXcsvSave, EXexcelDisp, EXexcelSave, EXexcelXmlDisp, EXexcelXmlSave, EXfasta, EXodsDisp, EXodsSave);
