...
uses
 ...
 Dialogs,
  {$ENDIF}
  Classes, KeywordConsts, StringUtils, Controls;
...
type
  ...
  public
    ...
    function IndexOfSeqName(aName: String): Integer;
...
uses
  ...
  htmloptionsdlg,
  {$ENDIF}
  SysUtils, MegaConsts, StrUtils, Math, mglobalsettings;
...
begin
  ...
  begin
    ...
    begin
      while (numSites < SITES_PER_LINE) and (i <= Length(aSequence)) do
      begin
...
begin
  ...
  try
    ...
    Result.Add(EmptyStr);
    Result.Add(Format('!FORMAT DataType=%s Indel=%s Identical=%s Missing=%s;', [DataTypeString, aGapSymbol, aIdenticalSymbol, aMissingSymbol]));
...
begin
  ...
  Result := ((numNucleoties > 27*numAminoAcids) or (numAminoAcids = 0));
end;
...
procedure TSequence.UnTranslate;
var
  j: integer;
  seq: AnsiString;
begin
...
begin
  ...
  begin
    ...
    begin
      tmpstr := '';
      j := Pos('gap ', GenbankResult[Result]) + 4;
...
begin
  ...
  if (NumNucleotidesFound / NumCharsToTest) >= Cutoff then
    Result:= snNucleotide
  else
    Result := snProtein;
end;
