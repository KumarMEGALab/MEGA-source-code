...
var
  i, j : integer;
  MyMeanGpDComps : ArrayOfDouble = nil; // within gp means
  d: Double;
...
begin
    ...
    if MyComputeType in [gdWithinGroupMean, gdNetGroupMean, gdAvgDiversityWithinSubPops] then
    begin
    ...
    for i:= 1 to NoOfRows-1 do
      ...
      begin
        ...
        else if (GpI = GpJ) and (MyComputeType in [gdWithinGroupMean, gdNetGroupMean, gdAvgDiversityWithinSubPops]) then
        begin
          if IsLowerLeft then  // to make sure we get the right values
            d := CellContent(i+1, j+1, InvalidNum, Bogus)
          else
...
var
  ...
  OffenseCount: ArrayOfInteger = nil;
begin
  AStrList := nil;
  try try
...
begin
  ...
  case FComputationType of
    ...
    gdAvgDiversityWithinSubPops, gdWithinGroupMean, gdOverallMean:
      case (ACol-1) of
...
begin
  ...
  case FComputationType of
    ...
    gdOverallMean:
    begin
      if Assigned(FStdErrArray) then
        Result := FStdErrArray[x];
    end;
  end;
  InvalidNum := Result <= InvalidDistValue;
end;
 ...
 MyD  := nil;
  ...
  try try
    ...
    case FComputationType of
      ...
      gdAvgDiversityWithinSubPops, gdWithinGroupMean, gdOverallMean:  // bug fix, allows matrixes with only 1 number in cell 0, 0 to be exported.  This is more important in NV though since exporting is the only form of output.
         begin
...
begin
  ...
  // check if we have all the data
    ...
    if D_DisplayMatrix.FComputationType in [gdOverallMean, gdWithinGroupMean, gdAvgDiversityWithinSubPops] then
    begin
...
begin
  try
    try
      ...
      if (Sender <> SortOriginalItem) and (D_DisplayMatrix.ComputationType in [gdWithinGroupMean, gdAvgDiversityWithinSubPops, gdBetweenGroupMean, gdNetGroupMean]) then { for these cases, we have group names but not taxa names}
      begin
        OriginalList := TStringList.Create;
        SortedList := TStringList.Create;
        SortedList.Sorted := True;
        SortedList.Duplicates := dupError;
        for i := 0 to D_DisplayMatrix.NoOfGps - 1 do
        begin
          OriginalList.Add(D_DisplayMatrix.GpName[i]);
          SortedList.Add(D_DisplayMatrix.GpName[i])
        end;
        for i := 0 to nRows - 1 do
           D_DisplayMatrix.CurSet[i] := OriginalList.IndexOf(SortedList[i]);
      end
      else if Sender = SortByDistItem then
      begin
      ...
      on E:Exception do
        ShowMessage('Application error when sorting items: ' + E.Message);
    end;
  finally
...
procedure TDisplayMatrixDlg.IniPropStorage1RestoreProperties(Sender: TObject);
begin
  {$IFDEF DARWIN}
  if (DataGrid.Font.Name = 'default') then
  begin
    DataGrid.Font.Name := 'Courier New';
  end;
  {$ENDIF}
end;

procedure TDisplayMatrixDlg.IniPropStorage1SavingProperties(Sender: TObject);
begin
  {$IFDEF DARWIN}
  if (DataGrid.Font.Name = 'Courier New') then
  begin
    DataGrid.Font.Name := 'default';
  end;
  {$ENDIF}
end;
...
procedure TDisplayMatrixDlg.MenuItem16Click(Sender: TObject);
begin
  OpenUrl(MegaGptUrl);
end;
...
var
  ...
  procedure SetOutputTypeForDialog;
  begin
    if (Sender = ExportDataBtn) or (Sender = ExportDataAction) then
      OutputMatrixDlg.SetExportToXL
    else if (Sender = ExportXlBtn) or (Sender = ExportExcelAction) then
      OutputMatrixDlg.SetExportToXL
    else if (Sender = ExportCsvBtn) or (Sender = ExportToCsvAction) then
      OutputMatrixDlg.SetExportToCSV
    else if (Sender = ExportMegaBtn) or (Sender = ExportToMegaAction) then
      OutputMatrixDlg.SetExportToMEGA
    else if (Sender = ExportTxtBtn) or (Sender = ExportTextAction) then
      OutputMatrixDlg.SetExportToText;
  end;
...
{$IFDEF VISUAL_BUILD}
 try try
    ...
    case D_DisplayMatrix.ComputationType of
      ...
      gdWithinGroupMean, gdAvgDiversityWithinSubPops:
        begin
          OutputMatrixDlg.AllowMEGAFormat := False;
    ...
    begin
      ...
      SetOutputTypeForDialog;
      if ShowModal = mrOk then
      begin
        ExportOptions := OutputMatrixDlg.GetExportOptions;
        Results := D_DisplayMatrix.WriteExportToFile(ExportOptions);
        if (ExportOptions.OutputFormat in [ExportExcel, ExportExelXML, ExportODS, ExportCSV]) and ExcelWrite.hasExcel then
          RunAProgram(ExportOptions.SaveExcelFileTo)
...
var
  aFont: TFont = nil;
  {$IFDEF LINUX}
  aDlg: TMegaFontDialog = nil;
  {$ENDIF}
begin
  {$IFDEF LINUX}
  aDlg := TMegaFontDialog.Create(nil);
  aDlg.Initialize(DataGrid.Canvas.Font);
  if aDlg.Execute then
    aFont := aDlg.GetSelectedFont;
  {$ELSE}
  ...
  begin
    with DataGrid do
    begin
      DataGrid.Font.Assign(aFont);
      DataGrid.Canvas.Font.Assign(aFont);
    end;
...
var
  ...
  MyMeanGpD : ArrayOfDouble = nil; // within gp means
...
var
  ...
  StartAt : Integer;
  Result : TStringList = nil;
  Offenses : ArrayOfInteger = nil;
begin
