...
uses
  ...
  mcalibrationdensity, MSimpleTreeNode, StrHashMap, mtimetree_map;
...
{ Description is given below at TCalibrationDlg.GetInternalNodeLabelFunc}
  ...
  TCalibrationTarget = (ctInternalNode, ctLeafNode, ctNotSpecified);
  ...
  TCalibrationTime = Class(TObject)
    ...
    public
      ...
      function DisplayString(longestNameLength: Integer): String;
      function IntervalString: String;
      ...
      function Clone: TCalibrationTime;
      ...
      property TimetreeMap: TTimeTreeMap read FTimetreeMap write FTimetreeMap;
  end;
  ...
  TCalibrations = class(TObject)
    ...
    public
      ...
      constructor Create;
      destructor Destroy; override;
      function NumTimetreeCalibrations: Integer;
      function NewCalibrationsHaveDuplicates(var Msg: String; var NewCalibrations: TCalibrations): Boolean;
      function RemoveDuplicateCalibrationsFromOtherList(otherList: TCalibrations): Integer;
      ...
      procedure RemoveTimetreeMaps;
      procedure RemoveTimetreeMap(Index: Integer);
      ...
      function GetTimetreeMap(treeBoxNodeId: Integer): TTimeTreeMap;
      procedure RemoveCalibrationForNode(aNodeId: Integer);
      function GetCalibrationByNodeId(aNodeId: Integer): TCalibrationTime;
      function GetCalibrationByTreeViewFormIndex(treeViewerIndex: Integer): TCalibrationTime;
      function GetDisplayData(const TreeViewIndex: Integer; var minTime: Double; var maxTime: Double; var isSelected: Boolean): Boolean;
      function LongestNameLength: Integer;
...
function TCalibrations.NumTimetreeCalibrations: Integer;
var
  i: Integer = -1;
begin
  Result := 0;
  if FCalibrations.Count > 0 then
    for i := 0 to FCalibrations.Count - 1 do
      if Assigned(GetCalibration(i).TimetreeMap) then
        inc(Result);
end;
...
function TCalibrations.RemoveDuplicateCalibrationsFromOtherList(otherList: TCalibrations): Integer;
var
  i: Integer;
  c: TCalibrationTime = nil;
begin
  Result := 0;
  try
    otherList.InitFastNodeIdLookupList(NumNodes + 1);
    if (otherList.Count > 0) and (Self.Count > 0) then
      for i := 0 to Self.Count - 1 do
      begin
        c := GetCalibration(i);
        if otherList.HasCalibrationForNode(c.NodeID) then
        begin
          inc(Result);
          otherList.RemoveCalibrationForNode(c.NodeID);
        end;
      end;
  finally
    otherList.ClearFastNodeIdLookupList;
  end;
end;
...
function TCalibrations.GetTimetreeMap(treeBoxNodeId: Integer): TTimeTreeMap;
var
  i: Integer = -1;
  c: TCalibrationTime = nil;
begin
  ...
  if FCalibrations.Count > 0 then
    for i := 0 to FCalibrations.Count - 1 do
    begin
      c := TCalibrationTime(FCalibrations[i]);
      if Assigned(c.TimetreeMap) and (c.TimetreeMap.TreeBoxNodeId = treeBoxNodeId) then
      begin
        Result := c.TimetreeMap;
        Exit;
      end;
    end;
end;

procedure TCalibrations.RemoveCalibrationForNode(aNodeId: Integer);
var
  c: TCalibrationTime = nil;
  i: Integer = -1;
begin
...
function TCalibrations.GetCalibrationByTreeViewFormIndex(treeViewerIndex: Integer): TCalibrationTime;
var
  i: Integer = -1;
  c: TCalibrationTime = nil;
begin
  ...
  if Count > 0 then
    ...
    begin
      c := GetCalibration(i);
      if c.TreeViewFormIndex = treeViewerIndex then
        Exit(c);
    end;
end;
...
function TCalibrations.LongestNameLength: Integer;
var
  c: TCalibrationTime = nil;
  i: Integer = -1;
begin
  ...
  if FCalibrations.Count > 0 then
    ...
    begin
      c := GetCalibration(i);
      if Length(c.CalibrationName) > Result then
        Result := Length(c.CalibrationName);
    end;
end;
...
procedure TCalibrations.RemoveTimetreeMaps;
var
  i: Integer;
  ACalib: TCalibrationTime;
begin
  ...
  begin
    ACalib := TCalibrationTime(FCalibrations[i]);
    ACalib.TimetreeMap := nil;
  end;
end;

procedure TCalibrations.RemoveTimetreeMap(Index: Integer);
var
        ...
        case StringToCaseSelect(Key, ['TaxonA', 'TaxonB','minTime', 'time', 'maxTime', 'MRCA', 'NodeName', 'CalibrationName', 'Taxon', 'Distribution', 'mean', 'stddev', 'lambda', 'rate', 'offset', 'sd', 'sampleTime', 'tipDate', 'IngroupRoot', 'Taxa']) of
          ...
          begin
            ...
            if CompareText(Value, TempCalibrationTime.NodeB) = 0 then
              raise Exception.Create(Format('TaxonA refers to the same name as Taxon B. Both are %s', [Value]));
          ...
          begin
            ...
            if CompareText(TempCalibrationTime.NodeA, Value) = 0 then
              raise Exception.Create(Format('TaxonB refers to the same name as Taxon A. Both are %s', [Value]));
            if Value = EmptyStr then
          ...
          begin
            ...
            if MinTime < 0 then
              raise Exception.Create('Minimum time is required to be a number larger than or equal to 0.');
      ...
      case densityType of
        ...
        cddNone:
          begin
            ...
            else if CompareValue(TempCalibrationTime.MinTime, 0, FP_CUTOFF) > 0 then
              TempCalibrationTime.CalibrationCategory := ccMinTimeOnly
            else if CompareValue(TempCalibrationTime.MaxTime, 0, FP_CUTOFF) > 0 then
              TempCalibrationTime.CalibrationCategory := ccMaxTimeOnly;
          end;
      end;
...
function TCalibrationTime.DisplayString(longestNameLength: Integer): String;
var
  nameFormatStr: String = '';
begin
  nameFormatStr := Format('%%-%ds', [longestNameLength]);
  if Assigned(FDensity) then
  begin
    Result := Format(nameFormatStr + ' [%.1f - %.1f MYA]', [CalibrationName, FDensity.MinTime, FDensity.MaxTime]);
  end
  else
  begin
    case FCalibrationCategory of
      ccMinTimeOnly: Result := Format(nameFormatStr + ' [≥ %.1f]', [CalibrationName, FMinTime]);
      ccMaxTimeOnly: Result := Format(nameFormatStr + ' [≤ %.1f]', [CalibrationName, FMaxTime]);
      ccMinMaxTime: Result := Format(nameFormatStr + ' [%.1f - %.1f]', [CalibrationName, FMinTime, FMaxTime]);
      ccFixedTime: Result := Format(nameFormatStr + ' [%.1f]', [CalibrationName, FMaxTime]);
      ccFixedRate: raise Exception.Create('fixed rates are not supported');
    end;
  end;
end;

function TCalibrationTime.IntervalString: String;
begin
  Result := EmptyStr;
  if Assigned(FDensity) then
  begin
    Result := Format('[%.1f - %.1f]', [FDensity.MinTime, FDensity.MaxTime]);
  end
  else
  begin
    case FCalibrationCategory of
      ccMinTimeOnly: Result := Format('[≥ %.1f]', [FMinTime]);
      ccMaxTimeOnly: Result := Format('[≤ %.1f]', [FMaxTime]);
      ccMinMaxTime: Result := Format('[%.1f - %.1f]', [FMinTime, FMaxTime]);
      ccFixedTime: Result := Format('[= %.1f]', [FMaxTime]);
      ccFixedRate: raise Exception.Create('fixed rates are not supported');
    end;
  end;
end;
...
begin
  ...
  if Assigned(Other.FTimetreeMap) then
  begin
    if not Assigned(FTimetreeMap) then
      FTimetreeMap := TTimeTreeMap.Create(Other.FTimetreeMap.TreeBoxNodeId);
    FTimetreeMap.Assign(Other.FTimetreeMap);
  end;
end;

function TCalibrationTime.Clone: TCalibrationTime;
begin
  Result := TCalibrationTime.Create(IsSamplingTime);
  Result.Assign(Self);
end;
...
begin
  ...
  case AValue of
    ccMinTimeOnly, ccMaxTimeOnly, ccMinMaxTime, ccFixedTime:
      begin
        if Assigned(FDensity) then
          FreeAndNil(FDensity);
      end;
...
begin
  if Assigned(FDensity) then
    raise Exception.Create('cannot set max time when using a density distribution');
  FMaxTime := AValue;
end;
...
begin
  ...
  begin
    ...
    begin
      if (NodeA <> EmptyStr) and (NodeB <> EmptyStr) then
...
begin
  ...
  begin
    if FMinTime > 0 then
      ...
      FSamplingMinTimes[repNum] := 0;
    if FMaxTime > 0 then
      FSamplingMaxTimes[repNum] := FMaxTime
    else
      FSamplingMaxTimes[repNum] := MaxInt;
  end;
end;
