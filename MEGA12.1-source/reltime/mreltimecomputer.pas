...
type
  ...
  TReltimeComputer = class(TObject)
    ...
    public
      ...
      procedure ReadFromFile(var aFile: File; const sessionVersion: Integer; SessionUsesExtended: Boolean);
...
begin
  ...
  //{$IFNDEF VISUAL_BUILD}
  //{$IFDEF DEBUG}
  //Debug := WriteTreeToNewickFile(FCorrTestTree, FNamesList, NextAvailableFilenameNV('_dev_corrtest_input.nwk'), msg);
  //{$ENDIF}
  //{$ENDIF}
end;
...
procedure TReltimeComputer.ReadFromFile(var aFile: File; const sessionVersion: Integer; SessionUsesExtended: Boolean);
var
  i: Integer = -1;
  n: Integer = -1;
  c: AnsiChar = #0;
...
begin
  ...
  begin
    ...
    BlockReadExtendedType(aFile, FLatestSampleDate, SessionUsesExtended);
    ...
    if n > 0 then
      for i := Low(FOrigSamplingTimes) to High(FOrigSamplingTimes) do
        BlockReadExtendedType(aFile, FOrigSamplingTimes[i], SessionUsesExtended);

    BlockRead(aFile, n, SizeOf(Integer));
    SetLength(FOriginalReltimes, n);
    if n > 0 then
      for i := Low(FOriginalReltimes) to High(FOriginalReltimes) do
        BlockReadExtendedType(aFile, FOriginalReltimes[i], SessionUsesExtended);
  end;

  BlockRead(aFile, IsVarBlen, SizeOf(Boolean));
  BlockReadExtendedType(aFile, Zvalue, SessionUsesExtended);
  BlockRead(aFile, FIsBLensOnly, SizeOf(Boolean));
  BlockRead(aFile, FNoOfOtus, SizeOf(Integer));
  BlockReadExtendedType(aFile, FTimeFactor, SessionUsesExtended);
  BlockReadExtendedType(aFile, FMinTimeFactor, SessionUsesExtended);
  BlockReadExtendedType(aFile, FMaxTimeFactor, SessionUsesExtended);
  BlockReadExtendedType(aFile, FLatestTime, SessionUsesExtended);
  BlockRead(aFile, FIsSamplingTime, SizeOf(Boolean));
  BlockReadExtendedType(aFile, FMaxRateRatio, SessionUsesExtended);

  BlockRead(aFile, n, SizeOf(Integer));
  SetLength(FMinTimes, n);
  if n > 0 then
    for i := 0 to n - 1 do
      BlockReadExtendedType(aFile, FMinTimes[i], SessionUsesExtended);
  BlockRead(aFile, n, SizeOf(Integer));
  SetLength(FMaxTimes, n);
  if n > 0 then
    for i := 0 to n - 1 do
      BlockReadExtendedType(aFile, FMaxTimes[i], SessionUsesExtended);

  BlockRead(aFile, n, SizeOf(Integer));
  SetLength(FOrigMinTimes, n);
  if n > 0 then
    for i := 0 to n - 1 do
      BlockReadExtendedType(aFile, FOrigMinTimes[i], SessionUsesExtended);
  BlockRead(aFile, n, SizeOf(Integer));
  SetLength(FOrigMaxTimes, n);
  if n > 0 then
    for i := 0 to n - 1 do
      BlockReadExtendedType(aFile, FOrigMaxTimes[i], SessionUsesExtended);
  ...
  begin
    ...
    begin
      FNodes[i] := TRelTimeTreeNode.Create(i);
      FNodes[i].ReadFromFile(aFile, SessionUsesExtended);
    end;
