...
type
  ...
  private
    ...
    procedure UpdateForReltimeCalibrationsMode;
...
procedure TTimeTreeWizardForm.UpdateForReltimeCalibrationsMode;
begin
  LoadSeqDataPanel.Visible := False;
  LoadSeqDataAction.Enabled := False;
  AnalysisSettingsPanel.Visible := False;
  AnalysisPreferencesAction.Enabled := False;
  Image4.Visible := False;
  SkipBtn.Visible := False;
  LaunchBtn.Visible := False;
  LaunchAnalysisAction.Enabled := False;
  ClientHeight := (HeaderPanel.Height + LoadTreePanel.Height + OutgroupPanel.Height + CalibrationsPanel.Height + 3*CancelLabel.Height + 20);
end;
...
begin
  case WizardMode of
    ...
    ttwmReltimeCalibrations: UpdateForReltimeCalibrationsMode;
  end;
end;
...
begin
  ...
  case WizardMode of
    ...
    ttwmBlens, ttwmReltimeCalibrations:
      begin
        FProcessStep := ttpsLoadTree;
      end;
  end;
...
begin
  ...
  StatusBar1.SimpleText := format('TreePanel: %d, OutgroupPanel: %d, CalibsPanel: %d, LaunchPanel: %d, Client: %d', [LoadTreePanel.Height, OutgroupPanel.Height, CalibrationsPanel.Height, LaunchPanel.Height, ClientHeight]);
  Exit;
  HighlightAllPanels;
  //case FProcessStep of
  //  ttpsLoadTree:
...
begin
  ...
  if MegaForm.HasActiveData or (WizardMode in [ttwmBlens, ttwmReltimeCalibrations]) then
  begin
...
begin
  if Value in [ttpsCancelAnalysis, ttpsDone] then
  begin
