...
type
  TSetupCalibrationsCallback = procedure(aCalibType: TCalibrationTarget) of object;
  ...
  TTimeTreeWizard = class(TInterfacedObject, ITimeTreeWizard)
    private
      ...
      procedure CalibrationsDoneCallback(Sender: TObject);
      procedure CalibrationsCancelledCallback(Sender: TObject);
      procedure AskUserForCalibrationsType;
      procedure CalibrationsTypeNotify(Sender: TObject);
    public
...
uses
  {$IFDEF VISUAL_BUILD}mega_main, non_modal_message_dlg,{$ENDIF}
  ...
  MTreeDataAdapter, MOtuInfo, ProcessTreeCmds, MTimeTreeWizardForm,
...
begin
  Result := True;
  case UserOp of
    dtdoRelTimeML, dtdoRelTimeBLens, dtdoRelTimeLS, dtdoLbsAnalyzeTree, dtdoLbsInference, dtdoLbsTiming, dtdoReltimeCalibrations: aType := ctInternalNode;
...
var
  ...
  HasUserSelectedFile: Boolean = False;
  {$ENDIF}
begin
  Result := False;
  ...
  try
    ...
    begin
      begin
        ...
        if not(HasUserSelectedFile) then
        begin
           WizardFormUpdate(ttpsLoadTree);
           Exit;
        end;
    ...
    else if FWizardMode in [ttwmBlens, ttwmReltimeCalibrations] then
    begin
      ...
      begin
        if Trim(TreeLoader.GetMsg) <> EmptyStr then
          ...
          raise Exception.Create('Please validate all input files.');
        WizardFormUpdate(ttpsLoadTree);
      end
    end
    else
...
begin
  ...
  begin
    if WizardMode in [ttwmBlens, ttwmReltimeCalibrations] then
...
begin
  ...
  try
    try
      ...
      FCalibrationEditor := ATreeViewer.CalibrationDlg;
      if FCalibrationType = ctLeafNode then
        FCalibrationEditor.CalibrationsType := ctLeafNode;
      ATreeViewer.PrepareToWorkWithTimeTreeWizard(AnalysisInfo);
      FCalibrationEditor.FinalizeCalibrationFunc := AnalysisInfo.MyOriTreeList.FinalizeCalibration;
      FCalibrationEditor.CalibrationWizard := Self;
      FCalibrationEditor.HasBadCalibsFunc := ATreeViewer.HasInvalidCalibrations;
      ...
      begin
        if Assigned(calibrationStrings) and (calibrationStrings.Count > 0) then
          FCalibrationEditor.WarnToCheckInferredTipDates
        else
          FCalibrationEditor.PromptIfTipDatesInNames;
      end
      else
        FCalibrationEditor.ShowWelcomeMsg;

      ATreeViewer.SkipCloseQuery := True; { so the user is not prompted to save a session file}
      if UsrOperation = dtdoReltimeCalibrations then
        FCalibrationEditor.DisplayMode := cdmJustGeneratingCalibrations;
      FCalibrationEditor.SaveChangesNotify := CalibrationsDoneCallback;
      FCalibrationEditor.CancelChangesNotify := CalibrationsCancelledCallback;
      FCalibrationEditor.PopupMode := pmExplicit;
      FCalibrationEditor.PopupParent := TTimeTreeWizardForm(WizardForm);
      FCalibrationEditor.ShowInTaskBar := stNever;
      FCalibrationEditor.Show;
      FCalibrationEditor.AutoSizeActionExecute(Self);
    except
      on E:Exception do
...
procedure TTimeTreeWizard.CalibrationsDoneCallback(Sender: TObject);
begin
  FCalibrationEditor.HideWelcomeMsg;
  AssignCalibrationTimes(FCalibrationEditor.GetCalibrations);
  if Assigned(FTreeViewer) then
  begin
    TTreeViewForm(FTreeViewer).FormIsClosing := True;
    TTreeViewForm(FTreeViewer).Close;
    FTreeViewer := nil;
  end;
  if UsrOperation = dtdoReltimeCalibrations then
    WizardFormUpdate(ttpsDone)
  else if LaunchedFrom = ttsMegaMain then
    WizardFormUpdate(ttpsDoSettings)
  else
    WizardFormUpdate(ttpsLaunchAnalysis);
  FCalibrationEditor := nil;
...
procedure TTimeTreeWizard.CalibrationsCancelledCallback(Sender: TObject);
begin
  if Assigned(FTreeViewer) then
  begin
    TTreeViewForm(FTreeViewer).FormIsClosing := True;
...
{ Because using a modal dialog (QuestionDialog) causes MEGA to crash on macOS,
  we are using our own non-modal question dialog with a TNotifyEvent to capture
  the user's response}
procedure TTimeTreeWizard.AskUserForCalibrationsType;
var
...
begin
  try
    ...
    choices.Add('Internal Node Constraints');
    choices.Add('Sample Times');
    choices.Add('Cancel');
    Assert(Assigned(WizardForm));
    with WizardForm as TTimeTreeWizardForm do
    begin
      aPosition.X := Left + 100;
      aPosition.y := Top + 100;
    end;
    ShowNonModalQuestionDlg(choices, 'Calibrations Type', 'What kind of calibrations will be used?', aPosition, CalibrationsTypeNotify);
  finally
...
procedure TTimeTreeWizard.CalibrationsTypeNotify(Sender: TObject);
var
  aButton: Integer = -1;
begin
  ...
  begin
    ...
    case aButton of
      0: FCalibrationType := ctInternalNode;
      1: FCalibrationType := ctLeafNode;
      2: FCalibrationType := ctNotSpecified;
      else
        raise Exception.Create('Missing handler for calibration type');
    end;
  end;
  if FCalibrationType in [ctInternalNode, ctLeafNode] then
    SetupCalibrationsCallback(FCalibrationType)
  else
    WizardFormUpdate(ttpsDoCalibrations);
end;
...
begin
  ...
  SelectOutgroupTaxaDlg.Position := poOwnerFormCenter;
...
begin
  ...
  TaxaGpsDlg.Position := poOwnerFormCenter;
