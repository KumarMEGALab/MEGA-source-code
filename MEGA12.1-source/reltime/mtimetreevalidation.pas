  ...
  if Length(FCalibrations) > 0 then
    ...
    begin
      ...
      begin
        ...
        {$IFNDEF VISUAL_BUILD}
        warn_NV(Format('Invalid calibration: %s', [aCalib.AsShortString]));
        {$ENDIF}
        AddMessage(vsError);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {$IFNDEF VISUAL_BUILD}
        warn_NV('Taxa name not found - ' + aCalib.NodeA);
        {$ENDIF}
      end
      else
    ...
    begin
      ...
      begin
        ...
        {$IFNDEF VISUAL_BUILD}
        warn_NV(Format('Invalid sample time: %.3e', [FTree[i].Value]));
        {$ENDIF}
        AddMessage(vsError);
        aCalib.IsValid := False;
        aCalib.IsSelected := False;
...
var
  ...
  hasIngroupRootCalib: Boolean = False;
begin
  Result := False;
  Msg := EmptyStr;
  HasMin := False;
  ...
  begin
    ...
    begin
      ...
      begin
        if hasIngroupRootCalib then
        begin
          Msg := 'only 1 calibration can be specified for the ingroup-root but more than 1 was found';
          Exit(False);
        end
        else
          hasIngroupRootCalib := True;
      end;
      if CompareValue(aCalib.MinTime, 0.0, FP_CUTOFF) > 0 then
        HasMin := True;
      if CompareValue(aCalib.MaxTime, 0.0, FP_CUTOFF) > 0 then
        HasMax := True;

      if HasMin and HasMax then
      begin
        if CompareValue(aCalib.MinTime, aCalib.MaxTime, FP_CUTOFF) > 0 then
        begin
          Msg := 'Invalid calibration: ' + aCalib.AsShortString;
          {$IFNDEF VISUAL_BUILD}
          warn_NV(Msg);
          {$ENDIF}
          AddMessage(vsError);
