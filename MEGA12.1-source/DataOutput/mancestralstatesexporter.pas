...
type
  ...
  TMLAncestralStatesExporter = class(TAncestralStatesExporter)
    ...
    public
      constructor Create(aMlAnalyzer: TMLTreeAnalyzer; {$IFDEF VISUAL_BUILD}aTreeBox: TTreeBox;{$ENDIF} aShowOption: String);
...
function TMLAncestralStatesExporter.GetNumNodes: Integer;
begin
  Result := 2*MLAnalyzer.NoOfSeqs - 1;
end;

constructor TMLAncestralStatesExporter.Create(aMlAnalyzer: TMLTreeAnalyzer; {$IFDEF VISUAL_BUILD}aTreeBox: TTreeBox;{$ENDIF} aShowOption: String);
begin
...
begin
  try
    ...
    Assert(Assigned(MLAnalyzer.SeqNames));
    ...
    for node := 0 to GetNumNodes - 2 do
    begin
      b.Clean;
      if node < MLAnalyzer.NoOfSeqs then
        NodeName := Format('>%s', [MLAnalyzer.SeqNames[node]])
      else
        NodeName := Format('>Node_%d %s', [node + 1, MLAnalyzer.CoordsName(node)]);

      NodeName := StringReplace(NodeName, ' ', '_', [rfReplaceAll]);
      Result.Add(NodeName);
      ...
      begin
        ...
        begin
          if usesCutoff then
          begin
            if CompareValue(tempProbs[0].Prob, AncestralStatesProbabilityCutoff, FP_CUTOFF) < 0 then
              b.Add('?')
            else
