...
const
  ...
  ESL_HELP_PAGE = 'https://github.com/kumarlabgit/MyESL/tree/DrPhylo';
...
begin
  ...
  begin
    ...
    {$IFDEF DARWIN}ClientHeight := ClientHeight + 20;{$ENDIF}
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMessage('The specified FASTA directory was not found');
        Exit;
      end;
    end;
    ...
    if not (DrpNumGroupsSpinEdit.Value > 0) then
    begin
      ShowMessage('"No. of Groups (columns)" must be a value greater than zero');
      Exit;
    end;
    if not (DrpNumTaxaSpinEdit.Value > 0) then
    begin
      ShowMessage('"No. of Species/Taxa (rows)" must be a value greater than zero');
      Exit;
    end;
    LaunchDrPhylo;
  end
...
begin
  {$IFDEF DARWIN}
  if not NumSitesPerPartitionSpinEdit.Enabled then
    Exit;
  {$ENDIF}
...
begin
  {$IFDEF DARWIN}
  if not UseGenesDomainsBtn.Enabled then
    Exit;
  {$ENDIF}
  ...
  begin
    if Assigned(D_InputSeqData) and Assigned(D_InputSeqData.AllDomainInfo) then
      begin
        ...
        {$IFDEF DARWIN}DrpNumGroupsSpinEdit.MaxValue := maxDomains;{$ENDIF}
