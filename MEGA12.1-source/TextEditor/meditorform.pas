...
uses
  ...
  ComCtrls, Menus, Buttons, ImgList, ActnList,
  ...
  mimageform, MegaPrivateFiles, recentlyusedfiles,
  {$ENDIF}
...
type
  ...
  private
    ...
    procedure DoConvertAction(data: PtrInt);
    ...
    function FindOpenTab(aCaption: AnsiString): Integer;
    ...
    procedure CloseSheet(aSheet: TTabSheet);
  ...
  public
    ...
    function CloseStringList(aTabCaption: AnsiString): Boolean;
  ...
  function  IsFileOpen( fn : TFileName ) : boolean;
  function StringListIsOpen(aCaption: AnsiString): Boolean;
  function CloseStringListTab(aTabCaption: AnsiString): Boolean;
  function  FindOpenFilePage(fn: TFileName): integer;
...
begin
  ...
  try
    ...
    on E:Exception do
      ShowMessage('Exception in FindTextEditorWindow: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    on E:Exception do
      ShowMessage('Exception in FindOpenFilePage: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    on E:Exception do
      ShowMessage('Exception in IsFileOpen: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    Result := (editor.FindOpenTab(aCaption) >= 0);
  except
    on E:Exception do
      ShowMessage('Exception in StringListIsOpen: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    editor := FindTextEditorWindow;
    Result := editor.CloseStringList(aTabCaption);
    if editor.PageControl1.PageCount = 0 then
      editor.Close;
  except
    on E:Exception do
      ShowMessage('Exception in CloseStringListTab: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    {$IFDEF DARWIN}
    // Mac OS workaround - force resize the window
    // prevents the text control from disappearing when multiple tabs are added programmatically
    // i.e. as with Little Bootstrap analyses
    ...
    on E:Exception do
      ShowMessage('Application error when opening a string list in the text editor: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    Result.AppendToRecentlyUsedFiles(FileName);
...
begin
  ...
  CloseBtn.Width := 30;
  CloseBtn.Height := 30;
  CloseBtn.Top := StandardToolBar.Height;
  CloseBtn.Left := ClientWidth - CloseBtn.Width - 1;
  {$ENDIF}
...
function TEditorForm.FindOpenTab(aCaption: AnsiString): Integer;
var
  InnerForm: TInnerForm = nil;
  i: Integer = -1;
  aTabCaption: AnsiString = '';
begin
  Result := -1;
  for i := 0 to PageControl1.PageCount - 1 do
  begin
    InnerForm := SheetToForm(PageControl1.Pages[i]);
    aTabCaption := Trim(InnerForm.Caption);
    if aTabCaption[1] = '*' then
      aTabCaption := Copy(aTabCaption, 2, Length(aTabCaption));
    if aTabCaption = trim(aCaption) then
      Exit(i);
  end;
end;
...
var
  i: Integer = -1;
  aMemo: TSynMemo = nil;
  aInnerForm: TInnerForm = nil;
begin
  Result := 0;
  for i := 0 to PageControl1.PageCount - 1 do
  begin
    aInnerForm := SheetToForm(PageControl1.Pages[i]);
    if Assigned(aInnerForm) then
    begin
      aMemo := aInnerForm.Memo;
...
begin
  try
    ...
    on E:Exception do
      ShowMessage('Application error when closing a text editor file: ' + E.Message);
  end;
end;

function TEditorForm.CloseStringList(aTabCaption: AnsiString): Boolean;
var
  index: Integer = -1;
  t: TTabSheet = nil;
begin
  try
    ...
    on E:Exception do
      ShowMessage('Application error when closing a text editor string list: ' + E.Message);
  end;
end;
...
begin
  ...
  {$ENDIF}
  except  { this is here to hide an EInvalidTypeCast error that sometimes occurs in the SynEdit code with Lazarus 2.2.6}
  {$IFDEF DEBUG}
    on E:Exception do
      ShowMessage('Error when closing the Text Editor: ' + E.Message);
  {$ENDIF}
  end;
end;

procedure TEditorForm.CloseAllTabsActionExecute(Sender: TObject);
var
  t: TTabSheet = nil;
  i: Integer = -1;
begin
...
procedure TEditorForm.CloseOtherTabsActionExecute(Sender: TObject);
var
  t: TTabSheet = nil;
  i: Integer = -1;
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            aForm := OpenStringList(aList, ExtractFilename(FileNames[i]), True);
            AppendToRecentlyUsedFiles(Filenames[i]);
            if Assigned(aForm) then
            begin
              innerForm := aForm.GetActiveInnerForm;
              if Assigned(innerForm) then
                innerForm.FileName := Filenames[i];
            end;
            aList.Clear;
          end;
        end;
...
begin
  try
    ...
    on E:Exception do
      ShowMessage('Failed to initialize the help browser: ' + E.Message);
  end;
end;

procedure TEditorForm.HelpMenuItemClick(Sender: TObject);
var
  aHelp: String = '';
begin
  aHelp := MapHelpContextToKeyword(HC_Text_Editor);
  if aHelp <> EmptyStr then
    ShowContextSensitiveHelp(aHelp)
  else
    ShowContextSensitiveHelp(HelpKeyword);
end;
...
begin
  ...
  {$IFDEF DEBUG}
    on E: Exception do
      ShowMessage('Exception in MemoStatusChange: ' + E.Message);
  {$ENDIF}
  end;
end;
...
begin
  ...
  {$IFDEF DEBUG}
    on E:Exception do
      ShowMessage('Exception in MemoStateChange: ' + E.Message);
  {$ENDIF}
  end;
end;
...
var
  aForm: TInnerForm = nil;
begin
  aForm := GetActiveInnerForm;
  if Assigned(aForm) then
    Result := aForm.Memo
  else
    Result := nil;
...
begin
  ...
  begin
    ...
    try
      ...
      AppendToRecentlyUsedFiles(OpenDialog1.FileName);
    Except on E:Exception do
      begin
        if E.ClassType = EOutOfMemory then
          MessageDlg('The file you are trying to open is too large for MEGAs text editor to handle.', mtError, [mbOK], 0)
...
procedure TEditorForm.PageControl1ContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean);
var
  p: TPoint;
begin
  CloseOtherTabsAction.Enabled := (PageControl1.PageCount > 1);
  CloseAllTabsAction.Enabled := (PageControl1.PageCount > 1);
  p := ClientToScreen(MousePos);
  TabsPopupMenu.PopUp(p.X, p.Y);
  Handled := True;
end;
...
begin
  ...
  {$IFDEF DEBUG}
    on E:Exception do
      ShowMessage('Exception in GetActiveInnerForm: ' + E.Message);
  {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    try
      // Use the suggested name as the default filename otherwise it's blank.
...
begin
  InnerForm := GetActiveInnerForm;
  if FileExists(InnerForm.FileName) then
    SaveLocation := OutputUtils.AskUserForFileSaveLocation(InnerForm.FileName)
  else
...
procedure TEditorForm.CloseActiveSheet;
var
  InnerForm: TInnerForm = nil;
  DoSave: Integer;
  NewFileName: String;
begin
  ...
  try
    ...
    on E:Exception do
      ShowMessage('Application error when closing the active tab: ' + E.Message);
  end;
end;

procedure TEditorForm.CloseSheet(aSheet: TTabSheet);
var
  InnerForm: TInnerForm;
  DoSave: Integer;
  NewFileName: String;
begin
  InnerForm := SheetToForm(aSheet);
  if not Assigned(InnerForm) then
    Exit;
  ...
  try
    ...
    on E:Exception do
      ShowMessage('Application error when closing a text editor tab: ' + E.Message);
  end;
end;
...
begin
  Application.QueueAsyncCall(DoConvertAction, placeholder);
end;
...
var
  ...
  InnerForm: TInnerForm;
begin
  if FFormIsClosing or (not Visible) then
    Exit;
...
begin
  ...
  try
    ...
    {$IFDEF DEBUG}
    on E:Exception do
      ShowMessage('Exception in ActionList1Update: ' + E.Message);
    {$ENDIF}
  end;
end;
    ...
    for i := 0 to aFiles.Count - 1 do
      ...
      begin
        ...
        try
          ...
          AppendToRecentlyUsedFiles(aFiles[i]);
        Except on E:Exception do
          begin
            if E.ClassType = EOutOfMemory then
              MessageDlg('The file you are trying to open is too large for MEGAs text editor to handle.', mtError, [mbOK], 0)
...
procedure TEditorForm.DoConvertAction(data: PtrInt);
var
  i : integer = -1;
  InnerForm: TInnerForm = nil;
  TempInnerForm: TInnerForm = nil;
  FileName : String = '';
  outstrlist : TStrings = nil;
  in_type : TImportFormatType;
  AConvertOptDlg : TFormatConvertOptDlg = nil;
  AConverter     : TFormatConvertToMega = nil;
...
begin
  ...
  try
    ...
    begin
      ...
      on E:Exception do
        ShowMessage('An application error has occured: ' + E.Message);
    end;
  finally
...
procedure TEditorForm.InitRecentlyUsedFiles;
var
  storage: String;
  i: Integer;
  aMenuItem: TMenuItem;
begin
  storage := GetPrivateFile(mfEditorMruFiles, False);
...
procedure TEditorForm.RecentlyUsedFilesItemClick(Sender: TObject);
var
  filename: String = '';
...
begin
  ...
  begin
    InnerForm := sheetToForm(PageControl1.Pages[i]);
    if InnerForm.FileName = filename then
    begin
      PageControl1.ActivePageIndex := i;
      Exit;
    end;
...
procedure TEditorForm.AppendToRecentlyUsedFiles(aFilename: String);
var
  aMenu: TMenuItem;
begin
  try
    ...
    on E:Exception do
      ShowMessage('Application error when updating the text editor recently used files list: ' + E.Message);
  end;
end;
...
begin
  ...
  {$IFDEF DEBUG}
     on E:Exception do
       ShowMessage('Exception in TEditorForm.FormActivate: ' + E.Message);
  {$ENDIF}
  end;
end;
