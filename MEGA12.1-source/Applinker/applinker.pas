...
begin
  ...
  try
    ...
    begin
      Result := False;
      MessagesLog.Add(FMuscleLink.ErrorCodeMessage);
    end
    else
      Result := True;
  except
...
begin
  try
    try
      ...
      begin
        if (AppExitCode = 2) or FNewLine.Contains('OUT OF MEMORY') then
          ErrorCode := notEnoughMemory
        else
          ErrorCode := UnknownErr;
        raise Exception.Create(ExtractFileName(FExe) + ' finished with exit code ' + IntToStr(AppExitCode));
      end;
      SetCurrentDirUTF8(UserDir);
    except
      on E:Exception do
...
begin
  case ErrorCode of
     noOutput: Result := 'The MUSCLE application did not produce any output.';
     notAbleToSave, UnknownErr, UnknownOption: Result := 'MUSCLE alignment failed with an unknown error.';
     notFinished: Result := 'The MUSCLE log file did not end properly, suggesting an unhandled exception.';
     notEnoughMemory, notEnoughMemorySpecified: Result := 'MUSCLE alignment failed with an "OUT OF MEMORY" error. The version of MUSCLE being used is a 32-bit application and is limited in the amount of memory it can use.';
     noLogFile: Result := 'No MUSCLE alignment log file was found which indicates that MUSCLE crashed.';
     FatalException: Result := 'The MUSCLE application reported a "Fatal Exception".';
     noError: Result := 'no error';
     else
       Result := 'MUSCLE produced an unknown error code';
  end;
end;
