...
uses
  ...
  fpjson, jsonparser, PopupNotifier, AppLinker, mdistpack,
...
type
  ...
  TMegaForm = class(TForm)
    ...
    GenerateCalibrationsAction: TAction;
    AskMegaGptAction: TAction;
    ...
    procedure AskMegaGptActionExecute(Sender: TObject);
    ...
    procedure GenerateCalibrationsActionExecute(Sender: TObject);
    procedure MegaGptLinkClick(Sender: TObject);
    procedure MegaGptLinkMouseEnter(Sender: TObject);
    procedure MegaGptLinkMouseLeave(Sender: TObject);
  ...
  private
    ...
    FShowDataBtn: TToolButton;
     ...
     FTrayPopupMenusToRemove: array of TPopupMenu;
     FUserWasAskedToSaveSession: Boolean;
     FUserWasWarnedAboutRunningCalculations: Boolean;
     FUserConfirmedCloseMega: Boolean;
     procedure DoExportData(data: PtrInt);
     procedure DoOpenAlignmentAction(data: PtrInt);
     procedure DoBuildAlignment(data: PtrInt);
     procedure DoEditSequencerAction(data: PtrInt);
     procedure DoConcatenateFiles(data: PtrInt);
     procedure DoOpenFileAction(data: PtrInt);
     procedure ConfirmCloseMega(data: PtrInt);
     procedure WarnUserThatCalculationsAreRunning(data: PtrInt);
     procedure CheckSaveDataSession(data: PtrInt);
     procedure HandleFastaFile(filename: PtrInt);
     procedure InitTimetreeWizard(operation: TDistTreeDlgOption);
     procedure SetDataCollectPrefsUpdated(value: Boolean);
     function GetDataCollectPrefsUpdated: Boolean;
   ...
   public
     ...
     {$IFDEF DEBUG}
     procedure DevActionNotify(Sender: TObject);
     {$ENDIF}
     ...
     procedure DoDrPhyloThreadDone(data: PtrInt);
     ...
     procedure MagnifyTrayIcon(Btn: TToolButton); deprecated;
...
uses
  ...
  MExampleFiles, Printers, math, ExcelWrite,
  ...
  mwebdialogfiles, MMdWelcomeForm, MCalibrationDlg, Walk_Through_MEGA,
  ...
  mcustom_msg_dlg, MDisplayMatrixDlg, MDomainInfo, mdisplay_settings_form,
  mdeveloper_settings_form, mesl_options_dlg, MutationExplorer, MCalibrationData,
  mesl_menu_builder, mnewick_export_thread, mmega_font_dialog, non_modal_message_dlg,
  dr_phylo_example_files, mdata_collection_settings_form, syncobjs;
...
var
  placeholder: PtrInt = 0;
begin
  Application.QueueAsyncCall(DoOpenAlignmentAction, placeholder);
end;
...
begin
  if IsPrototyper then
    Exit;
  Application.QueueAsyncCall(DoOpenFileAction, placeholder);
end;
...
begin
  ...
  Except
    on E: Exception do
      ShowMessage('An application error occurred when displaying the web browser: ' + E.Message);
  end;
end;
...
begin
  try
    try
      ...
      begin
        if (Sender = ReltimeBlensMenuItem) or (Sender = ReltimeBLensMenu2) then
          aOperation := dtdoRelTimeBLens
        else if (Sender = RtdtBlensMenuItem) or (Sender = RtdtBlensMenu2) then
          aOperation := dtdoRtdtBLens
        else
          raise Exception.Create('invalid caller for Reltime branch lengths action');
        InitTimetreeWizard(aOperation);
        ProcessPack := TProcessPack.Create;
        if (Sender = ReltimeBlensMenuItem) or (Sender = ReltimeBLensMenu2) then
        begin
          ProcessPack.AddProcessType(ppRelTimeBlens);
...
begin
  ...
  Except
    on E: Exception do
      ShowMessage('An application error occurred when saving data: ' + E.Message);
  end;
end;
...
procedure TMegaForm.DoExportData(data: PtrInt);
begin
  try
    if not HasActiveData then
    begin
  ...
  Except
    on E: Exception do
      ShowMessage('An application error occurred when exporting data: ' + E.Message);
  end;
end;

procedure TMegaForm.DoOpenAlignmentAction(data: PtrInt);
begin
  try
  ...
  Except
    ...
    begin
      ...
      ShowMessage('An application error occurred when activating the Alignment Explorer: ' + E.Message);
    end;
  end;
end;

procedure TMegaForm.DoBuildAlignment(data: PtrInt);
var
  editor: TAlignEditMainForm = nil;
begin
  try
...
procedure TMegaForm.DoEditSequencerAction(data: PtrInt);
var
  TrEdt: TTraceEditForm = nil;
begin
  try
...
procedure TMegaForm.DoConcatenateFiles(data: PtrInt);
var
...
procedure TMegaForm.DoOpenFileAction(data: PtrInt);
var
  aFilename: String = '';
  aFileExt: String = '';
  MyOpenDlg: TOpenDialog = nil;
begin
  try
    try
      MyOpenDlg := TOpenDialog.Create(nil);
      ...
      on E: Exception do
        ShowMessage('An application error occurred when opening the data file: ' + E.Message);
    end;
  finally
    if Assigned(MyOpenDlg) then
      MyOpenDlg.Free;
  end;
end;

procedure TMegaForm.ConfirmCloseMega(data: PtrInt);
var
  checked: Boolean = False;
  aSyncChecked: TASyncBoolean = nil;
begin
  try
    aSyncChecked := TASyncBoolean(data);
    checked := aSyncChecked.Value;
    if MessageDlgCheck(Self, CONFIRM_CLOSE_MEGA_STRING, 'Closing MEGA', UserPref_DoNotWarnOnMEGACloseStr, checked, mtConfirmation, [mbYES, mbNO], 0) = mrYes then
    begin
      UpdatePreferencesMainMenu(UserPref_MegaMainDoNotWarnOnMEGACloseStr, checked, BoolToStr(True, True));
      FUserConfirmedCloseMega := True;
      Close;
    end
    else
    begin
      UpdatePreferencesMainMenu(UserPref_MegaMainDoNotWarnOnMEGACloseStr, checked, BoolToStr(False, True));
      FUserConfirmedCloseMega := False;
    end;
  finally
    if Assigned(aSyncChecked) then
      aSyncChecked.Free;
  end;
end;

procedure TMegaForm.WarnUserThatCalculationsAreRunning(data: PtrInt);
begin
  FUserWasWarnedAboutRunningCalculations := True;
  if MessageDlg('Confirm cancel calculation', 'At least one calculation is currently running in MEGA. Are you sure you want to close MEGA?', mtConfirmation, mbYesNo, 0) = mrYes then
    Close;
end;

procedure TMegaForm.CheckSaveDataSession(data: PtrInt);
var
  aMsg: String = '';
  response: Integer = -1;
  info: TStringList = nil;
  title: String = '';
  yesBtn: String = '';
  noBtn: String = '';
  cancelBtn: String = '';
begin
  try
    info := TStringList(data);
    if (info.Count < 4) or (info.Count > 5) then
      ShowMessage(Format('expected 4 or 5 parameters in "CheckSaveDataSession" but got %d', [info.Count]))
    else
    begin
      title := info[0];
      aMsg :=  info[1];
      ...
      begin
        response := QuestionDlg(title, aMsg, mtCustom, [mrYes, yesBtn, mrNo, noBtn], -1);
        if response = mrYes then
          SaveSession(False);
        FUserWasAskedToSaveSession := True;
        ClearData;
        if FIsClosing then
          Close;
      end
      else if info.Count = 5 then
      begin
        cancelBtn := info[4];
        response := QuestionDlg(title, aMsg, mtCustom, [mrYes, yesBtn, mrNo, noBtn, mrCancel, cancelBtn], -1);
        case response of
         mrYes:
          begin
            SaveSession(True);
            ...
            if FIsClosing then
              ...
              FUserWasAskedToSaveSession := False;
          end;
         mrNo:
          begin
            ClearData;
...
procedure TMegaForm.HandleFastaFile(filename: PtrInt);
var
  payload: TASyncString = nil;
  aFilename: UnicodeString = '';
  isSuccess: Boolean = False;
  response: Integer = -1;
begin
  payload := TASyncString(filename);
  aFilename := payload.Value;
  response := QuestionDlg('How would you like to open this fasta file?', 'Analyze or Align File?', mtInformation, [21, 'Align', 22, 'Analyze'], 0);
  if response = 21 then
    ShowAlignmentFromFile(aFilename)
  else if response = 22 then
  begin
    ...
    isSuccess := DoOpenDataFile(aFilename);
    if isSuccess then
    begin
      if not IsPrototyper then
      begin
        UpdateRecentFilesList(aFileName);
...
procedure TMegaForm.InitTimetreeWizard(operation: TDistTreeDlgOption);
var
  showCloseDataBtn: Boolean = False;
begin
  TimeTreeWizard := TTimeTreeWizard.Create;
  TimeTreeWizard.UsrOperation := operation;
  if operation in [dtdoRtdtBlens, dtdoRtdtLS, dtdoRtdtML] then
    TimeTreeWizard.CalibrationType := ctLeafNode;
  TimeTreeWizardForm := TTimeTreeWizardForm.Create(Self);
  TimeTreeWizardForm.PopupMode := pmExplicit;
  TimeTreeWizardForm.PopupParent := Self;
  TimetreeWizardForm.ShowInTaskBar := stNever;
  TimetreeWizardForm.Position := poOwnerFormCenter;
  TimeTreeWizardForm.Wizard := TimeTreeWizard;
  TimeTreeWizard.WizardForm := TimeTreeWizardForm;
  TimeTreeWizard.WizardFormUpdate := TimeTreeWizardForm.UpdateView;
  showCloseDataBtn := (not (operation in [dtdoCorrTestBlens, dtdoRelTimeBLens, dtdoRtdtBlens, dtdoReltimeCalibrations]));
  AddWindowToTray(TimeTreeWizardForm, showCloseDataBtn);
end;

procedure TMegaForm.SetDataCollectPrefsUpdated(value: Boolean);
begin
  CheckUpdatesCS.Acquire;
  try
    FDataCollectPrefsUpdated := value;
  finally
    CheckUpdatesCS.Release;
  end;
end;

function TMegaForm.GetDataCollectPrefsUpdated: Boolean;
begin
  CheckUpdatesCS.Acquire;
  try
    Result := FDataCollectPrefsUpdated;
  finally
    CheckUpdatesCS.Release;
  end;
end;
...
begin
  if GetDataCollectPrefsUpdated then
    Exit;

  settingsFile := GetMegaGlobalFile('') + DATA_COLLECT_PREFS_FILE;
  if FileExists(settingsFile) then
  begin
    SetDataCollectPrefsUpdated(True);
...
begin
  ...
  try
    try
      ...
      dataCollectionSettings := TDataCollectionSettingsForm.Create(Self);
      dataCollectionSettings.SetCurrentOptions(collectionAllowed);
      dataCollectionSettings.ShowModal;
    except
      on E:Exception do
...
begin
  ...
  try
    ...
    begin
      if IsSessionTest then
      begin
        AppendSessionLog(Format('%s : FAILED session test for file %s: %s', [FormatDateTime('YYYY/MM/DD - hh:nn:ss', Now), aFile, E.Message]));
        Halt(MEGA_ERROR_EXIT_CODE);
      end;
      ShowMessage('An error occurred when processing command line parameters: ' + E.Message);
    end;
  end;
end;
...
begin
  ...
  if not t.IsSuccess then
    ShowMessage(Format('Application (build %s) error. Failed to extract DrPhylo example files: %s', [VER_MEGA_BUILD, t.ErrMsg]));
end;
...
procedure TMegaForm.DoDrPhyloThreadDone(data: PtrInt);
var
  aThread: TEslLinkerThread = nil;
  aLink: TEslLinker = nil;
  aLog: TStringList = nil;
...
begin
  ...
  begin
    ...
    if Assigned(EslAppLinksList) and (EslAppLinksList.Count > 1) then
      ShowMessage('Displayed is the DrPhylo Model-grid. You can select individual results files to view by clicking on the right-most "DrP" icon on the main form.')
    else
      ShowMessage('Displayed is the DrPhylo Model-grid. You can select individual results files to view by clicking on the "DrP" icon on the main form.');
  end;
end;
...
var
  aThread: PtrInt = 0;
begin
  aThread := PtrInt(Thread);
  Application.QueueASyncCall(DoDrPhyloThreadDone, aThread);
end;
...
begin
  ...
  DataPrinterSetupAction.Visible := False;
  DataPrinterSetupAction.Enabled := False;
  {$ENDIF}
end;
...
begin
  ...
  FeedbackLink.Font.Assign(PublicationsLink.Font);
  FeedbackLink.Visible := True;
  FeedbackLink.Alignment := taCenter;
  FeedbackLink.Parent := BottomPanel;
  FeedbackLink.Left := Round((BottomPanel.Width - FeedbackLink.Width)/2);
  TopToolbar.Top := 1;
  PinupsToolbar.Top := TrayToolbar2.Top + TrayToolbar2.Height;
  BetaLabel.Top := PinupsToolbar.Top + PinupsToolbar.ButtonHeight + 10;
  FeedbackLink.Top := PublicationsLink.Top;
...
begin
  ...
  begin
    try
      ...
      on E:Exception do
        ShowMessage('Application error in the OnIdle function: ' + E.Message);
    end;
  end;

  if Length(FTrayPopupMenusToRemove) = 0 then
  begin
    Done := True;
    Exit;
  end;
  ...
  try
    for i := High(FTrayPopupMenusToRemove) to Low(FTrayPopupMenusToRemove) do
    begin
      pm := FTrayPopupMenusToRemove[i];
      RemoveDropdownFromTray(pm);
      pm.Free;
      SetLength(FTrayPopupMenusToRemove, Length(FTrayPopupMenusToRemove) - 1);
    end;
    Done := True;
  except
    on E:Exception do
      ShowMessage('An application error occurred in the idle function: ' + E.Message);
  end;
end;
...
begin
  ...
  begin
    ...
    if AThread.TreeList.HasParserWarnings then
      ShowMessage(Format('The Newick parser generated the following warnings: %s%s%s Please check the tree carefully.', [LineEnding, AThread.TreeList.ParserWarnings, LineEnding]));
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        MegaForm.SetFocus;
        Application.QueueAsyncCall(HandleFastaFile, PtrInt(fname));
      end;
    end
...
{$IFDEF DEBUG}
procedure TMegaForm.DevActionNotify(Sender: TObject);
var
  aButton: Integer = -1;
begin
  with Sender as TNonModalMessageDlg do
  begin
    aButton := ClickedButton;
    ShowMessage('DevActionNotify called: ' + Sender.ClassName + ' ' + IntToStr(aButton));
  end;
end;
{$ENDIF}
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          SetLength(FTrayPopupMenusToRemove, Length(FTrayPopupMenusToRemove) + 1);
          FTrayPopupMenusToRemove[Length(FTrayPopupMenusToRemove) - 1] := aLink.ResultsPopup;
          aLink.ResultsPopup := nil;
...
begin
  exit; // deprecated - atleast until the code below can be refactored
...
begin
  try
    ...
    begin
      ...
      begin
        info.Add('Save Data Changes?');
        info.Add('Would you like to save your changes back to the open MDSX session file?');
        info.Add('Save Changes');
        info.Add('Discard Changes');
        Application.QueueAsyncCall(CheckSaveDataSession, PtrInt(info));
      end
      else if HasDataSubsetChanged then
      begin
        {$IFNDEF DEBUG} { don't prompt developers. It is just annoying. }
        info.Add('Save Data Session?');
        info.Add('Your data subset selection can be preserved by creating a session file. Would you like to create one?');
        info.Add('Save Session...');
        info.Add('Close Without Saving');
        info.Add('Cancel');
        Application.QueueAsyncCall(CheckSaveDataSession, PtrInt(info));
        {$ENDIF}
      end
      ...
      begin
        ...
        {$IFNDEF DEBUG}
        info.Add('Save Data Session?');
        info.Add('You can preserve your data view by creating a session file. Would you like to create one?');
        info.Add('Save Session...');
        info.Add('Close Without Saving');
        info.Add('Cancel');
        Application.QueueAsyncCall(CheckSaveDataSession, PtrInt(info));
...
var
  placeholder: PtrInt = 0;
begin
  Application.QueueAsyncCall(DoBuildAlignment, placeholder);
end;
...
begin
  ...
  try
    try
      ...
      begin
        if (Sender = ReltimeOlsMenuItem) or (Sender = ReltimeOLSMenu2) then
          aOperation := dtdoRelTimeLS
        else if (Sender = RtdtLsMenuItem) or (Sender = RtdtOLSMenu2) then
          aOperation := dtdoRtdtLS
        else
          raise Exception.Create('Invalid caller for Reltime OLS action');
    ...
    Except
    on E: Exception do
      ShowMessage('An application error occurred when setting up the Reltime analysis: ' + E.Message);
    end;
  finally
    if Assigned(ProcessPack) then
      ProcessPack.Free;
...
var
  placeholder: PtrInt = 0;
begin
  Application.QueueAsyncCall(DoConcatenateFiles, placeholder);
end;
...
begin
  ...
  Except
    on E: Exception do
      ShowMessage('An application error occurred when converting to the MEGA format: ' + E.Message);
  end;
end;
...
begin
  ...
  try
    try
      InitTimetreeWizard(dtdoCorrTestBlens);
      ProcessPack := TProcessPack.Create();
      ProcessPack.AddProcessType(ppCorrtestBlens);
      TimeTreeWizard.ProcessPack := ProcessPack;
      ProcessPack := nil;
...
begin
  ...
  try
    InitTimetreeWizard(dtdoCorrTestML);
    ProcessPack := TProcessPack.Create();
    ProcessPack.AddProcessType(ppCorrTestML);
    TimeTreeWizard.ProcessPack := ProcessPack;
...
begin
  try
    ...
    on E: Exception do
     ShowMessage('An error occurred when setting up a printer: ' + E.Message);
  end;
end;
...
var
  i: Integer;
begin
  DeleteRecoveryFile(GetPrivateFile(NewickRecoveryFile, False));
  FIsClosing := True;
  {$IFNDEF DEBUG}
  if Assigned(FUniqueInstance) then
    FUniqueInstance.Free;
  {$ENDIF}
  if Assigned(SaveNewickCS) then
    FreeAndNil(SaveNewickCS);
  if Assigned(CheckUpdatesCS) then
    FreeAndNil(CheckUpdatesCS);
  {$IFNDEF MSWINDOWS}
...
procedure TMegaForm.AskMegaGptActionExecute(Sender: TObject);
begin
  OpenUrl(MegaGptUrl);
end;
...
begin
  ...
  if not GetDataCollectPrefsUpdated then
  begin
    GetUserType;
...
procedure TMegaForm.GenerateCalibrationsActionExecute(Sender: TObject);
begin
  try
    if IsPrototyper then
      raise Exception.Create('Calibrations cannot be generated when in "Prototype" mode. Please switch to "Analyze" mode first.');
    InitTimetreeWizard(dtdoReltimeCalibrations);
    TimeTreeWizard.UsrOperation := dtdoReltimeCalibrations;
    TimeTreeWizard.WizardMode := ttwmReltimeCalibrations;
    TimeTreeWizardForm.Show;
    TimeTreeWizardForm := nil; { freed automatically when setting CloseAction := caFree in OnClose}
  Except
  on E: Exception do
...
procedure TMegaForm.MegaGptLinkClick(Sender: TObject);
begin
  OpenUrl(MegaGptUrl);
end;

procedure TMegaForm.MegaGptLinkMouseEnter(Sender: TObject);
begin
  MegaGptLink.Font.Color := $00379800;
  MegaGptLink.Invalidate;
end;

procedure TMegaForm.MegaGptLinkMouseLeave(Sender: TObject);
begin
  MegaGptLink.Font.Color := $122a0000;
  MegaGptLink.Invalidate;
end;
...
begin
  ...
  try
    ...
    Except
      on E: Exception do
        ShowMessage('An application error has occurred so that the analysis could not be completed: ' + E.Message);
    end;
  finally
    if Assigned(ProcessPack) then
      ProcessPack.Free;
...
var
  placeholder: PtrInt = 0;
begin
  Application.QueueAsyncCall(DoEditSequencerAction, placeholder);
end;
...
var
  placeholder: PtrInt = 0;
begin
  Application.QueueAsyncCall(DoExportData, placeholder);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      on E:Exception do
      begin
        {$IFNDEF DARWIN} { can't use a modal dialog here on macOS due to random crashes}
        ShowMessage('Error when saving preferences: ' + E.Message);
        {$ENDIF}
      end;
    end;
    CanClose := True;
  ...
  if (FNumCalculationsRunning > 0) and (not FUserWasWarnedAboutRunningCalculations) then
  begin
    CanClose := False;
    Application.QueueAsyncCall(WarnUserThatCalculationsAreRunning, PtrInt(placeholder));
    Exit;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        {$IFNDEF DEBUG}
        aSyncChecked := TASyncBoolean.Create;
        aSyncChecked.Value := checked;
        if not FUserConfirmedCloseMega then
        begin
          CanClose := False;
          Application.QueueAsyncCall(ConfirmCloseMega, PtrInt(aSyncChecked));
          Exit;
        end;
        {$ENDIF}
      end;
  ...
  FIsClosing := True;
  {$IFDEF DEBUG}
  FUserWasAskedToSaveSession := True; { to avoid the prompt when debugging}
  {$ENDIF}
  CanClose := FUserWasAskedToSaveSession;
  if not FUserWasAskedToSaveSession then
    SaveSessionWithPermission;
...
begin
  {$IFDEF LINUX}
  top1 := DrPhyloBtn.Top;
  top2 := TimeTreeBtn.Top;
  DrPhyloAction.Enabled := False;
  DrPhyloBtn.Visible := False;
  TimetreeBtn.Top := top1;
  DataMonkeyBtn.Top := top2;
  {$ENDIF}
  FUserWasWarnedAboutRunningCalculations := False;
  FUserWasAskedToSaveSession := False;
  FUserConfirmedCloseMega := False;
  SetLength(FTrayPopupMenusToRemove, 0);
  SaveNewickCS := TCriticalSection.Create;
  CheckUpdatesCS := TCriticalSection.Create;
  {$IFNDEF MSWINDOWS}
