...
type
  ...
  TAlignEditMainForm = class(TForm)
    ...
    procedure ActionShowSeqInfoExecute(Sender: TObject);
  ...
  private
    ...
    procedure CheckSaveChanges(Data: PtrInt);
...
procedure TAlignEditMainForm.MenuItem82Click(Sender: TObject);
begin
  OpenUrl(MegaGptUrl);
end;
...
procedure TAlignEditMainForm.CheckSaveChanges(Data: PtrInt);
var
  response: Integer = -1;
begin
...
procedure TAlignEditMainForm.ActionShowSeqInfoExecute(Sender: TObject);
begin
  ActionShowSeqInfo.Checked := not ActionShowSeqInfo.Checked;
  if Assigned(SourceAlignGrid) then
  begin
    SourceAlignGrid.ShowInfoInName := ActionShowSeqInfo.Checked;
    SourceAlignGrid.Invalidate;
  end;
  if Assigned(TranslatedAlignGrid) then
  begin
    TranslatedAlignGrid.ShowInfoInName := ActionShowSeqInfo.Checked;
    TranslatedAlignGrid.Invalidate;
  end;
  Invalidate;
end;
...
begin
 ...
 try
   ...
   aDlg.Initialize(CurAlignGrid.Font);
 ...
 {$ELSE}
 FontDlg.Font.Assign(CurAlignGrid.Font);
 if FontDlg.Execute then
 begin
   aFont := TFont.Create;
   aFont.Assign(FontDlg.Font);
 end;
 {$ENDIF}

 if Assigned(aFont) then
 begin
   SourceAlignGrid.Font.Assign(aFont);
   TranslatedAlignGrid.Font.Assign(aFont);
...
procedure TAlignEditMainForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
var
  data: PtrInt;
begin
  CanClose := FCanClose;
  if not FCanClose then
    Application.QueueAsyncCall(CheckSaveChanges, data);
end;
...
begin
  ...
  begin
    ...
    ActionEditEnabled.Enabled := True; { needs to always be true, regardless of FHasData}
    ...
    try
      ...
      begin
        ...
        if Trim(Msg) <> EmptyStr then
          ...
          ShowMessage('MUSCLE alignment failed with an unknown error');
      end
      else
        case aLink.ErrorCode of
          noOutput: ShowMessage('Alignment Failed. The MUSCLE application did not produce any output.');
          notAbleToSave, UnknownErr, UnknownOption: ShowMessage('MUSCLE alignment failed with an unknown error.');
          notFinished: ShowMessage('Alignment Failed. The MUSCLE log file did not end properly, suggesting an unhandled exception.');
          notEnoughMemory, notEnoughMemorySpecified: ShowMessage('MUSCLE alignment failed with an "OUT OF MEMORY" error.' {$IFDEF MSWINDOWS} + 'The MUSCLE version used is a 32-bit application and is limited in the amount of memory it can use.'{$ENDIF});
          noLogFile: ShowMessage('Alignment Failed. No MUSCLE alignment log file was found which indicates that MUSCLE crashed.');
          FatalException: ShowMessage('Alignment Failed. THe MUSCLE application reported a "Fatal Exception".');
          noError:
            begin
