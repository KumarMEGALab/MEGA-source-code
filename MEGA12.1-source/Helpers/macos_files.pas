...
function GetAppSupportDir(subDirectory: String): String;
{$ENDIF}
...
{$IFDEF DARWIN}
uses
  MegaUtils, Forms, MegaConsts, MegaPrivateFiles, MegaVerConsts, LazFileUtils;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Result := UTF8ToAnsi(StrPas(pathBuffer)) + PathDelim + ApplicationName {$IFDEF DEBUG}+ '_DEBUG'{$ENDIF} + PathDelim;
        if subDirectory <> EmptyStr then
          Result := ChompPathDelim(Result + subDirectory) + PathDelim;
      end;
...
var
  ...
  FullPath: String;
begin
  UserDir := GetAppSupportDir(MEGA_USER_DIR);
