...
const
  ...
  GUI_CITATION_PAGES = '41:1-9';
  {$ELSE}
  GUI_CITATION_TITLE = 'MEGA 12.1: Cross-platform release for macOS and Linux operating systems';
  GUI_CITATION_YEAR = '2025';
  GUI_CITATION_AUTHORS = 'Stecher G, Suleski M, Tao Q, Tamura K, and Kumar S';
  GUI_CITATION_JOURNAL = 'Journal of Molecular Evolution';
  GUI_CITATION_PAGES = '(submitted)';
  {$ENDIF}
...
{ NOTE: This is where the template for "MegaCitation" is set (instead of getting
  it from MTemplateConstants.ini). It is set here to handle different major version
  numbers. This is so that opening session files made from older versions of MEGA will have the
  citation referencing the version of MEGA used for originally generating the session file.}
function MapMajorVersionToCitation(aMajVersion: Integer): String;
begin
  case aMajVersion of
    ...
    12:
      begin
        {$IFNDEF MSWINDOWS} { per Sudhir, with MEGA 12.1 on Linux and macOS, captions should reference the 12.1 paper but on Windows, captions should reference the 12.0 paper}
        Result := 'MEGA_12_1';
        {$ELSE}
        Result := 'Kumar_et_al_2022';
        {$ENDIF}
      end
    else
      Result := EmptyStr;
