...
type
  TMegaAction = (maFindAlignment, maFindTrees, maFindTipDates, maProcessCommands, maConcatenateAlignments,
                 ...
                 maDomainsToFasta, maSegmentsToFasta, maGenerateTopHapAlignment);
  ...
  TD_MegaMain = class
    private
      ...
      FMinAlleleFreq: Double;
      FMaxAlleleFreq: Double;
      FFirstSeqIsRefSeq: Boolean;
      FUseRefSeqInFreqCalculation: Boolean;
      {$IFNDEF MSWINDOWS}
      procedure UpdatePeakMemUsage;
      {$ENDIF}
    procedure DomainsToFasta;
    procedure SegmentsToFasta;
    procedure GenerateTopHapAlignment;
...
uses
  ...
  mrobinson_foulds_metric, mtimetree_pruner, mallele_frequency_search;
...
begin
  try
    filename := ExtractFilePath(ExtractFileDir(Application.ExeName)) + 'megacc.log';
...
begin
  ...
  if FDataType <> snNoToken then
    FProcessPack.TextualSettingsList.Add(Format('datatype=%s', [MapTokenCodeToDataTypeString(FDataType)]));
end;
...
begin
  ...
  begin
    ...
    else if (TempStr = '-gtha') or (TempStr = '--generate-tophap-alignment') then
      FMegaAction := maGenerateTopHapAlignment
    else if (TempStr = '-min-allele-freq') or (TempStr = '--min-allele-freq') then
      FMinAlleleFreq := StrToFloat(ParamStr(i + 1))
    else if (TempStr = '-max-allele-freq') or (TempStr = '--max-allele-freq') then
      FMaxAlleleFreq := StrToFloat(ParamStr(i + 1))
    else if (TempStr = '-first-seq-is-ref-seq') or (TempStr = '--first-seq-is-ref-seq') then
      FFirstSeqIsRefSeq := True
    else if (TempStr = '-use-ref-seq-in-calc') or (TempStr = '--use-ref-seq-in-calc') then
      FUseRefSeqInFreqCalculation := True
    ...
    begin
      ...
      else if (ParamStr(i) = '-aspc') or (ParamStr(i) = '--ancestral-states-probability-cutoff') then
      begin
        if not TryStrToFloat(ParamStr(i + 1), tempDouble) then
          error_nv(Format('the --ancestral-states-probability-cutoff parameter must be a floating point value but got "%s"', [ParamStr(i + 1)]));
        if not InRange(tempDouble, 0.0, 1.0) then
          error_nv(Format('the --ancestral-states-probability-cutoff parameter must be a value between 0.0 and 1.0 but got %.8f', [tempDouble]));
        AncestralStatesProbabilityCutoff := tempDouble;
      end
...
begin
  ...
  writeLn('   -aspc --ancestral-states-probability-cutoff *optional *only used for ML ancestral states inferrence');
  writeLn('       This option applies to the FASTA file output of ancestral sequences that is produced when');
  writeLn('       running the ML ancestral sequence inferrence analysis. States with a probability less');
  writeLn('       than the provide value will written as missing data (i.e. written as "?").');
  writeLn('       The value must be a floating point value less than or equal to 1.0.');
  writeLn('       If this option is not used, all of the most-probable states will be written to the FASTA file.');
...
procedure TD_MegaMain.GenerateTopHapAlignment;
var
  fname: String = '';
  progress: TRuntimeProgress = nil;
  aft: TAlleleFreqsThread = nil;
  options: TSeqExportOptions = nil;
  edt: TExportSeqDataThread = nil;
  tempOutList: TStringList = nil;

  procedure UpdateAnalysisSummary;
  begin
   InitializeAnalysisSummary;
   FAnalysisSummary.AnalysisOptions.Add('Analysis=Generate TopHap alignment');
   FAnalysisSummary.AnalysisOptions.Add(Format('Min Allele Freq=%.1f%%', [FMinAlleleFreq]));
   FAnalysisSummary.AnalysisOptions.Add(Format('Max Allele Freq=%.1f%%', [FMaxAlleleFreq]));
   FAnalysisSummary.AnalysisOptions.Add(Format('First Seq is Ref=%s', [BoolToStr(FFirstSeqIsRefSeq, True)]));
   FAnalysisSummary.AnalysisOptions.Add(Format('Use Ref Seq in Calc=%s', [BoolToStr(FUseRefSeqInFreqCalculation, True)]));
   FAnalysisSummary.NumTaxa := D_InputSeqData.NoOfTaxa;
   FAnalysisSummary.NumSites := D_InputSeqData.NoOfSites;
   FAnalysisSummary.DataType := D_MegaMain.FDataType;
   FAnalysisSummary.DataFileName := fname;
   FAnalysisSummary.SettingsFileName := 'not applicable';
   FAnalysisSummary.CommandLineString := GetCommandLineStr;
  end;

  procedure CheckUserSettings;
  begin
    if not InRange(FMinAlleleFreq, 0, 100) then
      error_nv(Format('--min-allele-freq must be a value between 0.0 and 100.0 but got %.1f', [FMinAlleleFreq]));
    if not InRange(FMaxAlleleFreq, 0, 100) then
      error_nv(Format('--max-allele-freq must be a value between 0.0 and 100.0 but got %.1f', [FMaxAlleleFreq]));
    if CompareValue(FMinAlleleFreq, FMaxAlleleFreq, FP_CUTOFF) >= 0 then
      error_nv(Format('--min-allele-freq must be less than --max-allele-freq but got %.1f and %.1f', [FMinAlleleFreq, FMaxAlleleFreq]));
    if not FileExists(fname) then
      error_nv('aligment file not found: ' + fname);
    if FUseRefSeqInFreqCalculation and (not FFirstSeqIsRefSeq) then
      error_nv('if using the reference sequence as source of major alleles, then you must use the --first-seq-is-ref-seq option');
    if D_MegaMain.FDataType = snNoToken then
      error_nv('generating a TopHap alignment requires that a data type be specified (e.g. --nucleotide, --protein, --coding)');
  end;

  function SetupExportOptions: TSeqExportOptions;
  begin
   Result := TSeqExportOptions.Create;
   Result.IsFastaFormat := True;
   Result.isNucData := (not D_InputSeqData.IsAmino);
   Result.IsMEGAFormat := False;
...
begin
  try
    try
      fname := GetDataFileName;
      ConstructDummyProcessPack;
      FProcessPack.TextualSettingsList.Add(Format('%s=%s', ['Analysis', 'Generate TopHap alignment']));
      CheckUserSettings;
      if not DoOpenDataFile(fname, False, nil) then
        error_nv('failed to open the alignment data file: ' + fname);
      UpdateAnalysisSummary;
      progress := TRuntimeProgress.Create(nil);
      aft := TAlleleFreqsThread.Create(progress, FMinAlleleFreq/100, FMaxAlleleFreq/100, FFirstSeqIsRefSeq, FUseRefSeqInFreqCalculation);
      aft.FreeOnTerminate := False;
      aft.Start;
      aft.WaitFor;
      if aft.IsSuccess then
      begin
        aft.Report.SaveToFile(NextAvailableFilenameNV('_report.txt'));
        D_InputSeqData.SelectFirstInEachGroup;
        D_InputSeqData.LabelSitesUsedForMinorAlleleIdentityComparisons('x');
        D_InputSeqData.UseUnlabeledSites := False;
        options := SetupExportOptions;
        tempOutList := TStringList.Create;
        edt := TExportSeqDataThread.Create(D_InputSeqData, options, NextAvailableFilenameNV('_alignment.fas'), tempOutList);
        edt.FreeOnTerminate := False;
        edt.Start;
        edt.WaitFor;
        if not edt.IsSuccess then
          error_nv('failed to export TopHap alignment: ' + edt.MsgLog.Text);
        FAnalysisSummary.WriteToFile(NextAvailableFilenameNV('_summary.txt'));
        ExitCode := 0;
      end
      else
        error_nv('failed to count allele frequencies: ' + aft.LogText);
    except
      on E:Exception do
        error_nv('failed to generate TopHap alignment: ' + E.Message);
    end;
  finally
    if Assigned(progress) then
      progress.Free;
    if Assigned(aft) then
      aft.Free;
    if Assigned(edt) then
      edt.Free;
    if Assigned(tempOutList) then
      tempOutList.Free;
  end;
end;
...
begin
  FMinAlleleFreq := 1.0;
  FMaxAlleleFreq := 30.0;
  FFirstSeqIsRefSeq := False;
  FUseRefSeqInFreqCalculation := False;
  ...
  {$ENDIF}
  {$IFDEF DEBUG}
  DebugCriticalSection := TCriticalSection.Create;
  {$ENDIF}
end;
