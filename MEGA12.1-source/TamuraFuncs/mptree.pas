...
type
  ...
  TMPTree = class
    ...
    RandomArray: TArrayOfInteger;
  private
    FKeepMaxNoOfTrees: Boolean;
    FNoOfTreesSearched: Integer;
    ...
    procedure SetKeepMaxNoOfTrees(AValue: Boolean);
  ...
  public
    ...
    property KeepMaxNoOfTrees: Boolean read FKeepMaxNoOfTrees write SetKeepMaxNoOfTrees;
    property NoOfTreesSearched: Integer read FNoOfTreesSearched;
    ...
    procedure GetTreeData(var tree: TTreeData; const index: integer);
    procedure GetTreeList(var treelist: TTreeList; const forceEqualLengthTrees: Boolean = True);

    function ComputeApproxBLens(var tree: TTreeData):double;
    function ComputeConsistency(var tree: TTreeData):double;
    function LengthHistogram(tree: TTreeData; var Histogram: array of integer): integer;
  ...
  private
    ...
    function GetNoOfTreesSearched: Integer;
    procedure SetMaxNoOfTrees(n :integer);
    procedure SetNoOfInitTrees(n :integer);
  ...
  protected
    FCanComputeSiteCoverage: Boolean;
  ...
  public
    property MyExceptionName: String read FExceptionName;
    property MyExceptionMessage: String read FExceptionMessage;
    property NoOfTrees: integer read GetNoOfTrees;
    property NoOfTreesSearched: Integer read GetNoOfTreesSearched;
  ...
  protected
    ...
    procedure ComputeBclStdDev(var aTree: TTreeData);
    procedure DoOnProgress; override;
...
uses
  ...
  MGlobalSettings, MAnalysisInfo, msitecoverage;
...
begin
  inherited Create;
  FKeepMaxNoOfTrees := False;
  FNoOfTreesSearched := 0;
...
procedure TMPTree.SetKeepMaxNoOfTrees(AValue: Boolean);
begin
  if FKeepMaxNoOfTrees = AValue then Exit;
  FKeepMaxNoOfTrees := AValue;
end;
...
function TMPTree.CountTreeLength:integer;
  ...
  begin
    ...
    d1 := p.des1.InfoSite3;
    d2 := p.des2.InfoSite3;
    p.TreeLen3 := p.des1.TreeLen3 +p.des2.TreeLen3 +ComputePLongMPSteps(q, d1, d2, FNoOfInfoSites, FreqTable);
  end;
...
function TMPTree.LengthHistogram(tree: TTreeData; var Histogram: array of integer): integer;

  function CountStepUp(p : TMPTreeNode; index: integer): integer;
  var
    c,c1,c2: Longint;
    n1,n2: integer;
  begin
    ...
    n2 := 0;
    if not p.des1.OTU then
      n1 := CountStepUp(p.des1, index);
    if not p.des2.OTU then
      n2 := CountStepUp(p.des2, index);

    result := n1+n2;

    c  := p.InfoSite3[index];
    c1 := p.des1.InfoSite3[index];
    c2 := p.des2.InfoSite3[index];

    c := c1 and c2;
    if c = 0 then
    begin
      inc(result);
      p.InfoSite3[index] := c1 or c2;
    end
    else
      p.InfoSite3[index] := c;
  end;
...
begin
  ...
  for i := 0 to h do
    Histogram[i] := 0;
  for i := 0 to NoOfInfoSites-1 do
  begin
    n := CountStepUp(NodeArray[2*NoOfSeqs-2], i);
    if n > h then
      inc(Histogram[h])
    else
      inc(Histogram[n]);
  ...
  Histogram[0] := NoOfTotalSites -NoOfInfoSites -NoOfSingletons;
  Histogram[1] := Histogram[1] +NoOfSingletons;
  result := m;
end;
...
var
  ...
  begin
    ...
    if (ParsimTrees.NoOfTrees > 0) and (Round(tree.Value) < TreeLength) then
      ParsimTrees.Reset;
    if (ParsimTrees.NoOfTrees < ParsimTrees.MaxNoOfTrees) and ((FKeepMaxNoOfTrees) or(not (Round(tree.Value) > TreeLength))) then
...
var
  ...
  var
    ...
    aIndex: Integer = -1;
  begin
    inc(FNoOfTreesSearched);
    CurTreeLength := NodeArray[2*NoOfSeqs-2].TreeLen3 + NoOfSingletons;

    if (not FKeepMaxNoOfTrees) and (CurTreeLength > TreeLength) then
      exit;

    if (CurTreeLength = TreeLength) and (ParsimTrees.NoOfTrees = ParsimTrees.MaxNoOfTrees) then
      exit;
    ...
    begin
      if not FKeepMaxNoOfTrees then
        ParsimTrees.Reset;
    end;
    if (CurTreeLength > TreeLength) and (ParsimTrees.NoOfTrees = ParsimTrees.MaxNoOfTrees) then
    begin
      aIndex := ParsimTrees.FindTreeWithValueGreaterThan(CurTreeLength);
      if aIndex >= 0 then
        ParsimTrees.RemoveTree(aIndex);
    end;
    ParsimTrees.AddTreeData(tree, true);
    tree.Free;
  end;
...
function TMPTree.ComputeApproxBLens(var tree: TTreeData):double;
var
...
begin
  ...
  result := 0;
  for i := 0 to 2*NoOfSeqs-2 do
    result := result + tree.BLen[i];
end;


function BitCount(Value: LongInt): Integer;
begin
...
function TMPTree.ComputeConsistency(var tree: TTreeData):double;

  procedure ComputeConsistencyNode(n: TMPTreeNode; var flag: array of boolean);
  var
    flag1,flag2: array of boolean;
    i,k,c: integer;
    state0,state1: LongInt;
    aInfoSites: Integer = 0;
  begin
    state0 := 0;
    state1 := 0;
    if n.index < NoOfSeqs then
    begin
      flag[n.index] := true;
      exit;
    end;

    setlength(flag1, NoOfSeqs);
    setlength(flag2, NoOfSeqs);

    ComputeConsistencyNode(n.des1, flag1);
    ComputeConsistencyNode(n.des2, flag2);

    for i := 0 to NoOfSeqs-1 do
      flag[i] := flag1[i] or flag2[i];
    ...
    begin
      ...
      for i := 0 to NoOfSeqs-1 do
        if flag[i] then
          state1 := state1 or NodeArray[i].InfoSite3[k]
        else
          state0 := state0 or NodeArray[i].InfoSite3[k];
      if BitCount(state0 and state1) < 2 then
        inc(c);
    end;
    aInfoSites := NoOfInfoSites;
    if (aInfoSites > 0) and (c > 0) then
      tree.StatsArray[n.index-NoOfSeqs] := c/aInfoSites
    else
      tree.StatsArray[n.index - NoOfSeqs] := 0;
    result := result + tree.StatsArray[n.index-NoOfSeqs];
...
begin
  ...
  i := 0;
  while NodeArray[i].anc <> nil do
    i := NodeArray[i].anc.index;
  root := i;
  ...
  for i := 0 to NoOfSeqs-1 do
    flag[i] := false;
  ComputeConsistencyNode(NodeArray[root].des1, flag);

  for i := 0 to NoOfSeqs-1 do
    flag[i] := false;
  ComputeConsistencyNode(NodeArray[root].des2, flag);

  if NoOfSeqs = 3 then
    raise Exception.Create('division by zero in TMPTree.ComputeConsistency');
  result := result/(NoOfSeqs-3);
end;

procedure TMPTree.GetTreeData(var tree: TTreeData; const index: integer);
var
...
procedure TMPTree.GetTreeList(var treelist: TTreeList; const forceEqualLengthTrees: Boolean = True);
begin
  ParsimTrees.ExtractTreeList(treelist);
  if forceEqualLengthTrees and (not treeList.ValidateParsimonyTrees) then
    raise Exception.Create('TTreeList contains parsimony trees with different tree lengths');
end;

constructor TMPTreeSearchThread.Create(TreeList: TTreeList; MappedSites: TList; NoOfSites:integer; NoOfBits: integer);
begin
  inherited Create;
  FCanComputeSiteCoverage := True;
  FExceptionName := 'none';
...
function TMPTreeSearchThread.GetNoOfTreesSearched: Integer;
begin
  if Assigned(MPTree) then
    Result := MPTree.NoOfTreesSearched
  else
    Result := 0;
end;
...
begin
  try
    inherited;
    {$IFDEF VISUAL_BUILD}
    if Assigned(Info) and FCanComputeSiteCoverage then
    begin
      analysisInfo := TAnalysisInfo(Info);
      ComputeSiteCoverage(analysisInfo);
    end;
    {$ENDIF}
  Except
...
procedure TMPTreeSearchThread.GetTreeData(var tree: TTreeData; const index: integer);
begin
...
procedure TBootstrapMPTreeSearchThread.ComputeBclStdDev(var aTree: TTreeData);
var
  i: Integer = -1;
  aStdDev: Double = 0;
  numReps: Integer = 0;
begin
  aTree.IsStatsStdDev := True;
  for i := aTree.NoOfOTUs to 2*(aTree.NoOfOTUs - 1) - 1 do
  begin
    numReps := Round(BootstrapTrees.TotalFrequency);
    aStdDev := ExpectedBclPrecision(trunc(aTree.Stats[i]/numReps*100), numReps);
    aTree.StatsStdDev[i] := aStdDev
  end;
end;
...
function TBootstrapMPTreeSearchThread.AnalysisDescription: String;
begin
  Result := 'MP Search with Bootstrap';
end;
