...
//    function PatternBias: Extended; virtual; abstract;
  ...
  public
    ...
    function ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean; virtual;
...
function TGammaRateVariationModel.ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean;
var
  i: Integer;
  version: Integer = -1;
begin
  ...
  try
    ...
    BlockReadExtendedType(data, deltaL, SessionUsesExtended);
    ...
    setlength(FRate, FNoOfRates);
    for i := 0 to FNoOfRates-1 do
      BlockReadExtendedType(data, FRate[i], SessionUsesExtended);

    BlockRead(data, FNoOfStates, sizeof(integer));
    for i := 0 to FNoOfStates-1 do
      BlockReadExtendedType(data, FFreq[i], SessionUsesExtended);

    BlockRead(data, FUseInvar, sizeof(boolean));
    BlockReadExtendedType(data, FInvar, SessionUsesExtended);

    BlockRead(data, FFixedGamma, sizeof(boolean));
    BlockReadExtendedType(data, FGamma, SessionUsesExtended);
...
var
  tempCancelled: Boolean = False;
begin
  if Assigned(SubtaskCheckCancel) then
  begin
    if not FHideSubtaskProgress then
      tempCancelled := SubtaskCheckCancel(aProgress, aStatus);
    if aProgress < 100 then  { bug fix for small data where the calculated thread has already terminated}
      FCancelled := tempCancelled;
  end;
  if FCancelled then
    Abort;
end;
