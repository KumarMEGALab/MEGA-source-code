...
//    property NoOfClocks: integer read FNoOfClocks;
    ...
    function ReadFromFile(var data: File; SessionVersion: Integer; SessionUsesExtended: Boolean; OutGroupMembers: TBoolArray):boolean;
    procedure WriteToFile(var data: File);
    function StateToStringList(Comment: AnsiString = ''): TStringList;
    function CoordsName(nodeIndex: Integer): String;
...
begin
  LockThread.Free;
  for i := 1 to Length(ChildThread) - 1 do
...
//      L1 := tmpMPTree.ComputeApproxBLens(t1);
    ...
    if Assigned(MPTreeSearchThread) then
      MPTreeSearchThread.Free;
...
function TMLTreeAnalyzer.ReadFromFile(var data: File; SessionVersion: Integer; SessionUsesExtended: Boolean; OutGroupMembers: TBoolArray):boolean;
type
  TArrayOfChar = array [0..MaxPointerElts-1] of AnsiChar;
  PArrayOfChar = ^TArrayOfChar;
var
...
begin
  ...
  BlockReadExtendedType(data, AExt, SessionUsesExtended);
  ...
  begin
    ...
    if (SessionVersion < 701) or (n > 0) then
      for i := 0 to n - 1 do
      begin
        BlockReadExtendedType(data, TempRates[i], SessionUsesExtended);
        BlockReadExtendedType(data, TempVarH[i], SessionUsesExtended);
        BlockReadExtendedType(data, TempVarB[i], SessionUsesExtended);
        BlockReadExtendedType(data, TempVarR[i], SessionUsesExtended);

        // From ver. 3
        if FMLTreeAnalyzerVersion >= 3 then
        begin
          BlockReadExtendedType(data, TempHeight[i], SessionUsesExtended);
          BlockReadExtendedType(data, TempMaxH[i], SessionUsesExtended);
          BlockReadExtendedType(data, TempMinH[i], SessionUsesExtended);
        end;
      end;
  end;
...
// Model
  ...
  if not FModel.ReadFromFile(data, SessionUsesExtended) then
...
//    MLTree.Optimize(nil, ''); // takes as long as the original analysis but with this commented out, log likelihood differs
  ...
  begin
    ...
    begin
      ...
      for i := 0 to NoOfSeqs-1 do
        BlockReadExtendedType(data, SamplingTime[i], SessionUsesExtended);
    end;
  end;
...
function TMLTreeAnalyzer.CoordsName(nodeIndex: Integer): String;
begin
  if Assigned(MLTree.Node[nodeIndex].des1) and Assigned(MLTree.Node[nodeIndex].des2) then
    Result := Format('parent_of_%d_and_%d', [MLTree.Node[nodeIndex].des1.index + 1, MLTree.Node[nodeIndex].des2.index + 1]);
end;
