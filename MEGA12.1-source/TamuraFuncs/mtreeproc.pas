...
procedure SortBranchByOrderSimple(p : TSimpleTreeNode);
procedure SetClusterSizeSimple(root : TSimpleTreeNode);
...
begin
    ...
    if (not Assigned(root^.des1)) or (not Assigned(root^.des2)) then Exit;
    with root^ do begin
        SetMaxLen2(des1);
        SetMaxLen2(des2);
...
begin
    if not Assigned(root) then Exit;
    if not Assigned(newposition) then Exit;
    ...
    b0.meanBcl := max(root.des1.branch.meanBcl, root.des2.branch.meanBcl);
    root.des1.branch.meanBcl := b0.meanBcl;
    root.des2.branch.meanBcl := b0.meanBcl;
...
begin
  if not Assigned(p) then Exit;
  if p^.OTU then Exit;
  if (not Assigned(p^.des1)) or (not Assigned(p^.des2)) then Exit;
  if p^.des1^.minOTU > p^.des2^.minOTU then
    SwapNode(p^.des1,p^.des2);
...
begin
  if (not Assigned(Root)) or (not Assigned(NewPosition)) then Exit;
  if newposition = root then Exit;
...
begin
  if not Assigned(p) then Exit;
  if p.IsOtu then Exit;
  if p.des1.minOTU > p.des2.minOTU then
      SwapNodeSimple(p.des1,p.des2);
  SortBranchByOrderSimple(p.des1);
  SortBranchByOrderSimple(p.des2);
end;
...
begin
  ...
  with root do
    if not IsOTU then
    begin
      SetClusterSizeSimple(des1);
      SetClusterSizeSimple(des2);
      size := des1.size + des2.size;
      if des1.minOTU < des2.minOTU then
        minOTU := des1.minOTU
      else
...
begin
  tipNodes.Clear;
  if not Assigned(ancestor) then Exit;
