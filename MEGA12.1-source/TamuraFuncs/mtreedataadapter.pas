...
type
  ...
  TSimpleTreeDataAdapter = class(TObject)
    ...
    public
      ...
      function GetTipToRootDistance: Double;
      {$IFDEF DEBUG}
...
function GetRootNodeHeight(aTree: TTreeData; otuNames: TStringList): Double; overload; { this function assumes that the tree is ultrametric. A check must be made before calling this function}
function GetRootNodeHeight(aNewick: String; otuNames: TStringList): Double; overload; { this function also assumes that the tree is ultrametric}
function TreeIsUltrametric(aTree: TTreeData; otuNames: TStringList; isRooted: Boolean): Boolean; overload;
function TreeIsUltrametric(aNewick: String; otuNames: TStringList): Boolean; overload;
...
function GetRootNodeHeight(aTree: TTreeData; otuNames: TStringList): Double;
var
  adapter: TSimpleTreeDataAdapter = nil;
begin
  try
    ...
    if Assigned(otuNames) then
      adapter.SetTreeData(aTree, True, otuNames)
    else
      adapter.SetTreeData(aTree, True);
    Result := adapter.GetTipToRootDistance;
  finally
    if Assigned(adapter) then
      adapter.Free;
  end;
...
function GetRootNodeHeight(aNewick: String; otuNames: TStringList): Double;
var
  aList: TTreeList = nil;
begin
  try
    aList := TTreeList.Create;
    if not aList.ImportFromNewick(aNewick, otuNames, False) then
      raise Exception.Create('failed to import user tree');
    if Assigned(otuNames) then
      Result := GetRootNodeHeight(aList[0], otuNames)
    else
      Result := GetRootNodeHeight(aList[0], nil);
  finally
...
function TreeIsUltrametric(aTree: TTreeData; otuNames: TStringList; isRooted: Boolean): Boolean;
var
  adapter: TSimpleTreeDataAdapter = nil;
begin
  if not aTree.isBLen then
    Exit(False);

  try
    adapter := TSimpleTreeDataAdapter.Create;
    if Assigned(otuNames) then
      adapter.SetTreeData(aTree, isRooted, otuNames)
    else
      adapter.SetTreeData(aTree, isRooted);
...
function TreeIsUltrametric(aNewick: String; otuNames: TStringList): Boolean;
var
  aList: TTreeList = nil;
begin
  try
    ...
    if not aList.ImportFromNewick(aNewick, otuNames, False) then
      raise Exception.Create('failed to import user tree');
    if Assigned(otuNames) then
      Result := TreeIsUltrametric(aList[0], otuNames, aList.isRooted)
    else
      Result := TreeIsUltrametric(aList[0], nil, aList.isRooted);
  finally
    if Assigned(aList) then
      aList.Free;
  end;
...
function TSimpleTreeDataAdapter.GetTipToRootDistance: Double;
var
  n: TSimpleTreeNode = nil;
begin
  n := FNodes[0];
