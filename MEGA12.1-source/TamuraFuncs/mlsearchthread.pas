...
begin
  ...
  try
    ...
    MLTree := MLTreeAnalyzer.MLTree;
    // TODO: Refactor subtask checking\cancelling on Unix until a way can be found to make it thread-safe
    {$IFDEF MSWINDOWS}
    MLTree.HideSubtaskProgress := False;
    {$ELSE}
    MLTree.HideSubtaskProgress := True;
    {$ENDIF}
    if IsBootstrapReplicate then
