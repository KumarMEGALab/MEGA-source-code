...
uses
  Classes, MegaConsts, MegaUtils, MLTree, MatrixConsts, MatrixFuncs;
...
type
  ...
  public
    ...
    function ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean; override;
...
var
  ...
  i,j: integer;
begin
  {$IFDEF DEBUG}WriteToDevLog('begin TGTRModel.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TGTRModel.SetParamsFromSeqs started');{$ENDIF}

  TN93Model := TTN93Model.Create(0, UseInvar, NoOfRates);
  TN93Model.SiteCoverage  := SiteCoverage;
  if Assigned(FBootTable) then
  begin
    TN93Model.BootTable := FBootTable;
    TN93Model.OrigNoOfSites := OrigNoOfSites;
  end;
  TN93Model.SetParamsFromSeqs(seqs);

  inherited Assign(TN93Model);
  ...
  FInitialized := true;
  {$IFDEF DEBUG}WriteToDevLog('end TGTRModel.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TGTRModel.SetParamsFromSeqs ended');{$ENDIF}
end;
...
procedure TGTRModel.OptimizeParameters(tree: TMLTree; aCheckCancel: TCheckCancelFunc);
var
  L0,dL0,dL1,dL2,dL,ddL,r0,dr: extended;
  p,d,r1: array [1..6] of extended;
  iter,i,j: integer;
begin
  iter := 0;
...
function TGTRModel.ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean;
var
  i,version: integer;
begin
  version := -1;
  result := false;
  try
    if not inherited ReadFromFile(data, SessionUsesExtended) then
      exit;

    BlockRead(data, version, sizeof(integer));

    for i := 1 to 6 do
      BlockReadExtendedType(data, r[i], SessionUsesExtended);
...
var
  b: array of extended;
  diff: PArrayOfLongint3D;
  m: array of PMatrixOfExtended;
  p1,p2,q,r,s,s2: extended;
  d1,d2,dL,ddL,dL0,dL1,dL2,L,L0,k0: extended;
  i,j,k,n,it,n0: integer;
begin
  Assert(Assigned(seqs) and (seqs.Count > 0));
  {$IFDEF DEBUG}WriteToDevLog('begin TTN93Model.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TTN93Model.SetParamsFromSeqs started');{$ENDIF}
  ...
  CreateArrayOfLongint3D(diff, seqs.Count, 4);

  GetParamsFromSeqs(seqs, diff, s,s2, n,n0);
  InitGamma(s,s2,n,n0);
  ...
  DestroyArrayOfLongint3D(diff, seqs.Count, 4);
  ...
  FInitialized := true;
  {$IFDEF DEBUG}WriteToDevLog('end TTN93Model.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TTN93Model.SetParamsFromSeqs ended');{$ENDIF}
end;
...
function TTN93Model.ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean;
var
  version: integer;
begin
  version := -1;
  ...
  try
    ...
    BlockReadExtendedType(data, k1, SessionUsesExtended);
    BlockReadExtendedType(data, k2, SessionUsesExtended);
    pA := Freq[0];
    pT := Freq[1];
    pC := Freq[2];
    pG := Freq[3];
...
var
  ...
  m: array of PMatrixOfExtended;
  p1,p2,q,r,s,s2: extended;
  dk,dL,ddL,dL0,dL1,dL2,L,k0,k1: extended;
  i,j0,j1,n,n0,it: integer;
begin
  {$IFDEF DEBUG}WriteToDevLog('THKYModel.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' THKYModel.SetParamsFromSeqs started');{$ENDIF}
  setlength(b,seqs.Count);
  setlength(m,seqs.Count);
  for i := 0 to seqs.Count-1 do
    CreateMatrix(m[i], 4);
  ...
  FInitialized := true;
  {$IFDEF DEBUG}SaveMLlog(' THKYModel.SetParamsFromSeqs ended');{$ENDIF}
  {$IFDEF DEBUG}WriteToDevLog('end THKYModel.SetParamsFromSeqs');{$ENDIF}
end;
...
function THKYModel.ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean;
var
  version: integer;
begin
  version := -1;
  ...
  try
    ...
    BlockReadExtendedType(data, k, SessionUsesExtended);
    pA := Freq[0];
    pT := Freq[1];
    pC := Freq[2];
    pG := Freq[3];
...
var
  ...
  b: array of extended;
  m: array of PMatrixOfExtended;
  p,q,r,s,s2,pCG: extended;
  dk,dL,ddL,dL0,dL1,dL2,L,k0: extended;
  i,j0,j1,n,n0,it: integer;
begin
  {$IFDEF DEBUG}WriteToDevLog('begin TT3Model.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TT3Model.SetParamsFromSeqs started');{$ENDIF}
  setlength(b,seqs.Count);
  setlength(m,seqs.Count);
  for i := 0 to seqs.Count-1 do
    CreateMatrix(m[i], 4);
  ...
  begin
    ...
    p := m[i][0,3]+m[i][3,0]+m[i][1,2]+m[i][2,1];
    q  := m[i][0,1]+m[i][1,0]+m[i][0,2]+m[i][2,0]+m[i][1,3]+m[i][3,1]+m[i][2,3]+m[i][3,2];
    r  := p+q +m[i][0,0] +m[i][1,1] +m[i][2,2] +m[i][3,3];
    if r = 0 then
      continue;
    p := p/r;
    q  := q/r;
    if (pCG > 0) and (pCG < 1) then
      if ((p/2/pCG/(1-pCG) +q) < 1) and (2*q < 1) then
      begin
        r := -ln(1 -p/2/pCG/(1-pCG) -q)/2 +ln(1 -2*q)/4;
        if r > 0 then
          k := k +r;
        r := -ln(1 -2*q)/4;
        s  := s +r;
  ...
  FInitialized := true;
  {$IFDEF DEBUG}SaveMLlog(' TT3Model.SetParamsFromSeqs ended');{$ENDIF}
  {$IFDEF DEBUG}WriteToDevLog('end TT3Model.SetParamsFromSeqs');{$ENDIF}
end;
...
var
  ...
  i,j0,j1,n,n0,it: integer;
begin
  {$IFDEF DEBUG}WriteToDevLog('begin TK2Model.SeqParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TK2Model.SetParamsFromSeqs started');{$ENDIF}
  setlength(b,seqs.Count);
  setlength(m,seqs.Count);
  for i := 0 to seqs.Count-1 do
    CreateMatrix(m[i], 4);
  ...
  begin
    ...
    p := p/r;
    q  := q/r;
    if ((2*p +q) < 1) and (2*q < 1) then
    begin
      r := -ln(1 -2*p -q)/2 +ln(1 -2*q)/4;
      if r > 0 then
        k := k +r;
      r := -ln(1 -2*q)/4;
      s  := s +r;
  ...
  FInitialized := true;
  {$IFDEF DEBUG}SaveMLlog(' TK2Model.SetParamsFromSeqs ended');{$ENDIF}
  {$IFDEF DEBUG}WriteToDevLog('end TK2Model.SeqParamsFromSeqs');{$ENDIF}
end;
...
var
  ...
  i,n,n0: integer;
begin
  {$IFDEF DEBUG}WriteToDevLog('begin TJCModel.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TJCModel.SetParamsFromSeqs started');{$ENDIF}
//  inherited;
  setlength(b,seqs.Count);
  setlength(m,seqs.Count);
  for i := 0 to seqs.Count-1 do
  ...
  FInitialized := true;
  {$IFDEF DEBUG}SaveMLlog(' TJCModel.SetParamsFromSeqs ended');{$ENDIF}
  {$IFDEF DEBUG}WriteToDevLog('end TJCModel.SetParamsFromSeqs');{$ENDIF}
end;
...
begin
  ...
  repeat
    ...
    inc(iter);
    if Assigned(aCheckCancel) then
      aCheckCancel(Round(iter/MaxIter*100), 'Optimizing JC parameters');
  until (abs(tree.LogLikelihood-L0) < deltaL) or (iter = MaxIter);
  FInitialized := true;
  if Assigned(aCheckCancel) then
    aCheckCancel(100, 'Optimizing JC parameters');
end;
...
var
  ...
  s: extended;
begin
  {$IFDEF DEBUG}WriteToDevLog('begin TProteinMatrixModel.SetParamsFromSeqs');{$ENDIF}
  {$IFDEF DEBUG}SaveMLlog(' TProteinMatrixModel.SetParamsFromSeqs started');{$ENDIF}
  inherited;
  ...
  begin
    ...
    for i := 0 to 19 do
      FFreq[i] := DefaultFreq[i];

  ComputeEigenVector;

  FInitialized := true;
  {$IFDEF DEBUG}SaveMLlog(' TProteinMatrixModel.SetParamsFromSeqs ended');{$ENDIF}
  {$IFDEF DEBUG}WriteToDevLog('end TProteinMatrixModel.SeqParamsFromSeqs');{$ENDIF}
end;
...
function TProteinMatrixModel.GetDescription: String;
begin
...
function TProteinMatrixModel.ReadFromFile(var data: File; SessionUsesExtended: Boolean):boolean;
var
  i,version: integer;
begin
  version := -1;
  ...
  try
    ...
    for i := 0 to 19 do
      BlockReadExtendedType(data, DefaultFreq[i], SessionUsesExtended);
...
begin
  ...
  ModelInfo.FullName  := 'Adachi-Hasegawa (1996) protein-encoded mitochondrial DNA model'+ModelInfo.FullName;
end;
...
begin
  ...
  ModelInfo.FullName  := 'Adachi et al . (2000) protein-encoded chloroplast DNA model'+ModelInfo.FullName;
end;
