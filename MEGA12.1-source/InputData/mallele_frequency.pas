...
type
  ...
  TAlleleFrequency = class(TObject)
    private
      ...
      FIncludeRefSeq: Boolean;
      FFirstSeqIsRef: Boolean;
    ...
    public
      ...
      constructor Create(isAmino: Boolean; numSeqs, numSites: Integer; includeRefSeq: Boolean; firstSeqIsRef: Boolean);
...
begin
  ...
  for i := Low(FAlleleFreqs) to High(FAlleleFreqs) - 1 do
  begin
...
constructor TAlleleFrequency.Create(isAmino: Boolean; numSeqs, numSites: Integer; includeRefSeq: Boolean; firstSeqIsRef: Boolean);
var
  i: Integer;
begin
  FIsComputed := False;
  ...
  FNumSeqs := numSeqs;
  FNumSites := numSites;
  FIncludeRefSeq := includeRefSeq;
  FFirstSeqIsRef := firstSeqIsRef;
  if FIsAminoAcid then
...
begin
  if not FIsComputed then
    ComputeValues;
  if FFirstSeqIsRef then
  begin
    if FIncludeRefSeq then
      Result := FAlleleFreqs[ord(allele)]/FNumSeqs
    else
      Result := FAlleleFreqs[ord(allele)]/(FNumSeqs - 1);
  end
  else
    Result := FAlleleFreqs[ord(allele)]/FNumSeqs;
end;
...
begin
  ...
  begin
    if(MinorAllele <> gdResiX) then
      Result := FAlleleFreqs[ord(MinorAllele)]/n
    else
      Result := 0.0;
  end
  else
...
begin
  ...
  begin
    if(MajorAllele <> gdResiX) then
      Result := FAlleleFreqs[ord(MajorAllele)]/n
    else
      Result := 0.0;
  end
  else
...
begin
  if IsAminoAcid then
    inc(FAlleleFreqs[ord(AminoToDistMap(allele))])
  else
