...
type
  ...
  private
    ...
    procedure DoSaveSession(data: PtrInt);
...
uses
  ...
  MegaPrivateFiles, MutationExplorer, mmega_font_dialog,
...
begin
  ...
  else if (FMouseGridCoords.X = 1) and ActionDispGroupNames.Checked then
      EditGroup.PopUp(coords.X, coords.Y)
  else
  begin
...
begin
  ...
  try
    ...
    begin
      gpName := Trim(D_InputSeqData.OtuInfos.Otu[i].GpName);
      if gpName = EmptyStr then
        continue;
      if EditGroup.Items.Find(gpName) = nil then
      begin
...
begin
  ...
  {$ENDIF}
  ActionAddDomainsForGroupIdentityComparisonSites.Visible := IsDeveloper;
  ActionDisableIndependentSites.Visible := IsDeveloper;
  ActionLabelSitesUsedForGroupIdentityComparison.Visible := IsDeveloper;
  ActionGroupSeqsByIdentity.Visible := IsDeveloper;
end;
...
procedure TV_SeqDataExplorer.MenuItem126Click(Sender: TObject);
begin
  OpenUrl(MegaGptUrl);
end;
...
procedure TV_SeqDataExplorer.DoSaveSession(data: PtrInt);
var
...
begin
  Application.QueueAsyncCall(DoSaveSession, placeholder);
end;
...
begin
  ...
  begin
    VS_SeqDataExplorer.CurFont.Assign(aFont);
...
begin
  ...
  begin
    ...
    promptNewGroupNameStr := 'Group Name (for selected taxa)';

  aRow := VS_SeqDataExplorer.CurColorRowRange.X;
  if aRow > 0 then
    newGpStr := D_InputSeqData.GetGpName(aRow - 1);
