...
type
  ...
  private
    FMissingDataCounts: TIntArray;
    FCountMissingData: Boolean;
  ...
  public
    ...
    function HasAnyGeographicalInfo: Boolean;
   ...
   // function IsGoodAA(Base: Char): Boolean;
    ...
    function GetStopCodonsSitesConvertedToMissingData: TIntArray;
    function PrepareDataForDistAnalysis(AOptions: TDataSubsetOptions;
    ...
    function PrepareDataForParsimAnalysis(AOptions: TDataSubsetOptions;
      const ASeqList, MyUsedOtuInfos: TList; var TotalTaxa, TotalSites,
      InfoSites, ConstContribute: Integer;
      LabelsToInclude: TStringList;  // EmptyStr means all, 'space string' = non-label sites
      ARecodeScheme: AnsiString;
      SiteCoverage: Integer): TIntArray;
    function PrepareDataForMLAnalysis(MAI: TObject): TIntArray; overload;
    function PrepareDataForMLAnalysis(MAI: TObject; const UpdateMissingDataCounts: Boolean): TIntArray; overload;
    function PrepareDataForPartitionedMLAnalysis(AOptions: TDataSubsetOptions;
    ...
    procedure SelectFirstInEachGroup;
  public
  ...
  private
    FTreatStopCodonsAsMissingData: Boolean;
    FStopCodonsConvertedToMissingData: TLongintList;
    FCreatedWithoutGui: Boolean;
    ...
    //function GetDomainName(Index: LongInt): AnsiString;
    //function GetGeneName(Index: LongInt): AnsiString;
    //function GetGpName(Index: LongInt): AnsiString;
    function GetNoOfDomains: Integer;
    //function GetPopName(Index: LongInt): AnsiString;
    //function GetSearchResult(Index: LongInt): TSearchResult;
    //function GetSequence(Index: LongInt): PAnsiChar;
    //function GetSeqUsed(Index: LongInt): Boolean;
    //function GetSiteLabel(Index: LongInt): AnsiChar;
    //function GetSiteUsed(Index: LongInt): Boolean;
    //function GetSpName(Index: LongInt): AnsiString;
  ...
  Public
    function MostSparseSequenceInfo(var seqName: String; var seqIndex: Integer; var dataCoverage: Integer): Boolean;
    property TreatStopCodonsAsMissingData: Boolean read FTreatStopCodonsAsMissingData write FTreatStopCodonsAsMissingData;
    property MissingDataCounts: TIntArray read FMissingDataCounts;
...
begin
  inherited;
  FTreatStopCodonsAsMissingData := False;
  FStopCodonsConvertedToMissingData := TLongintList.Create;
  FCountMissingData := False;
  SetLength(FMissingDataCounts, 0);
  ...
  CurDomainInfo := nil;
  ARP := nil;
  if FCreatedWithoutGui then
    PleaseWait := TPleaseWait.CreateWithoutGui
  else
    PleaseWait := TPleaseWait.Create(nil);
  InitSearchResults;
end;

constructor TD_InputSeqData.CreateWithoutGui;
begin
  FCreatedWithoutGui := True;
  Create;
end;
...
begin
  try
   ...
   {$ENDIF}
   if Assigned(FStopCodonsConvertedToMissingData) then
     FStopCodonsConvertedToMissingData.Free;
   SetLength(FMissingDataCounts, 0);
...
begin
    {$IFDEF VISUAL_BUILD}
    if (FNoOfSites*FNoOfTaxa > SDE_MAX_TOTAL_SITES) and (not IsDeveloper) then
...
var
  ...
  SitesPerPercent, SitesDone: Int64;
begin
  try
    Result := False;
...
begin
  x := -1; y := -1; z := -1;
  case Codon[0] of  'U', 'T', 'u', 't': x:=0; 'C', 'c': x:=1; 'A', 'a': x:=2; 'G', 'g': x:=3; end;
  case Codon[1] of  'U', 'T', 'u', 't': y:=0; 'C', 'c': y:=1; 'A', 'a': y:=2; 'G', 'g': y:=3; end;
  case Codon[2] of  'U', 'T', 'u', 't': z:=0; 'C', 'c': z:=1; 'A', 'a': z:=2; 'G', 'g': z:=3; end;
...
function TD_InputSeqData.HasAnyGeographicalInfo: Boolean;
var
  i: Integer = -1;
begin
  if (not Assigned(FOtuInfos)) or (FOtuInfos.NoOfOtus = 0) then
    Exit(False);
  for i := 0 to FOtuInfos.NoOfOtus - 1 do
    if FOtuInfos[i].HasAnyGeographicalInfo then
      Exit(True);
  Result := False;
end;
...
begin
  ...
  begin
    if Index >= AACodonStarts.Count then
      Exit;
    Result := FDomainMarks[AACodonStarts[Index]]; // domain mark is from
  end
  else
  begin
    if Index >= FDomainMarks.NoOfSites then
      Exit;
      ...
      begin
       ...
       // check for too many ambiguous bases as well as stop codons and gap-only sites
        ...
        if CountAmbiguousInNucCol(CodonCol3rd, MyNTaxa) > MaxAmbiguousAllowed then
            Continue;
        if FTreatStopCodonsAsMissingData then
          ConvertCodonMapStopToMissingData(CodonMaps, MyNTaxa, FCodeTable)
        else if HasStopCodonInCodonMapCol(CodonMaps, MyNTaxa, FCodeTable) then // stop codon sites are removed
          Continue;
...
function TD_InputSeqData.PrepareDataForDistAnalysis(
  ...
  ARecodeScheme: AnsiString; SiteCoverage: Integer): TIntArray;
var
  ASeq: PAnsiChar;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FTreatStopCodonsAsMissingData then
          begin
            if ConvertStopToMissingData(ColConfig, MyNTaxa, i=(AACodonStarts.Count-1)) then
              FStopCodonsConvertedToMissingData.Add(i + 1);
          end
          else if HasStopAminoInAminoCol(ColConfig, MyNTaxa, i=(AACodonStarts.Count-1)) then // stop codon sites are removed
            Continue;
        ...
        begin
          if CountAmbiguousInCodonMapCol(ColConfig, MyNTaxa) > MaxAmbiguousAllowed then
              Continue;
          if FTreatStopCodonsAsMissingData then
            ConvertCodonMapStopToMissingData(ColConfig, MyNTaxa, FCodeTable)
          else if HasStopCodonInCodonMapCol(ColConfig, MyNTaxa, FCodeTable) then // stop codon sites are removed
            Continue;
          for j:=0 to MyNTaxa-1 do
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FTreatStopCodonsAsMissingData then
          begin
            if ConvertStopToMissingData(ColConfig, MyNTaxa, i=MaxSites-1) then
              FStopCodonsConvertedToMissingData.Add(i + 1);
          end
          else if HasStopAminoInAminoCol(ColConfig, MyNTaxa, i=MaxSites-1) then // stop codon sites are removed
            Continue;
          for j:=0 to MyNTaxa-1 do
          begin
            if dsoDistMap in AOptions then
...
function TD_InputSeqData.PrepareDataForParsimAnalysis(
  AOptions: TDataSubsetOptions; const ASeqList, MyUsedOtuInfos: TList;
...
begin
  ...
  try
    ...
    Result := PrepareDataForDistAnalysis(AOptions, TempSeqList, MyUsedOtuInfos, TotalTaxa, TotalSites, LabelsToInclude, ARecodeScheme, SiteCoverage);
...
function TD_InputSeqData.PrepareDataForPartitionedMLAnalysis(
...
begin
  ...
  try try
    ...
    TotalSites := WriteAtIndex;
    TotalTaxa  := MyNTaxa;
    SetLength(Result, Length(includedSites));
    for i := Low(IncludedSites) to High(IncludedSites) do
      Result[i] := IncludedSites[i];
  except
    On E: Exception do
    begin
...
function TD_InputSeqData.GetStopCodonsSitesConvertedToMissingData: TIntArray;
var
  i: Integer = -1;
begin
  SetLength(Result, FStopCodonsConvertedToMissingData.Count);
  if FStopCodonsConvertedToMissingData.Count > 0 then
    for i := 0 to FStopCodonsConvertedToMissingData.Count - 1 do
      Result[i] := FStopCodonsConvertedToMissingData[i];
end;

function TD_InputSeqData.PrepareDataForMLAnalysis(MAI: TObject): TIntArray;
var
...
begin
  ...
  try
    TempSeqList := TList.Create;
    if aInfo.MyUsrOperation = dtdoEpML then
      FTreatStopCodonsAsMissingData := True;
    // now we use the DistAnalysis system to get the data
    Result := PrepareDataForDistAnalysis(AOptions, TempSeqList, MyUsedOtuInfos, aInfo.MyNoOfSeqs, aInfo.MyNoOfSites, LabelsToInclude, ARecodeScheme, SiteCoverage);
    ...
    if FCountMissingData then
    begin
      if not (Length(FMissingDataCounts) = AInfo.MyNoOfSeqs) then
        SetLength(FMissingDataCounts, aInfo.MyNoOfSeqs);
      for i := Low(FMissingDataCounts) to High(FMissingDataCounts) do
        FMissingDataCounts[i] := 0;
      for i := 0 to TempSeqList.Count - 1 do
      begin
        ACharSeq := TempSeqList[i];
        for j := 0 to AInfo.MyNoOfSites - 1 do
        begin
          if ACharSeq[j] = '-' then
          inc(FMissingDataCounts[i]);
        end;
      end;
    end;
  finally
...
function TD_InputSeqData.PrepareDataForMLAnalysis(MAI: TObject;  const UpdateMissingDataCounts: Boolean): TIntArray;
var
  seq: Integer = 0;
begin
  FCountMissingData := UpdateMissingDataCounts;
  if FCountMissingData then
  begin
    SetLength(FMissingDataCounts, FNoOfTaxa);
    for seq := Low(FMissingDataCounts) to High(FMissingDataCounts) do
      FMissingDataCounts[seq] := 0;
  end;
  Result := PrepareDataForMLAnalysis(MAI);
end;
...
begin
  ...
  {$ENDIF}
  index := 0;
  while (index < Length(taxa)) and (index < limit) do
  begin
    {$IFDEF VISUAL_BUILD}
    if index = 9 then
      Result := Result + Format('%s%s+%d more', [#9, #9, Length(taxa) - 9]) + LineEnding
    else
    {$ENDIF}
      Result := Result + #9 + #9 + TOtuInfo(otus[taxa[index]]).Name + LineEnding;
    inc(index);
  end;
  {$IFDEF VISUAL_BUILD}
...
begin
  {$IFDEF VISUAL_BUILD}
   if Assigned(PleaseWait) and (not FIsRunningInThread) and PleaseWait.Visible then
     PleaseWait.Hide;
  {$ENDIF}
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        aInfo.InvalidDataRemoved := True;
        if Assigned(aInfo.MyOriTreeList) then
        begin
          for seq := Length(toRemove) - 1 downto 0 do
          begin
            msg := EmptyStr;
            aInfo.MyOriTreeList.RemoveOtu(aInfo.MyOtuNames[toRemove[seq]], True, msg);
            aInfo.AnalysisSummary.AddMessage(Format('Warning: "%s" was removed from the input tree because sequence data is missing for that taxon', [aInfo.MyOtuNames[toRemove[seq]]]));
            if msg <> EmptyStr then
              aInfo.AnalysisSummary.AddMessage('Warning: ' + msg);
          end;
          aInfo.MyUserNewickTree := aInfo.MyOriTreeList.OutputNewickTree(0, aInfo.MyOriTreeList.isBLen, aInfo.MyOriTreeList.isStats, 0.0);
        end
        ...
        begin
          ...
          begin
            msg := EmptyStr;
            aTreeList.RemoveOtu(aInfo.MyOtuNames[toRemove[seq]], True, msg);
            aInfo.AnalysisSummary.AddMessage(Format('Warning: "%s" was removed from the input tree because sequence data is missing for that taxon', [aInfo.MyOtuNames[toRemove[seq]]]));
            if msg <> EmptyStr then
              aInfo.AnalysisSummary.AddMessage('Warning: ' + msg);
          end;
          aInfo.MyUserNewickTree := aTreeList.OutputNewickTree(0, aTreeList.isBLen, aTreeList.isStats, 0.0);
        end;
        for seq := Length(toRemove) - 1 downto 0 do
        begin
...
//                            IntToStrWidth(Round(Input[Integer(CodonMap)]),Z);
    ...
    CodonXls.WriteLine;
    aRect := CodonXls.LastCellWriteXY(0);
    aRect.Right := 11;
    CodonXls.MergeCells(aRect);
    CodonXls.NewLine();
  end;
...
begin
  ...
  try try
    ...
    begin
      ...
      CodonXls.WriteLine('All frequencies are averages over all taxa.', 0);
      CodonXls.MergeCells(Rect(0, 0, 11, 0));
    end;
    ResultOutput.Add(rtfParaStr+'Relative synonymous codon usage is given in parentheses');
    CodonXls.WriteLine('Relative synonymous codon usage is given in parentheses following the codon frequency', 0);
    CodonXls.MergeCells(Rect(0, 1, 11, 1));
    CodonXls.NewLine();
...
var
  ColsHighlighted: Boolean = False;
  doBreak: Boolean = False;
  NextStartSite: Integer = 0;
  i: Integer = 0;
  j: Integer = 0;
  TheFirstSeq: Integer = 0; // the first sequence written
  MaxLabelLen: Integer = 0; // counts full length, including gp if needed to be written
  MaxSitesCount: Integer = 0; // holds the actual number of sites to write
  MaxTaxaCount: Integer = 0; // actual number of taxa to write
  DigitNum: Integer = 0;
  ...
  PDone: Integer = 0;
  FirstIncludedSite: Integer = 0;
  Accumulate : Integer = 0;
  AClusterSize: Integer = 0;
  ASitesPerLine: Integer = 0;
  StartSiteForLabels: Integer = 0;
  AMaxSiteNumStrLen: Integer = 0;
  CurDomainStartSite: Integer = 0;
  CurStartSite: Integer = 0;

  ACodonOptionsValid    : Boolean = False;
  AInclude1stPos        : Boolean = False;
  AInclude2ndPos        : Boolean = False;
  AInclude3rdPos        : Boolean = False;
  AInclude0thPos        : Boolean = False;
  AIncludeMissData      : Boolean = True;
  AIncludeGapData       : Boolean = True;
  AIncludeGps           : Boolean = True;
  AIncludeSps           : Boolean = True;
  AIncludePops          : Boolean = True;
  AIncludeHighlighted   : Boolean = False;
  AIncludeUnHighlighted : Boolean = False;
  AUseIdenSymbol        : Boolean = False;
  AWriteCodonByCodon    : Boolean = False;
  AWriteNumAtEnd        : Boolean = False;
  AWriteNumAtTop        : Boolean = False;
  AInterleaved          : Boolean = False;
  AIsPAUP4Format        : Boolean = False;
  AIsPAUP3Format        : Boolean = False;
  AIsMEGAFormat         : Boolean = False;
  AIsPhylipFormat       : Boolean = False;
  AIsCSVFormat          : Boolean = False;
  AIsFastaFormat        : Boolean = False;
  doXl: Boolean = False;
  tempSeq: PChar = nil;
  tsi: Int64 = 0; { tempSeq index}
  ...
  {
      This result does not need to be perfect but it cannot
      be too small otherwise memory corruption can occur. It
      is OK if the result is larger than needed.
  }
  function SizeOfLongestExpectedString: Integer;
  begin
    Result := FNoOfSites;
    if Options.SitesPerLine > Result then
      Result := Options.SitesPerLine;
    if Options.IsCodonByCodonWriting then
      Result := Result + (Result div 3) + 1;
    if Options.IsWriteSiteNumAtEnd then
      Result := Result + Length(Format('%.0n', [FNoOfSites*1.0])) + 2;
    Result := Result + (Result div 10) + 1; { for spaces every ten characters}
  end;
...
begin
 ...
 try
   try
    ...
    GetMem(tempSeq, SizeOf(Char)*SizeOfLongestExpectedString + 1);
...
procedure TD_InputSeqData.SelectFirstInEachGroup;
var
...
function TD_InputSeqData.GetNoOfDomains: Integer;
begin
  Result := 0;
  if Assigned(FAllDomainInfo) then
    Result := FAllDomainInfo.NoOfDomains;
end;
...
function TD_InputSeqData.MostSparseSequenceInfo(var seqName: String; var seqIndex: Integer; var dataCoverage: Integer): Boolean;
var
  i: Integer = -1;
  maxMissingDataIndex: Integer = 0;
  maxMissingData: Integer = -1;
begin
  if Length(FMissingDataCounts) = 0 then
    Exit(False);
  Assert(Length(FMissingDataCounts) = FOtuInfos.NoOfOtus, Format('mismatched dimensions. MissingDataCounts has %d items but FOtuInfos has %d items', [Length(FMissingDataCounts), FOtuInfos.NoOfOtus]));
  maxMissingData := FMissingDataCounts[0];
  if Length(FMissingDataCounts) > 1 then
    for i := 1 to High(FMissingDataCounts) do
    begin
      if FOtuInfos[i].IsUsed then
      begin
        if FMissingDataCounts[i] > maxMissingData then
        begin
          maxMissingData := FMissingDataCounts[i];
          maxMissingDataIndex := i;
        end;
      end;
    end;
  Result := True;
  seqName := FOtuInfos[maxMissingDataIndex].Name;
  seqIndex := maxMissingDataIndex;
  dataCoverage := FNoOfSites - maxMissingData;
end;
...
begin
  ...
  for i := Low(FAlleleFrequencies) to High(FAlleleFrequencies) do
    FAlleleFrequencies[i] := TAlleleFrequency.Create(FIsAmino, numUsed, NoOfSites, includeReferenceSeq, useReferenceSeq);
  ...
  begin
    ...
    begin
      if VS_SeqDataExplorer.IsTranslated then
        aSeq := PAnsiChar(FOtuInfos[i].RsvData)
      else
...
begin
  try
   ...
   groupCounts := TFPHashList.Create;
   ...
   {$IFNDEF VISUAL_BUILD}
   FreeAndNil(groupCounts);
   groupCounts := GetGroupCounts;
   report := TStringList.Create;
   report.Add(Format('%10s HaplotypeName', ['Frequency']));
   for i := 0 to FNoOfTaxa - 1 do
   begin
     aInfo := FOtuInfos[i];
     if aInfo.GpName <> EmptyStr then
     begin
       aCount := groupCounts.Find(aInfo.GpName);
       if Assigned(aCount) then
       begin
         report.Add(Format('%10d %s', [aCount.Count, aInfo.Name]));
         groupCounts.Remove(aCount);
         FreeAndNil(aCount);
       end;
     end
     else
     begin
       report.Add(Format('%10d %s', [1, aInfo.Name]));
     end;
   end;
   report.SaveToFile(NextAvailableFilenameNV('_frequencies.txt'));
   {$ENDIF}
  finally
