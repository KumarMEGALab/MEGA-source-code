...
begin
  ...
  try
    try
      ...
      //aInfo.UpdateInfo(99, 88, 77, 99999999, 'FAKE-MEGA-UPDATE-TEST', OPERATING_SYSTEM, ARCHITECTURE, 'beta', 'a dummy release for testing the update system in MEGA');
      //FReleases.Add(aInfo);
      //{$ENDIF}
      if d.Count > 0 then
...
begin
  Result := (OperatingSystem = aOs){$IFNDEF DARWIN} and (CpuArchitecture = aArch){$ENDIF};
end;
...
begin
  ...
  if (FReleaseType = 'alpha') and (aInfo.ReleaseType = 'developer') then
    Exit;
  Result := (OperatingSystem = aInfo.OperatingSystem){$IFNDEF DARWIN} and (CpuArchitecture = aInfo.CpuArchitecture){$ENDIF};
end;
