...
{$IFDEF VISUAL_BUILD}
uses
  Dialogs, Controls, DateUtils, StringUtils, fpjson, jsonparser, opensslsockets, {$IFDEF DARWIN}ns_url_request,{$ENDIF} MegaConsts;
...
begin
  ...
  try
    try
      ...
      HTTP.Method   := 'GET';
      HTTP.Address  := GetCurrentReleaseListQueryStr;
      if not (HTTP.SendAndReceive(Result)) then
