...
procedure MinimizeCaptionWhitespace(var aCaption: TStringList);
function MinimizeWhiteSpace(htmlInnerText: WideString): WideString;
function CleanupMegaCaptionText(htmlInnerText: String): String; { a hack for Linux since we can't use THtmlViewer to get clean inner text (we get it from TChromium but all the extra white space is there)}
...
begin
  if aCaption.Count > 0 then
    for i := aCaption.Count - 1 downto 0 do
    begin
      aCaption[i] := Trim(aCaption[i]);
      if aCaption[i] = EmptyStr then
        aCaption.Delete(i);
    end;
end;
...
function CleanupMegaCaptionText(htmlInnerText: String): String;
var
  aList: TStringList = nil;
begin
  try
    aList := TStringList.Create;
    aList.Text := htmlInnerText;
