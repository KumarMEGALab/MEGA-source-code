...
type
  ...
  TDistTreeDlgOption = (
			...
			dtdoMLInferAncSeqMyPeg,
                        ...
                        dtdoBEAM, dtdoSiteCoverage, dtdoMLModelTestIQTree, dtdoMLModelTamer,
                        dtdoLbsInference, dtdoLbsAnalyzeTree, dtdoLbsTiming, dtdoLbsGamma, dtdoLbsTsTvBias,
                        dtdoReltimeCalibrations
                        );
  ...
  private
    ...
    function GetComputeGammaAndInvarParameters: Boolean;
  ...
  published
    ...
    property ComputeGammaAndInvarParameters: Boolean read GetComputeGammaAndInvarParameters;
  ...
  function OperationTypeString(aOperation: TDistTreeDlgOption): String;
  function DistModelCitation(Value: TDistType): AnsiString;
  function DistCorrectionCitation(Value: TDistType): AnsiString;
...
begin
  ...
  case AOperation of
    ...
    dtdoCorrTestML,
    dtdoEpML,
    dtdoMLModelTest, dtdoMLModelTestIQTree, dtdoMLModelTamer,
    dtdoMCLComputePattern,
    dtdoMCLTsTvBias:           Result := 3;

    dtdoMPTree, dtdoMLTree, dtdoMLInferSiteBySiteRates, dtdoMLGammRates, dtdoMLComputePattern,
    dtdoMLTsTvBias, dtdoMPComputeUserTreeBLens: Result := 4;
    ...
    dtdoOLSComputeUserTreeBLens,
    dtdoPhyloQ,
    dtdoMLComputeUserTreeBLens: Result := 3;
  end;
...
function OperationTypeString(aOperation: TDistTreeDlgOption): String;
begin
  Result := 'MISSING OPERATION TYPE STRING';
  case aOperation of
    ...
    dtdoRelTimeML, dtdoLbsTiming:
       Result := 'Estimate Divergence Times (ML)';
    dtdoRtdtML:
       Result := 'Estimate Divergence Times (ML-RTDT)';
    dtdoCorrTestML               :
    ...
    dtdoRtdtLS:
       Result := 'Estimate Divergence Times (LS-RTDT)';
    dtdoRelTimeBLens:
       Result := 'Estimate Divergence Times (Branch Lengths)';
    dtdoRtdtBlens:
          Result := 'Estimate Divergence Times (Branch Lengths-RTDT)';
    dtdoMLCodonOmega             :
    ...
    dtdoMLModelTestIQTree:
       Result := 'Model Test (IQ-TREE2)';
    dtdoMLTsTvBias               :
       Result := opsEstTsTvML;
    dtdoMLGammRates              :
       Result := opsEstRateVarAmongSitesML;
    dtdoMLInferSiteBySiteRates   :
    ...
    dtdoNJTree:
       Result := 'NJ Phylogeny Reconstruction';
    dtdoMLTree, dtdoLbsInference:
       Result := 'Likelihood Phylogeny Reconstruction';
    dtdoMETree:
       Result := 'ME Phylogeny Reconstruction';
    dtdoMPTree:
       Result := 'Parsimony Phylogeny Reconstruction';
    dtdoUPGMATree:
       Result := 'UPGMA Phylogeny Reconstruction';
    dtdoMLIQTree:
       Result := 'MEGA to IQ-TREE2 Data Options';
    ...
    dtdoGeneDupInference: Result := 'Gene Duplication Inference';
  end;
end;
...
begin
  ...
  case Value of
    ...
    begin
      ...
      {$IFDEF VISUAL_BUILD}
        MessageDlg('MEGA has encountered an error in retrieving a citation for your caption.'+LineEnding+'Please report this to the authors by the bug report form on our webpage.'+LineEnding+'Technical Info: ' + 'DistModelName; The value "' + ValName + '" : TDistType was not found (missing case?)', mtInformation, [mbOK], 0);
      {$ELSE}
        Warn_NV('MEGA has encountered an error in retrieving a citation for your caption.'+LineEnding+'Please report this to the authors by the bug report form on our webpage.'+LineEnding+'Technical Info: ' + 'DistModelName; The value "' + ValName + '" : TDistType was not found (missing case?)');
      {$ENDIF}
    end;
  end;
end;
...
begin
  ...
  case Value of
    ...
    gdJonesPi:           Result := 'JTT matrix-based + Freq.';
    ...
    gdDayhoffPi:         Result := 'Dayhoff + Freq.';
    ...
    begin
      ValName := GetEnumName(TypeInfo(TDistType),integer(Value)) ;
      if IsDeveloper then
        Raise Exception.Create('TDistPack.DistCorrectionName; The value "' + ValName + '" : TDistType was not found (missing case?)')
      else
      {$IFDEF VISUAL_BUILD}
        MessageDlg('MEGA has encountered an error in retrieving a citation for your caption.'+ LineEnding +'Please report this to the authors by the bug report form on our webpage.'+LineEnding+'Technical Info: ' + 'TDistPack.DistCorrectionName; The value "' + ValName + '" : TDistType was not found (missing case?)', mtInformation, [mbOK], 0);
      {$ELSE}
        Warn_NV('MEGA has encountered an error in retrieving a citation for your caption.'+ LineEnding +'Please report this to the authors by the bug report form on our webpage.'+LineEnding+'Technical Info: ' + 'TDistPack.DistCorrectionName; The value "' + ValName + '" : TDistType was not found (missing case?)');
      {$ENDIF}
    end;
  end;
end;
...
function TDistPack.GetComputeGammaAndInvarParameters: Boolean;
begin
  Result := GetIsComputeGamma and GetIsComputeInvar;
end;
...
function TDistPack.GetRandSeed: Integer;
begin
  Result := FRandSeed;
end;
...
begin
  ...
  begin
    Result := Result + DistCorrectionCitation(DistCorrection);
  end
  else if DoesContain(gdThreeNuc) then
  begin
    Result := DistModelCitation(DistModel);
  end
  else if DoesContain(gdSynNonsyn) then
  begin
    Result := DistModelCitation(DistModel) ;
    if Length(DistCorrectionCitation(DistCorrection)) > 0 then
      Result := Result + ' ('+ DistCorrectionCitation(DistCorrection)+')';
  end
  else if DoesContain(gdAmino) then
  begin
    Result := Result + DistCorrectionCitation(DistCorrection);
    if DoesContain(gdGamma)  AND (Length(DistModelCitation(gdGamma)) > 0) then
    begin
      if DoesContain(gdHetero)  AND (Length(DistModelCitation(gdHetero)) > 0) then
        Result := Result + ' ('+ DistModelCitation(gdGamma)+','+DistModelCitation(gdHetero)+ ')'
      else
        Result := Result + ' ('+ DistModelCitation(gdGamma)+ ')';
    end;
    if DoesContain(gdHetero) AND (Length(DistModelCitation(gdHetero)) > 0)  AND (Length(DistModelCitation(gdGamma)) > 0) then
      Result := Result + ' ('+ DistModelCitation(gdGamma)+','+DistModelCitation(gdHetero)+ ')'
  end;
...
begin
  ...
  if  DoesContain(gdCompositionDistance) then
    ...
    Result := 'Disparity Index Test'
  else if DoesContain(gdRev) then  // GS - otherwise it goes to DistCorrectionNameStr and caption expert displays 'Data Specific model', 8-23-2011
...
begin
  ...
  try
    BlockRead(data, version, SizeOf(Integer)); // DIST_PACK_SESSION_VERSION
...
begin
  ...
  i := Length(FGeneticCode);
  BlockWrite(data, i, SizeOf(Integer));
  if i > 0 then begin
    StrPCopy(buffer, FGeneticCode);
