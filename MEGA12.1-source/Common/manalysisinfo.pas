...
type
  ...
  private
    ...
    FCalculateAbsoluteRates: Boolean;
    ...
    FUserTimeTreeRootNodeHeight: Double;
    ...
    function GetCanDisplayConsensusTree: Boolean;
  ...
  public
   ...
   MyTimetreeTaxonDataList: TTimeTreeTaxonDataList;
   procedure SetMyTimetreeTaxonData(otuIndex: Integer; ttData: TTimeTreeTaxonData);
   function TimetreeTaxonDataCount: Integer;
   function HasTimetreeData: Boolean;
   ...
   property CalculateAbsoluteRates: Boolean read FCalculateAbsoluteRates write FCalculateAbsoluteRates;
   property UserTimeTreeRootNodeHeight: Double read FUserTimeTreeRootNodeHeight write FUserTimeTreeRootNodeHeight;
  published
    ...
    property CanDisplayConsensusTree: Boolean read GetCanDisplayConsensusTree;
...
begin
  inherited create;
  FUserTimeTreeRootNodeHeight := 0.0;
  FCalculateAbsoluteRates := False;
  MyTimetreeTaxonDataList := nil;
...
//    FreeAndNil(MySeqStrings);
  ...
  if Assigned(MyTimetreeTaxonDataList) then
  begin
    if MyTimetreeTaxonDataList.Count > 0 then
      for i := 0 to MyTimetreeTaxonDataList.Count - 1 do
      begin
        if Assigned(MyTimetreeTaxonDataList[i]) then
           MyTimetreeTaxonDataList[i].Free;
      end;
    MyTimetreeTaxonDataList.Free;
  end;
end;
...
var
  ...
  {$ENDIF}
  activeDataNames:TStringList = nil;
  i, j: Integer;
  MyResult : Integer = -1;
  aName: String = '';
  aMsg: String = '';
  validator: TTreeList = nil;
  namesMissingInTree: TStringList = nil;
  extraNamesInTree: TStringList = nil;
  userInputRequired: Boolean = False;
  prunedTreeFile: String = '';
begin
  Result := False;

  try try
    activeDataNames := TStringList.Create;
    if Assigned(TaxaNames) then
      TaxaNames.Clear;
    for i := 0 to FMyUsedOtuInfos.Count - 1 do
      with TOtuInfo(FMyUsedOtuInfos[i]) do
      begin
        activeDataNames.Add(Name);
        if Assigned(TaxaNames) then
          TaxaNames.Add(Name);
      end;

    validator := TTreeList.Create;
    extraNamesInTree := TStringList.Create;
    namesMissingInTree := TStringList.Create;
    ...
    {$IFDEF VISUAL_BUILD}
      ...
      begin
        if not validator.ImportFromNewickFile(AFileName, nil, False) then
          raise Exception.Create('error when importing the Newick file: ' + ExtractFileName(AFileName));
        userInputRequired := (not validator.ValidateActiveDataNames(activeDataNames, namesMissingInTree, extraNamesInTree));
        if namesMissingInTree.Count > 0 then
        begin
        ...
        begin
          userInputRequired := False; { extra names in the tree file are automatically pruned here so no need for the user to fix it}
          if extraNamesInTree.Count > 0 then
          begin
            for i := 0 to extraNamesInTree.Count - 1 do
            begin
              validator.RemoveOtu(extraNamesInTree[i], validator[0].NumOutgroupMembers > 0, aMsg);
              AnalysisSummary.AddMessage(Format('Warning: tree taxon name not found in active data was pruned from the input tree: %s', [extraNamesInTree[i]]));
            end;
          end;
        end;
      end;
    ...
    { handle the user response if applicable - if they were never prompted, this case statement is skipped}
    case MyResult of
      MR_AUTOMATIC:
        begin
    ...
    if userInputRequired then
    begin
      ARP.Hide;
      ATreeDlg := TTreeInputForm.Create(Application);
      ATreeDlg.SetActiveDataNames(activeDataNames);
      if AFileName <> NotApplicableStr then
      begin
    ...
    if userInputRequired then { get the tree that the user edited using the TTreeInputForm}
    begin
    ...
    begin
      MyUserNewickTree := validator.OutputNewickTree(0, validator.isBLen, validator.isStats, 0.0);
      if extraNamesInTree.Count > 0 then { get the tree from the validator which has extra names pruned from it}
      begin
        prunedTreeFile := GetTempFileName(EmptyStr, ExtractFileName(AFileName));
        if not validator.ExportToNewickFile(prunedTreeFile, validator.isBLen, validator.isStats, 0.0) then
          raise Exception.Create('failed to save temp Newick file');
        MyTreePack.TreeFile := prunedTreeFile;
        MyUserTreeFile := prunedTreeFile;
      end
      else { the original input tree file works without modification}
      begin
        MyTreePack.TreeFile := AFileName;
        MyUserTreeFile := AFileName;
      end;
      FUserInputTreeIsRooted := validator.isRooted;
    end;
  {$ELSE}
      ...
      try
        ...
        begin
          ...
          pruner := TMegaTreePruner.CreateFromTreeList(aInputTree, activeDataNames, AGroupsList);
        ...
        ANamesList.Free;
        ANamesList := tempList;
        tempList := RegexReplaceAStringList(activeDataNames, '[^A-Za-z0-9.]', '_');
        activeDataNames.Free;
        activeDataNames := tempList;
        PerfectMatch := true;
        for i := 0 to activeDataNames.Count-1 do
        begin
          if ANamesList.IndexOf(activeDataNames.Strings[i]) < 0 then
            begin
            PerfectMatch := False;
            Result := False;
            Error_NV('The list of taxa in the tree file is ' +
              'different from the list of taxa in the sequence file, this must ' +
              'be corrected to complete this analysis.  Tree missing:' + activeDataNames[i]);
            end;
        end;
        for i := 0 to ANamesList.Count - 1 do
        begin
            if activeDataNames.IndexOf(ANamesList[i]) < 0 then
            begin
  ...
  {$ENDIF}
    ...
    begin
      if ATreeDlg.HasUnusedSequenceData then { the user has decided not to use all sequences in the alignment}
      begin
  ...
  if Assigned(validator) then
    validator.Free;
  if Assigned(namesMissingInTree) then
    namesMissingInTree.Free;
  if Assigned(extraNamesInTree) then
    extraNamesInTree.Free;
  ...
  if Assigned(aGroupsList) then
    aGroupsList.Free;
  {$ENDIF}
    if Assigned(activeDataNames) then
      activeDataNames.Free;
  end;
end;
  ...
  try
    ...
    begin
      if CurOperation = dtdoWithinGroupMean then
        ...
        RaiseLimitationMessage('At least '+IntToStr(ACount)+' taxa are required for ' + LowerCase(OperationTypeString(CurOperation)) + '.');
    end;
...
begin
  {$IFDEF VISUAL_BUILD}
  if MyUsrOperation = dtdoMLInferSiteBySiteRates then
  begin
    if FPrefsDlg.GetBooleanPickListSetting(opsIsTimetree2) then
      FCalculateAbsoluteRates := True;
  end;
...
begin
  ...
  begin
    ...
    if CurOperation in [dtdoMLGammRates,
                        dtdoMLInferSiteBySiteRates,
                        dtdoMLComputeUserTreeBLens,
                        dtdoMLModelTest,
                        dtdoMLModelTamer,
...
procedure TAnalysisInfo.SetMyTimetreeTaxonData(otuIndex: Integer; ttData: TTimeTreeTaxonData);
var
  i: Integer = -1;
  n: Integer = -1;
  aData: TTimeTreeTaxonData = nil;
begin
  if not Assigned(MyTimetreeTaxonDataList) then
  begin
    MyTimetreeTaxonDataList := TTimeTreeTaxonDataList.Create;
    n := GetNumTaxa;
    for i := 0 to n - 1 do
      MyTimetreeTaxonDataList.Add(nil)
  end;

  if Assigned(ttData) then
  begin
    if not Assigned(MyTimetreeTaxonDataList[otuIndex]) then
      MyTimetreeTaxonDataList[otuIndex] := TTimeTreeTaxonData.Create;
    MyTimetreeTaxonDataList[otuIndex].Assign(ttData)
  end
  else
  begin
    if Assigned(MyTimetreeTaxonDataList[otuIndex]) then
      MyTimetreeTaxonDataList[otuIndex].Free;
    MyTimetreeTaxonDataList[otuIndex] := nil;
  end;
end;

function TAnalysisInfo.TimetreeTaxonDataCount: Integer;
var
  i: Integer = -1;
begin
  Result := 0;
...
function TAnalysisInfo.HasTimetreeData: Boolean;
var
  i: Integer = -1;
begin
  Result := False;
...
function TAnalysisInfo.GetCanDisplayConsensusTree: Boolean;
begin
  Result := False;
  if not Assigned(MyBootPartitionList) then
    Exit;
  if MyBootPartitionList.NoOfPartitions > 0 then
  begin
