...
const
  ...
  MINOR_VERSION = 1;
  RELEASE_NUMBER = 0;
  AUTHOR_LIST = 'Sudhir Kumar, Glen Stecher, Michael Suleski, Maxwell Sanderford, Sudip Sharma, and Koichiro Tamura';
...
var
  ...
  aDay: String = '';
begin
  aRawDate := {$I %DATE%}; { formatted as YYYY/MM/DD see https://www.freepascal.org/docs-html/prog/progsu41.html}
  aYear := Copy(aRawDate, 3, 2);
  aMonth := Copy(aRawDate, 6, 2);
  aDay := Copy(aRawDate, 9, 2);
  Result := StrToInt(IntToStr(MAJOR_VERSION) + aYear + aMonth + aDay);
end;
