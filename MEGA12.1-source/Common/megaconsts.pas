...
type
  TASyncString = class(TObject) {use TASyncString to pass a string to QueueASyncCall}
    Value: UnicodeString;
  end;

  TASyncBoolean = class(TObject)
    Value: Boolean;
  end;
...
var
  HEAP_TRACE_SECTION_LABEL: AnsiString = 'HeapTraceSectionLabel'; // used for adding section labels to specific call trace blocks when using heaptrc so one can search the output easily
...
const
  ...
  RESEARCHER_ACADEMIA_STR = 'researcher_academia';
  RESEARCHER_INDUSTRY_STR = 'researcher_industry';
  RESEARCHER_GOVERNMENT_STR = 'researcher_government';
  INSTRUCTOR_12_STR = 'instructor_1_12';
  INSTRUCTOR_COLLEGE = 'instructor_college';
  DOCTORAL_STUDENT_STR = 'student_doctoral';
  POSTDOC_STR = 'researcher_postdoc';
  MASTERS_STUDENT_STR = 'student_masters';
  BACHELORS_STUDENT_STR = 'student_bachelors';
  SCHOOL_STUDENT_STR = 'student_1_12';
  IN_US_STR = 'located_in_us';
  UNDER_REPRESENTED_MINORITY_STR = 'under_represented_minority';
  FIRST_GEN_STUDENT_STR = 'first_gen_student';
  HAS_DISABILITY_STR = 'has_disability';
  LOW_INCOME_BG_STR = 'low_income_background';
  DECLINED = 'declined';
...
type
  ...
  TTimeTreeWizardMode = (ttwmSeqData, ttwmBlens, ttwmReltimeCalibrations);
...
const
  ...
  MTS_SESSION_VERSION = 1210; // the current version of tree session files
...
const
  ...
  {$IFDEF MSWINDOWS}
    ...
    {$ELSE}
    MUSCLE_EXE : String = 'muscleWin32.exe';
    {$ENDIF}
  {$ENDIF}
...
const
  ...
  SEARCH_BTN_DEFAULT_CAPTION = 'No Search Results Available';
...
var
  ...
  {$ENDIF}
  MegaGptUrl: String = 'https://chatgpt.com/g/g-RmeN18Ssp-mega-gpt';
  ...
  EnableSubsamplingAnalyses: Boolean = False;
  ...
  {$IFDEF LINUX}
  DrPhyloEnabled: Boolean = False;
  {$ELSE}
  DrPhyloEnabled: Boolean = True;
  {$ENDIF}

  AncestralStatesProbabilityCutoff: Double = -1.0;
  ...
  {$IFDEF DEBUG}
    DebugCriticalSection: TCriticalSection;
    {$IFNDEF VISUAL_BUILD}
 ...
 CheckUpdatesCS: TCriticalSection;
const
