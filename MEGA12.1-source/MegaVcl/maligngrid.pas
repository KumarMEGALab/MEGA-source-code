...
type
  ...
  private
    ...
    {$IFDEF DARWIN}FCaughtDoubledKeyEvent: Boolean;{$ENDIF}
    ...
    ColWidthSet: Boolean;
    function GetSeqNamesHaveInfo: Boolean;
  ...
  public
    ...
    property ShowInfoInName: Boolean read FShowInfoInName write FShowInfoInName;
    property SeqNamesHaveInfo: Boolean read GetSeqNamesHaveInfo;
...
begin
  ...
  {$IFDEF DARWIN}
  // workaround for spaces appearing twice
  // due to Cocoa IME issue introduced in Lazarus 3.0
  // certain controls that accept text input activate IME
  // which triggers the KeyPress event twice
  if (Key = ' ') then
  begin
    if (not FCaughtDoubledKeyEvent) then
      FCaughtDoubledKeyEvent := True
    else
    begin
      FCaughtDoubledKeyEvent := False;
      Exit;
    end;
  end;
  {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      if FShowInfoInName and (aSeq.SeqInfo <> EmptyStr) then
        aText := aText + ' ' + aSeq.SeqInfo;
    end;
    x := aRect.Left + 2;
    y := aRect.Top + 2;
...
function TAlignGrid.GetSeqNamesHaveInfo: Boolean;
var
  i: Integer = -1;
  s: TSequence = nil;
begin
  Result := False;
  if Assigned(SeqList) and (SeqList.Count > 0) then
    for i := 0 to SeqList.Count - 1 do
    begin
      s := SeqList[i];
      if s.SeqInfo <> EmptyStr then
        Exit(True);
    end;
end;
