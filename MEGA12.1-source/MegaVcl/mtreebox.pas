...
uses
  ...
  mtaxa_name_search, mtree_display_settings_cache, fpjson, jsonparser, mtimetree_map, syncobjs;
...
type
  ...
  private
    ...
    function GetNetBootstrap: Integer;
  ...
  public
    ...
    property NetBootstrap: Integer read GetNetBootstrap;
    constructor Create(Source: TTreeCustomControl);
    destructor Destroy; override;
  end;
  {$ENDIF VISUAL_BUILD}
  ...
  TNode   = record
      ...
      dataCoverage: Double;
      otuInfo: POtuInfo;
      timetreeData: TTimeTreeTaxonData;
      sortValue: LongWord;
 ...
 {$M+}
  ...
  private
    ...
    FAllowNegativeNodeStats: Boolean;
    ...
    procedure SetAllowNegativeNodeStats(AValue: Boolean);
    ...
    procedure SetShowNbsValues(AValue: Boolean);
  ...
  public
    DblClickNotify: TNotifyEvent;
    NodeFocusedChangeNotify: TNotifyEvent;
    IsCalibrationEditor: Boolean;
    TimeScale : double;
    ...
    function GetNode(index: Integer): TpNode;
    procedure SetPrivateName(aName: String; index: Integer);
    procedure SetTimetreeTaxonData(aData: TTimeTreeTaxonData; index: Integer);
    ...
    procedure GetDataForTimeTreeMap(const clusterRoot: Integer; var timetreeMap: TTimeTreeMap);
    procedure SetTimetreeTaxonData(const ttDataList: TTimeTreeTaxonDataList);
    ...
    function FindMrca(aCalib: TCalibrationTime): Integer; overload;
    ...
    function GetFocusedOtuName: String;
    ...
    function FirstLeafNodeForAncestorWithoutPrivateName(nodeIndex: Integer): Integer;
    ...
    property IsNbsTree: Boolean read GetIsNbsTree;
    ...
    property AllowNegativeNodeStats: Boolean read FAllowNegativeNodeStats write SetAllowNegativeNodeStats;
    property ShowNbsValues: Boolean read FShowNbsValues write SetShowNbsValues;
  published
...
private
  ...
  function GetPrivateNameList: TStringList;
  procedure ClearUndoStack;
public
  NameEditedNotify: TNotifyEvent;
  UndoStack: TStack;
  DropLocation: TPoint;
 ...
 { TTreeBox}
  ...
  TCustomTimetreeBox = class(TTreeBox)
    ...
    public
      ...
      destructor Destroy; override;
      function ReadFromFile(var data: File; sessionFileVersion: integer; SessionUsesExtended: Boolean):boolean; overload;
...
function TCustomTimetreeBox.ReadFromFile(var data: File; sessionFileVersion: integer; SessionUsesExtended: Boolean): boolean;
var
  aTimeTree: TLittleBootstrapsTimetree = nil;
  i: Integer = -1;
begin
  ...
  try
    if Result then
    begin
      aTimeTree := TLittleBootstrapsTimetree.CreateFromSessionFile(data, sessionFileVersion, SessionUsesExtended, TreeList.NoOfOTUs, TreeList.OTUNameList);
      FTimeTree := aTimeTree;
    end
    else
...
begin
    ...
    NodeFocusedChangeNotify := nil;
    FAllowNegativeNodeStats := False;
    IsCalibrationEditor := False;
    {$IFDEF DARWIN}
    FTextCanvasBitmap := TBitmap.Create;
    {$ENDIF}
    FScaleBarHeight := 0;
...
begin
  if IsCalibrationEditor then
  begin
    if Assigned(DblClickNotify) then
      DblClickNotify(Self);
  end
  else
  begin
    ...
    else if EditTimesEnabled and (FocusedTimeIndex > 0) then
      StartUserEditingTime
    else if Assigned(DblClickNotify) then
      DblClickNotify(Self);
  end;
  inherited;
end;
...
begin
  Result := False;
  {$IFDEF DARWIN}
  // Disabling the "hide overlapping times" feature on MacOS because
  // Cocoa canvases are not pixel-aware and creating a separate pixel map
  // negatively impacts performance, makes TreeBox unusable for large trees

  // Ideal solution would check whether the textbox overlaps only the lines
  // of neighboring nodes, and adjust the positioning of the text
  // An R-tree for tracking all drawn objects would be great, but possibly overkill
  Exit;
  {$ELSE}
  if aNode.OTU then
    Exit;
...
begin
    ...
    begin
      ...
      with aNode do
      begin
        if not FAllowNegativeNodeStats then
        begin
          if FShowNbsValues and (branch.meanBcl < StatsCutoff) then continue;
          if (not FShowNbsValues) and (branch.stat2 < StatsCutoff) then continue;
        end;

        if FShowStats or (CompareValue(branch.StatsStdDev, 0.0, FP_CUTOFF) <= 0) then
        begin
          if FShowNbsValues then
            aText := Format('%.0f', [branch.meanBcl])
          else
        ...
        if FShowNbsValues then
        begin
          if branch.meanBcl < 0 then
            Canvas.Font.Color := clBlue
          else
            Canvas.Font.Color := StatsFont.Color;
        end
        else
        begin
          if branch.stat2 < 0 then
            Canvas.Font.Color := clBlue
          else
            Canvas.Font.Color := StatsFont.Color;
        end;
...
function TTreeCustomControl.GetIsNbsTree: Boolean;
begin
  Result := False;
  if Assigned(TreeList) and (TreeList.Count > 0) then
    Result := TreeList[0].HasMeanBcl;
end;
...
procedure TTreeCustomControl.GetDataForTimeTreeMap(const clusterRoot: Integer; var timetreeMap: TTimeTreeMap);
var
  aRoot: TpNode = nil;

  procedure ProcessNode(aNodeIndex: Integer; isDes1: Boolean);
  var
    n: TpNode = nil;
  ...
  begin
    ...
    if n.OTU then
    begin
      if Assigned(n.timetreeData) then
      begin
        aData := n.timetreeData;
        aData.IsDescendantOneLineage := isDes1;
        timetreeMap.Add(aData);
      end;
    end;

    if Assigned(n.des1) then
      ProcessNode(n.des1.index, isDes1);
    if Assigned(n.des2) then
      ProcessNode(n.des2.index, isDes1);
  end;

begin
  aRoot := Node[clusterRoot];
  if aRoot.OTU then
    raise Exception.Create('cannot get data for timetree map from a tip node');
  ProcessNode(aRoot.des1.index, True);
  ProcessNode(aRoot.des2.index, False);
end;

procedure TTreeCustomControl.SetTimetreeTaxonData(const ttDataList: TTimeTreeTaxonDataList);
var
  i: Integer = -1;
  ttData: TTimeTreeTaxonData = nil;
begin
  for i := 1 to NoOfNodes do
    Node[i].timetreeData := nil;
  for i := 0 to ttDataList.Count - 1 do
  begin
    if Assigned(ttDataList[i]) then
    begin
      ttData := ttDataList[i];
      Node[ttData.OtuIndex + 1].timetreeData := ttData;
    end;
  end;
end;
...
procedure TTreeCustomControl.SetAllowNegativeNodeStats(AValue: Boolean);
begin
  if FAllowNegativeNodeStats = AValue then Exit;
  FAllowNegativeNodeStats := AValue;
end;
...
procedure TTreeCustomControl.SetShowNbsValues(AValue: Boolean);
begin
  if FShowNbsValues = AValue then Exit;
  FShowNbsValues := AValue;
  if FShowNbsValues then
  begin
    FShowDataCoverage := False;
    FShowNodeIds := False;
...
function TTreeCustomControl.FirstLeafNodeForAncestorWithoutPrivateName(nodeIndex: Integer): Integer;
var
  n: TpNode = nil;

  function PreorderTraversal(aNode: TpNode): Integer;
  begin
    Result := -1;
    if aNode.Otu and (aNode.PrivateName = EmptyStr) then
      Exit(aNode.index);
    if Assigned(aNode.des1) then
    begin
      Result := PreorderTraversal(aNode.des1);
      if Result <> -1 then
        Exit(Result);
    end;
    if Assigned(aNode.des2) then
    begin
      Result := PreorderTraversal(aNode.des2);
      if Result <> -1 then
        Exit(Result);
    end;
  end;
...
function TTreeCustomControl.GetFocusedOtuName: String;
begin
  Result := EmptyStr;
  if FocusedOnOtuBranch or FocusedOnOtuNode then
    Result := GetOTUName(FFocusedNameIndex);
end;
...
begin
    ...
    begin
        ...
        begin
          if FAllowNegativeNodeStats then
            SetDepth(Root, -100)
          else
            SetDepth(Root, 0);
        end;
...
var
  ...
  { swap red and blue bits because the virtual scrollbox is mixing RGB and BGR colors}
  function SwapRedBlue(Color: TColor): TColor;
  begin
    Result :=
      (Color and $FF000000) or               // Preserve alpha
      ((Color and $000000FF) shl 16) or      // Red becomes Blue
      (Color and $0000FF00) or               // Green stays
      ((Color and $00FF0000) shr 16);        // Blue becomes Red
  end;
begin
  try
    aName := p.name;
    ...
    begin
      ...
      if FTreeBoxType = tttEditBox then
        PrivateNameRect := Rect(x+xbase,y+ybase-(StrHeight(p.PrivateName, pfont)), x+xbase+(StrLength(p.PrivateName + ' ', pfont)), y+ybase)
      else
        PrivateNameRect := Rect(x+xbase,y+ybase-(StrHeight(p.PrivateName, pfont)), x+xbase+(StrLength(p.Name + ' ', pfont)), y+ybase);
      if FShowNodeIds then
        PrivateNameRect.Right := PrivateNameRect.Right + Canvas.TextWidth(Format('(%d)', [p.index]));
      FreeAndNil(pfont);
      BoxToClient(x+xbase, y+ybase, clientX, clientY);
      ...
      begin
        ...
        begin
          if FTreeBoxType = tttEditBox then
            ...
            Canvas.Font.Color := SwapRedBlue($00cb3232);
        end;
...
function TTreeCustomControl.FindMrca(aCalib: TCalibrationTime): Integer;
begin
  if (aCalib.NodeA = EmptyStr) or (aCalib.NodeB = EmptyStr) then
    raise Exception.Create('missing taxa names for finding mrca');
  Result := FindMrca(aCalib.NodeA, aCalib.NodeB);
end;
...
begin
  ...
  if Assigned(NodeFocusedChangeNotify) then
    NodeFocusedChangeNotify(Self);
...
begin
  if @OnNameClick <> nil then
    OnNameClick(Sender);
  if (FFocusedNameIndex > 0) then
    FocusOnNode(FFocusedNameIndex);
end;
...
begin
    ...
    begin
      FFocusedIndex := FFocusedNameIndex;
      ClickName(Self);
      FFocusedNameIndex := FFocusedIndex; { reset because it gets changed in procedures called by ClickName which clear focus}
      Result := true;
      Invalidate;
    end
...
function TTreeCustomControl.GetNode(index: Integer): TpNode;
begin
  Result := node[index];
end;

procedure TTreeCustomControl.SetPrivateName(aName: String; index: Integer);
begin
  if index <= NoOfNodes then
    Node[index].PrivateName := aName
  else
    raise Exception.Create(Format('Bad TTreeCustomControl index = %d. Must be 1 to %d', [index, NoOfNodes]));
end;

procedure TTreeCustomControl.SetTimetreeTaxonData(aData: TTimeTreeTaxonData;index: Integer);
begin
  if index <= NoOfNodes then
    Node[index].timetreeData := aData
  else
    raise Exception.Create(Format('Bad TTreeCustomControl index = %d. Must be 1 to %d', [index, NoOfNodes]));
end;
...
function TTreeCustomControl.FocusedOtuHasTimetreeData: Boolean;
begin
  if (FFocusedIndex > 0) and (FFocusedIndex <= FNoOfOTUs) then
    Result := Assigned(Node[FFocusedIndex].timetreeData)
  else
    Result := False;
end;
...
function TTreeCustomControl.FocusedOnCalibratedNode: Boolean;
var
  i: Integer = -1;
begin
  Result := False;
  if (Length(FCalibratedNodes) > 0) and (FFocusedIndex > 0) then
    for i := Low(FCalibratedNodes) to High(FCalibratedNodes) do
      if FCalibratedNodes[i] = FFocusedIndex then
        Exit(True);
end;
...
begin
  ...
  if aNode.OTU then
    Result := Format('isOtu: %s NBS: %.0f BCL: %.0f isOutgroup: %s index: %d  attr: %d  group: %d compIndex: %d isCompressed: %s auto_compressed: %s  anc: %d  d1: %d  d2: %d  %s', [BoolToStr(aNode.OTU, True), aNode.branch.meanBcl, aNode.branch.meanBcl, BoolToStr(aNode.outgroup, True), aNode.index, aNode.attrindex, aNode.groupindex, aNode.compressedIndex, BoolToStr(aNode.compressed, True), BoolToStr(aNode.autoCompressed, True), aNode.anc.index, -1, -1, aNode.Name])
...
function TTreeCustomControl.StatsDebugString(nodeIndex: Integer): String;
var
  aNode: TpNode;
begin
  aNode := Node[nodeIndex];
  if aNode.OTU then
    Result := Format('%-4d %8.4f %4d %s', [aNode.index, aNode.branch.stats, aNode.branch.stat2, aNode.name])
  else
    Result := Format('%-4d %8.4f %4d', [aNode.index, aNode.branch.stats, aNode.branch.stat2]);
end;
...
var
  size: Integer = 0;
  aMargin: Integer = -1;
  TextCanvas: TCanvas = nil;
begin
  {$IFDEF DARWIN}
  TextCanvas := FTextCanvasBitmap.Canvas;
  {$ELSE}
  TextCanvas := Canvas;
  {$ENDIF}
  ...
  size := TextCanvas.TextWidth(a.Caption + ' ');
  if a.ShowLabel then
...
begin
  Result := (Assigned(Root) and (FocusedIndex = Root.Index));
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TextCanvas.Font.Assign(aAttrib.Font);
          p.namesize.x := TextCanvas.TextWidth(p.name+' ');
          if p.PrivateName <> EmptyStr then
          begin
            p.namesize.x := p.namesize.x + (StrLength(p.PrivateName+' ', aAttrib.BoldFont));
          end;
            //p.namesize.x := p.namesize.x + (TextCanvas.TextWidth(p.PrivateName+' ') * 4);
          p.namesize.y := abs(TextCanvas.Font.Height);
        end;
        ...
        begin
          aAttrib := GetNodeAttrib(p);
          TextCanvas.Font.Assign(aAttrib.Font);
          p.speciesNameSize.x := TextCanvas.TextWidth(p.speciesName+' ');
          if p.PrivateName <> EmptyStr then
          begin
            p.speciesNameSize.x := p.speciesNameSize.x + (StrLength(p.PrivateName+' ', aAttrib.BoldFont));
          end;
          p.speciesNameSize.y := abs(TextCanvas.Font.Height);
        end;
      end
...
//        aAttrib.CaptionSize := TextCanvas.TextWidth(aAttrib.Caption)*4;
        if p.OTU then
        begin
          if p.compressed then
          begin
            TextCanvas.Font.Assign(aAttrib.CaptionFont);
            p.namesize.x := TextCanvas.TextWidth(aAttrib.Caption+' ');
            p.speciesNameSize.x := TextCanvas.TextWidth(aAttrib.Caption + ' ');
          end
          else
          begin
            TextCanvas.Font.Assign(aAttrib.Font);
...
begin
  ...
  // The way LCL is designed, calling Canvas.TextWidth on a control always triggers
  // a resize call (whether or not resizing is actually necessary).
  // The MacOS widgetset uses async calls when resizing controls, a serious issue
  // because the control may be freed before the async call is processed.
  // Since no drawing occurs in SetAttrIndex we can use the canvas from a
  // dedicated TBitmap object, which has no such async issues.

  {$IFDEF DARWIN}
  TextCanvas := FTextCanvasBitmap.Canvas;
  TextCanvas.Font.Assign(Font);
  {$ELSE}
  TextCanvas := Canvas;
  {$ENDIF}
...
var
  i : integer = -1;
  rootDescendantsStats: Integer = 0;
  rootMeanBcl: Double = 0;
  rootNetBootstrap: Integer = 0;
begin
  if MaxStats > 0.0 then
    for i := 1 to NoOfNodes do
    begin
      if Node[i] = Root then Continue;
      if (Node[i].branch.stats < -0.0000000000001) and (not FAllowNegativeNodeStats) then
        Node[i].branch.stat2 := -1
       else
        Node[i].branch.stat2 := Trunc(Node[i].branch.stats*100/MaxStats+0.000000000001);
    end
  ...
  if Assigned(aCalibrations) and (aCalibrations.Count > 0) then
    ...
    begin
      ...
      begin
        ...
        begin
          aMrca := FindMrca(calib.NodeA, calib.NodeB);
          FCalibratedNodes[High(FCalibratedNodes)] := aMrca;
          if calib.TreeViewFormNodeId <> aMrca then
            calib.TreeViewFormNodeId := aMrca;
        end
        else
...
function TBranchInfo.GetNetBootstrap: Integer;
begin
  with Tree do
    if (FocusedIndex = 0) or (FocusedIndex = Root.index) then
      Result := 0
    else
      Result := Round(Node[FocusedIndex].branch.meanBcl);
end;
...
begin
  ...
  begin
    ...
    newprivatename := GetPrivateNameList;
...
begin
  ...
  begin
    ...
    if Assigned(NameEditedNotify) then
      NameEditedNotify(Self);
  end;
end;
...
function TTreeEditBox.GetPrivateNameList: TStringList;
var
  i: Integer = 0;
begin
  Result := TStringList.Create;
  for i := 1 to FNoOfOTUs do
    Result.Add(Node[i].PrivateName);
end;
