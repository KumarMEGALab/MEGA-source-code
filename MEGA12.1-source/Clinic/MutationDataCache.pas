...
type
  ...
  TDataCacheThread = class(TThread)
    private
      ...
      AARegex: TRegExpr;
      FTokens: TStringList;
      FBadLines: TStringList;
      FFileList: TStringList;
      procedure BreakIntoFiles;
      procedure DoCachingCompleteEvent;
      procedure DoCachingCompleteEventWithBadlines;
      procedure DoCachingCancelledEvent;
      procedure DoCacheUpdateEvent;
...
procedure TDataCacheThread.DoCachingCompleteEventWithBadlines;
begin
  if Assigned(CachingCompleteEvent) and Assigned(FBadLines) then
     CachingCompleteEvent(FBadLines, True);
end;

procedure TDataCacheThread.DoCachingCompleteEvent;
begin
  if Assigned(CachingCompleteEvent) and Assigned(FFileList) then
     CachingCompleteEvent(FFileList, False);
end;

procedure TDataCacheThread.DoCachingCancelledEvent;
begin
  if Assigned(CachingCancelledEvent) and Assigned(FFileList) then
     CachingCancelledEvent(FFileList);
end;

procedure TDataCacheThread.DoCacheUpdateEvent;
begin
  if Assigned(CacheUpdateEvent) then
  ...
  try
    try
      ...
      begin
        try
          i := 0;
          FFileList.Add(TempDir + TempFileName + '_' + IntToStr(NumRequestsNeeded) + '.txt');
          ...
          begin
            ...
            begin
              FBadLines.Values[IntToStr(CurrentLine)] := TempStr;
              HasBadLines := True;
            end;
            inc(i);
            inc(CurrentLine);
          ...
          Synchronize(@DoCacheUpdateEvent);
        finally
          CloseFile(OutFile);
        end;

        if (Eof(InFile)) or (FBadLines.Count >= MAX_BAD_LINES_TO_SHOW) then
          break;
      ...
      begin
        ...
        Synchronize(@DoCacheUpdateEvent);
        if HasBadLines then
          Synchronize(@DoCachingCompleteEventWithBadlines)
        else
          Synchronize(@DoCachingCompleteEvent);
      end
      else
        Synchronize(@DoCachingCancelledEvent);
    except
      on E: Exception do
      begin
        FBadLines.Clear;
        FBadLines.Add('Error');
        FBadLines.Add(E.Message);
        HasBadLines := True;
        Synchronize(@DoCachingCompleteEventWithBadlines);
      end;
    end;
