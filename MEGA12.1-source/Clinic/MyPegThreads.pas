...
uses
  EvoD, MegaUtils, {$IFDEF DARWIN}ns_url_request{$ELSE}fphttpclient{$ENDIF};
...
var
  ...
  url, data: String;
  {$IFDEF DARWIN}
  aClient: TNSHTTPSendAndReceive = nil;
  {$ELSE}
...
begin
  ...
  try
    try
      ...
      {$IFDEF DARWIN}
      aClient := TNSHTTPSendAndReceive.Create;
      aClient.Address  := url;
      aClient.PostForm(data, Response);
      {$ELSE}
