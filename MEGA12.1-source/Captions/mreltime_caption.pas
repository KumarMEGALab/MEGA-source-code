...
begin
  ...
  try
    ...
    else if FAnalysisInfo.MyUsrOperation in [dtdoRelTimeBLens, dtdoRtdtBlens, dtdoSiteCoverage] then
    ...
    begin
      ...
      if FAnalysisInfo.CalibrationTimes.NumTimetreeCalibrations = 1 then
        b.Add(Format(' A single calibration time estimate was acquired from the Timetree resource [{%s:TimetreeCitation}][{%s:Mega12Point1Citation}].', [POST_PROCESS_CITATION, POST_PROCESS_CITATION]))
      else if FAnalysisInfo.CalibrationTimes.NumTimetreeCalibrations > 1 then
        b.Add(Format(' %d calibration time estimates were acquired from the Timetree resource [{%s:TimetreeCitation}][{%s:Mega12Point1Citation}].', [FAnalysisInfo.CalibrationTimes.NumTimetreeCalibrations, POST_PROCESS_CITATION, POST_PROCESS_CITATION]));
    end;
