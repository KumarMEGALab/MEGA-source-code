...
{ NOTE: the template for MegaCitation is not actually set here. It is set in
  mega_citation.pas which handles different major version numbers. This is
  so that opening session files made from older versions of MEGA will have the
  citation referencing the version of MEGA used for generating the session file.}
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        key := FTemplateConstants.Names[i];
        value := FTemplateConstants.Values[key];
        AssignData(key, value);
      end;
      AssignData('InDevelopment', '<h2>This template is still under development and the results can not be considered to be accurate.  This template will be corrected by the next release.</h2>');
      Result := True;
    end
    else
