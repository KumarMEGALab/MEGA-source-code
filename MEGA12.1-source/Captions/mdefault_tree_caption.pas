...
function TDefaultTreeCaption.ShowSiteCoverage: Boolean;
begin
  Result := False;
  {$IFDEF VISUAL_BUILD}
  if Assigned(FTreeBox) then
    Result := FTreeBox.ShowDataCoverage;
  {$ENDIF}
end;
...
function TDefaultTreeCaption.SiteCoverageCaption: String;
var
  b: TMegaStringBuilder = nil;
  tempStr: String = '';
begin
  try
    b := TMegaStringBuilder.Create;
    b.Add('The user-specified tree topology was analyzed to determine site coverage for each internal node in the tree. ');
    if ShowSiteCoverage then
    begin
      b.Add('The proportion of sites where at least 1 unambiguous base is present in at least 1 sequence for ');
      b.Add('each descendant clade is shown next to each internal node in the tree. ');
    end;
    ...
    begin
      ...
      b.Add(' &#32 taken from the user-provided tree.')
    end
    else
    begin
      if BLenCaption <> EmptyStr then
        b.Add(Format(' Branch lengths taken from the user-provided tree are shown &#32(%s). ', [BLenCaption]));
    end;
...
var
  ...
  tempStr: String = '';
begin
  if FAnalysisInfo.MyUsrOperation = dtdoSiteCoverage then
  begin
    Result := SiteCoverageCaption;
    Exit;
  end;
