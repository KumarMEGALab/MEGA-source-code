...
type
  ...
  public
    ...
    procedure SetHasAnalysisInfo(hasInfo: Boolean);
    ...
    procedure DisplayMessageLabel(aCaption: String);
    procedure HideMessageLabel;
...
uses
  Dialogs, MegaConsts, Clipbrd, StringUtils;
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := PutBackDisallowedChars(Result);
      if Trim(Result) = 'Tamura=Nei model' then
        Result := 'Tamura-Nei model';
    end;
  end;
end;
...
begin
  ...
  UpdateColumnWidths;
  g := GetGridForActiveTab;
  if (g = BranchInfoGrid) and (BranchInfoGrid.RowCount <= 1) then
    DisplayMessageLabel('Select a branch in the Tree Explorer to see info.')
...
procedure TTreeInfoFrame.SetHasAnalysisInfo(hasInfo: Boolean);
begin
  AnalysisSummaryTab.TabVisible := hasInfo;
end;
...
begin
  ...
  begin
    BranchInfoStrings.Add(Format('%.0n taxa in clade:', [tipNames.Count*1.0]));
...
procedure TTreeInfoFrame.DisplayMessageLabel(aCaption: String);
begin
  GetHeaderPanel.Visible := True;
  MessageLabel.Caption := aCaption;
end;

procedure TTreeInfoFrame.HideMessageLabel;
begin
  GetHeaderPanel.Visible := False;
end;
