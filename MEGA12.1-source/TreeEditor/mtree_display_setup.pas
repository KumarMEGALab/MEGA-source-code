...
begin
  ...
  try
    try
      ...
      aLegend := CleanupMegaCaptionText(caption.GenerateLegend);
...
{$IFNDEF VISUAL_BUILD}
procedure TTreeDisplaySetup.MLAncStatesToFasta;
var
  exporter: TMLAncestralStatesExporter = nil;
  fasta: TStringList = nil;
begin
  try
    if CompareValue(AncestralStatesProbabilityCutoff, 0.0, FP_CUTOFF) > 0 then
      D_MegaMain.AnalysisSummary.AddOptionString(Format('Ancestral states probability cutoff = %.4f', [AncestralStatesProbabilityCutoff]));
    exporter := TMLAncestralStatesExporter.Create(MAI.MyMLAnalysisPack, '');
    fasta := exporter.ExportMostProbableToFasta(MAI.isAminoAcid);
    fasta.SaveToFile(NextAvailableFilenameNV('_most_probable_sequences.fasta'));
  finally
    if Assigned(exporter) then
      exporter.Free;
...
begin
  try
    try
      ...
      D_MegaMain.AnalysisSummary.WriteToFile(ChangeFileExt(filename, '_summary.txt'));
      MinimizeCaptionWhitespace(aList);
...
begin
  try
    ...
    begin
      ...
      MinimizeCaptionWhitespace(CaptionStringList);
...
var
  ...
  sumOfRates: Double = 0.0;
  absRate: Double = 0.0;
  totalNumberOfSubstitutions: Double = 0.0;
...
begin
  ...
  try
    ...
    if MAI.MyMLAnalysisPack.Model.UseInvar then
      ExcelCOmponent.AddBlankCell;
    if aMAI.CalculateAbsoluteRates then
      ExcelComponent.AddBlankCell;
    ...
    if MAI.MyMLAnalysisPack.Model.UseInvar then
      ExcelComponent.AddBlankCell;
    if MAI.CalculateAbsoluteRates then
      ExcelComponent.AddBlankCell;
    ...
    if aMAI.CalculateAbsoluteRates then
      ExcelComponent.Add('Abs. Rate (x 1,000)');
    ...
    sumOfRates := s;
    totalNumberOfSubstitutions := aMAI.MyOriTreeList[0].SBL*MAI.NoOfSites;
    ...
    if MAI.CalculateAbsoluteRates then
    begin
      aSummary.AddCalculatedValue('Sum of relative rates', Format('%.3n', [sumOfRates]));
      aSummary.AddCalculatedValue('Rates scaling factor', Format('%.3f', [s]));
      aSummary.AddCalculatedValue('Total expected substitutions/site', Format('%.3f', [MAI.MyOriTreeList[0].SBL]));
      aSummary.AddCalculatedValue('Total expected substitutions', Format('%.3f', [totalNumberOfSubstitutions]));
      aSummary.AddCalculatedValue('Abs time elapsed (from user tree)', Format('%.3n', [aMAI.UserTimeTreeRootNodeHeight]));
    end;
    ...
    begin
      ...
      if aMAI.CalculateAbsoluteRates then
      begin
        absRate := ((r/sumOfRates)*totalNumberOfSubstitutions)/aMAI.UserTimeTreeRootNodeHeight*1000;
        ExcelComponent.Add(absRate);
      end;
      r := r/s;
  ...
  if MAI.MyMLAnalysisPack.NoOfRates > 1 then
    ...
    MinimizeCaptionWhitespace(Note);
