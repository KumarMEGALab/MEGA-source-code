...
type
  ...
  TTreeViewForm = class(TForm, IObservedForm)
    DisplayNetBootstrapAction: TAction;
    AllowNegativeStatsAction: TAction;
    ...
    procedure ActionMolecularClockExecute(Sender: TObject); deprecated;
    ...
    procedure AllowNegativeStatsActionExecute(Sender: TObject);
    ...
    procedure DisplayNetBootstrapActionExecute(Sender: TObject);
  ...
  private
    FUserWasPromptedToSaveSession: Boolean;
    ...
    {$IFDEF LINUX}
    FMegaFontDlg: TMegaFontDialog;
    {$ENDIF}
    procedure PromptToSaveSession(data: PtrInt);
    function GetFontPreferences(currentFont: TFont): TFont;
    procedure InitFonts;
    ...
    procedure SaveDialogTypeChanged(Sender: TObject);
    ...
    procedure BLensLineWidthNotify(cb: TObject);
    procedure SetTreeLineWidth(aWidth: Integer);
    ...
    procedure SetUpRelTimeCalibrations(aCalibType: TCalibrationTarget);
...
//    function GetRootedTrees(Progress: TRuntimeProgressDlg): TSimpleTreeNodeArrayArray; { gets an array of trees where each tree is rooted on a different branch; needed for finding the rootings which give the fewest number of gene duplications}
    ...
    procedure UpdateMacClientBounds;
...
uses
  ...
  MegaMainPreferences, MD_InputSeqData, non_modal_message_dlg;
...
begin
  FUserWasPromptedToSaveSession := False;
  IsNbsTree := False;
  SkipCloseQuery := False;
  ...
  FileSaveDialog.OnTypeChange := SaveDialogTypeChanged;
end;
...
begin
  ...
  Tree.Invalidate;
  {$IFDEF DARWIN}
  // Doesn't appear correctly, may need widgetset tweaks
  ProgressBar1.Visible := False;
  // Mac OS workaround - force resize the panel to properly show panel contents
  CaptionSplitter.MoveSplitter(1);
  CaptionSplitter.MoveSplitter(-1);
  // Additional workaround to prevent only half the panel from appearing
  UpdateClientSize;
  {$ENDIF}
...
begin
  ...
  Assert(CalibrationDlg.NumCalibrations > 0);
  try
    if InfoDisplay.Visible then
      InfoDisplay.Hide;
    ...
    CalibrationDlg.Show;
    if CalibrationDlg.CalibrationsType = ctLeafNode then
      CalibrationDlg.PromptIfTipDatesInNames
    else
      CalibrationDlg.ShowWelcomeMsg;
    CalibrationDlg.RefreshTimetreeTools;
    CalibrationDlg.AutoSizeActionExecute(Self);
  except
    on E:Exception do
    begin
...
begin
  ...
  begin
    ShowMessage('MEGA only supports the Reltime analysis without sequence data by using the Timetree Wizard. You can compute Reltime for a tree with branch lengths by clicking Clocks->Compute Timetree->Reltime-Branch Lengths on the main form.');
  ...
  try
    ...
    FTimeTreeWizard.WizardForm := FTimeTreeWizardForm;
...
begin
  raise Exception.Create('The molecular clock action is deprecated');
...
procedure TTreeViewForm.AllowNegativeStatsActionExecute(Sender: TObject);
begin
  AllowNegativeStatsAction.Checked := (not AllowNegativeStatsAction.Checked);
  Tree.AllowNegativeNodeStats := AllowNegativeStatsAction.Checked;
  Tree.Refresh;
end;
...
begin
  ...
  x := Self.Left + ((Self.Width - 420) div 2);
  y := Self.Top + ((Self.Height - 180) div 2);
  response := MessageDlgPos('Exiting Tree Explorer will discard the results. Save this session to store information for the future?', mtConfirmation, mbYesNoCancel, 0, x, y);
  WriteToDevConsole(Format('response = %d', [response]));
  {$ENDIF}
end;

procedure TTreeViewForm.DisplayNetBootstrapActionExecute(Sender: TObject);
begin
  DisplayNetBootstrapAction.Checked := not DisplayNetBootstrapAction.Checked;
  Tree.ShowNbsValues := DisplayNetBootstrapAction.Checked;
  if Tree.ShowNbsValues then
  begin
    Tree.AllowNegativeNodeStats := True;
    AllowNegativeStatsAction.Checked := True;
...
procedure TTreeViewForm.MenuItem144Click(Sender: TObject);
begin
  OpenUrl(MegaGptUrl);
end;
...
begin
  ...
  begin
    ...
    begin
      // Mac widgetset does not update TreeTabControl bounds at all the right times
      // UpdateClientSize must be called before and after expanding so that full control is drawn
      {$IFDEF DARWIN}UpdateClientSize;{$ENDIF}
      SidePanel.AutoSize := False;
      SidePanel.Width := 290;
...
var
  aFont: TFont = nil;
begin
  aFont := GetFontPreferences(Tree.BLensFont);
  if Assigned(aFont) then
  begin
    Tree.BLensFont.Assign(aFont);
    if IsReltimeTree then
      ReltimeTree.BLensFont.Assign(aFont);
    Tree.Invalidate;
    DataSaved := False;
...
var
  aFont: TFont = nil;
begin
  aFont := GetFontPreferences(Tree.CharStateFont);
  if Assigned(aFont) then begin
      OriTree.CharStateFont.Assign(aFont);
      if IsBootTree then
...
begin
  ...
  if Tree.NodeInfo.IsOrHasChildInOutgroup and (not (FCalibrationDlg.CalibrationsType = ctInternalNode)) then
  begin
...
begin
  ...
  begin
    ...
    try
      AncestralStatesProbabilityCutoff := -1;
      if InputQuery('Ancestral State Probability Cutoff', 'States with probability ≤ than the cutoff will be written as missing data (enter "0" or click "Cancel" to keep all states).', tempStr) then
      begin
      while (not TryStrToFloat(tempStr, tempFloat)) or (CompareValue(tempFloat, 0, FP_CUTOFF) < 0) or (CompareValue(tempFloat, 1.0, FP_CUTOFF) > 0) do
        if not InputQuery('Ancestral State Probability Cutoff', 'Please provide a number between 0.0 and 1.0 or click "Cancel" to include all states regardless of probability.', tempStr) then
          break;
        AncestralStatesProbabilityCutoff := tempFloat;
      end;
...
var
  aFont: TFont = nil;
begin
  aFont := GetFontPreferences(Tree.StatsFont);
  if Assigned(aFont) then
  begin
    OriTree.StatsFont.Assign(aFont);
    if IsBootTree then
...
begin
  ...
  begin
    ...
    ActionPointExecute(Sender); { turns the rooting tool off and sets cursor to the default}
    if ActionInfo.Checked then
      RenewBranchInfo;
...
var
  aFont: TFont = nil;
begin
  aFont := GetFontPreferences(Tree.ScaleFont);
  if Assigned(aFont) then
  begin
      OriTree.ScaleFont.Assign(aFont);
      if IsBootTree then
...
var
  ...
  aFont: TFont = nil;
begin
  aFont := GetFontPreferences(Tree.OTU_Font);
  if Assigned(aFont) then
  begin
    a := TNodeAttrib.Create;
    ...
    begin
      if CustomTimetree.GroupAttrib.Count > 0 then
        for i := 0 to CustomTimetree.GroupAttrib.Count-1 do
          if CustomTimetree.GroupAttrib.IsSameFont(i, CustomTimetree.OTU_Font) then
            CustomTimetree.GroupAttrib[i].Font.Assign(aFont);
      ...
      CustomTimetree.AttribList[0].Font.Assign(aFont);
    end;

    a.Free;
    OriTree.OTU_Font.Assign(aFont);
    if IsBootTree then
...
var
  aFont: TFont;
begin
  aFont := GetFontPreferences(Tree.TimesFont);
  if Assigned(aFont) then
  begin
    OriTree.TimesFont.Assign(aFont);
    if IsReltimeTree then
      ReltimeTree.TimesFont.Assign(aFont)
    else if (Tree = CustomTimetree) then
      CustomTimetree.TimesFont.Assign(aFont);
    Tree.Refresh;
    DataSaved := false;
    try
      IgnoreSideToolbarEvents(True);
...
var
  aResponse: Integer;
  aSyncString: TASyncString = nil;
{$ENDIF}
begin
  if IsSessionTest or SkipCloseQuery then
  begin
    CanClose := True;
    Exit;
  end;
  ...
  begin
    if FUserWasPromptedToSaveSession then
      CanClose := True
    else
    begin
    {$IFNDEF DEBUG}
    aSyncString := TASyncString.Create;
    CanClose := False;
    else
      aSyncString.Value := 'mega';
      Application.QueueAsyncCall(PromptToSaveSession, PtrInt(aSyncString));
    {$ENDIF}
    end;
  end;
end;
...
procedure TTreeViewForm.PromptToSaveSession(data: PtrInt);
var
  treeSource: String = '';
  aSyncString: TASyncString = nil;
  aPoint: TPoint;
  aMsg: String = 'Exiting Tree Explorer will discard the results. Save this session to store information for the future?';
  aDlgCaption: String = '';
  aButtonCaption: String = 'Save Session...';
  response: Integer = -1;
begin
  aSyncString := TASyncString(data);
  treeSource := aSyncString.Value;

  if treeSource = 'mega' then
  begin
    aDlgCaption := 'Save Session?';
    aButtonCaption := 'Save Session...';
  end
  else if treeSource = 'iqtree' then
  begin
    aDlgCaption := 'Save IQ-TREE2 Results Files?';
    aButtonCaption := 'Save IQ-TREE2 results files...';
  end
  else
  begin
    Assert(False, Format('invalid tree source "%s"', [treeSource]));
    aDlgCaption := 'Save Session?';
    aButtonCaption := 'Save Session...';
  end;
  ...
  try
    {$IFDEF DARWIN}
    aPoint.X := Self.Left + ((Self.Width - 420) div 2);
    aPoint.Y := Self.Top + ((Self.Height - 180) div 2);
    response := MessageDlgPos(aMsg, mtConfirmation, mbYesNoCancel, 0, aPoint.X, aPoint.Y);
    {$ELSE}
    response := QuestionDlg(aDlgCaption, aMsg, mtCustom, [mrYes, aButtonCaption, mrNo, 'Discard Results', mrCancel, 'Cancel'], 0);
    {$ENDIF}
    FUserWasPromptedToSaveSession := True;
    begin
      case response of
        mrYes: { Save Session}
          begin
            ActionSaveSessionExecute(nil);
            SkipCloseQuery := True;
            Self.Close;
            {$IFDEF DARWIN}Hide;{$ENDIF}
          end;
        mrNo: { Discard Results}
          begin
            SkipCloseQuery := True;
            Self.Close;
            {$IFDEF DARWIN}Hide;{$ENDIF}
    ...
    if Assigned(aSyncString) then
      aSyncString.Free;
  end;
end;

function TTreeViewForm.GetFontPreferences(currentFont: TFont): TFont;
begin
  Result := nil;
  {$IFDEF LINUX}
  if not Assigned(FMegaFontDlg) then
    FMegaFontDlg := TMegaFontDialog.Create(Self);
  FMegaFontDlg.Initialize(currentFont);
  if FMegaFontDlg.Execute then
    Result := FMegaFontDlg.GetSelectedFont;
  {$ELSE}
  FontDialog.Font.Assign(currentFont);
  if FontDialog.Execute then
  begin
    Result := TFont.Create;
    Result.Assign(FontDialog.Font);
  end;
  {$ENDIF}
end;
...
begin
  ...
  ActionTimesFont.Enabled := ((Tree = ReltimeTree) or (Tree = CustomTimetree));
  if Assigned(MAI) then
  begin
    if MAI.IsSubsampling then
      DisplayNetBootstrapAction.Enabled := IsNbsTree
    else
      DisplayNetBootstrapAction.Enabled := False;
  end
  else
    DisplayNetBootstrapAction.Enabled := False;
end;
...
procedure TTreeViewForm.SaveDialogTypeChanged(Sender: TObject);
var
  sa: TStringArray;
  index: Integer;
begin
  sa := FileSaveDialog.Filter.Split('|');
  index := (FileSaveDialog.FilterIndex - 1) * 2 + 1;
  FileSaveDialog.DefaultExt := sa[index].Remove(0,2);

  FileSaveDialog.FileName := ChangeFileExt(FileSaveDialog.FileName, '')
end;
...
var
  ...
  filter: String;
begin
  FileSaveDialog.InitialDir := GetCurrentDirUTF8;

  FileSaveDialog.Filter := 'Bitmap (.bmp)|*.bmp|' +
  ...
  case aFormat of
    iffBitmap: FileSaveDialog.DefaultExt := 'bmp';
    iffPdf: FileSaveDialog.DefaultExt := 'pdf';
    iffPng: FileSaveDialog.DefaultExt := 'png';
    iffSvg: FileSaveDialog.DefaultExt := 'svg';
    iffTiff: FileSaveDialog.DefaultExt := 'tif';
    iffEmf: FileSaveDialog.DefaultExt := 'emf';
  end;

  if SessionFileName <> '' then
    FileSaveDialog.FileName := ChangeFileExt(ExtractFileName(SessionFileName), FileSaveDialog.DefaultExt)
  else if Assigned(MAI) and (MAI.DataFileName <> EmptyStr) then
    FileSaveDialog.FileName := ChangeFileExt(ExtractFileName(MAI.DataFileName), FileSaveDialog.DefaultExt)
  else if MegaForm.DataFileName <> EmptyStr then
    FileSaveDialog.FileName := ChangeFileExt(ExtractFileName(MegaForm.DataFileName), FileSaveDialog.DefaultExt)
  else if Assigned(MAI) and (MAI.MyUserTreeFile <> EmptyStr) then
    FileSaveDialog.FileName := ChangeFileExt(ExtractFileName(MAI.MyUserTreeFile), FileSaveDialog.DefaultExt)
  else
    FileSaveDialog.FileName := 'MEGA_tree_image.' + FileSaveDialog.DefaultExt;

  case aFormat of
    iffBitmap: FileSaveDialog.FilterIndex := 1;
    iffPdf: FileSaveDialog.FilterIndex := 2;
    iffPng: FileSaveDialog.FilterIndex := 3;
    iffSvg: FileSaveDialog.FilterIndex := 4;
    iffTiff: FileSaveDialog.FilterIndex := 5;
    iffEmf: FileSaveDialog.FilterIndex := 6;
  end;
  ...
  begin
    try
      ...
      case FileSaveDialog.FilterIndex of
        1: Tree.SaveToBitmap(FileSaveDialog.FileName);
        2: Tree.SaveToPdf(FileSaveDialog.FileName);
        3: Tree.SaveToPng(FileSaveDialog.FileName);
        4: Tree.SaveToSvg(FileSaveDialog.FileName);
        5: Tree.SaveToTiff(FileSaveDialog.FileName);
        6: Tree.SaveToEmf(FileSaveDialog.FileName);
      end;
      except
        on E:Exception do
...
begin
  ...
  ImageForm.GetDialogButtonImageList.GetBitmap(5, FOkButton.Picture.Bitmap);
  ...
  ImageForm.GetDialogButtonImageList.GetBitmap(0, FCancelButton.Picture.Bitmap);
  ...
  {$IFDEF DARWIN}
  ButtonsPanel.Align := alTop;
  ButtonsPanel.Anchors := [akTop, akLeft, akRight];
  {$ELSE}
...
procedure TTreeViewForm.BLensLineWidthNotify(cb: TObject);
var
  b: TComboBox = nil;
  lineWidth: Integer = -1;
begin
  ...
  ValidateComboBox(cb);
  b := TComboBox(cb);
  lineWidth := b.ItemIndex + 1;
  SetTreeLineWidth(lineWidth);
end;

procedure TTreeViewForm.SetTreeLineWidth(aWidth: Integer);
var
  i: Integer = -1;
  a: TNodeAttrib = nil;
begin
...
begin
  ...
  try
    ...
    if aType <> STATS_DISPLAY_FREQUENCY then
      DisplayNetBootstrapAction.Checked := False;
...
begin
  if (Tree <> ReltimeTree) and (Tree <> CustomTimetree) then
  begin
    Assert(False, 'trying to change div times font on NOT a timetree');
    Exit;
  end;
...
begin
  ...
  FBLensFrame.LineWidthComboBox.OnChange := BLensLineWidthNotify;
end;
...
begin
  try
    ...
    FSidePanelToolbars.Add(FCommonlyUsedToolsFrame.ToolBar1);
    FSidePanelToolbars.Add(FCommonlyUsedToolsFrame.ToolBar2);
...
begin
  try
    ...
    FSidePanelToolbars.Add(FSubtreeFrame.ToolBar1);
...
begin
  ...
  try
    ...
    FBlensFrame.LineWidthComboBox.ItemIndex := max(0, Tree.BranchPen.Width - 1);
    ...
    if FAncestorsFrame.Visible and isAncState then
    begin
    ...
    if FAncestorsFrame.Visible and (not isAncState) then
    begin
      FAncestorsFrame.Top := currentTop;
      inc(currentTop);
    end;
...
begin
  ...
  begin
    ...
    ImageForm.UpdateToolbar(tbar);
    ImageForm.UpdateSpeedButton(FAncestorsFrame.AncExportSpeedBtn);
    ImageForm.UpdateSpeedButton(FAncestorsFrame.PrevChangeBtn);
    ImageForm.UpdateSpeedButton(FAncestorsFrame.NextChangeBtn);
  end;
end;
...
begin
  ...
  OriTree.DblClickNotify := TreeDblClickNotify;
  Tree := OriTree;
  ...
  BootTree.DblClickNotify := TreeDblClickNotify;
  ...
  ReltimeTree.DblClickNotify := TreeDblClickNotify;
...
begin
  ...
  begin
    if (not Assigned(MAI)) or (not (MAI.MyUsrOperation in [dtdoLbsInference, dtdoLbsAnalyzeTree])) then
    begin
      Inc(MaxTreeInfoIndex);
...
begin
  ...
  { the code below manually enables/disables the ancestral states actions due to a bug in Lazarus where
    the menu items are not synchronized with their associated actions. Maybe a race condition from
    enabling/disabling the parent 'Ancestors' menu or a problem with multiple components (see Ancestors
    frame in the side panel) being associated with the actions}
  MenuItem100.Enabled := ActionAncStateExportAllSites.Enabled;
  MenuItem98.Enabled := ActionActStateExportCurrentSite.Enabled;
  MenuItem97.Enabled := ActionActStateExportMostProbableSequence.Enabled;
  ExtCharAncItem.Enabled := ActionAncStateExtended.Enabled;
  MenuItem31.Enabled := ActionAncStateShowNone.Enabled;
  MenuItem30.Enabled := ActionAncStateHideAmbiguous.Enabled;
  MenuItem29.Enabled := ActionAncStateShowMost.Enabled;
  MenuItem28.Enabled := ActionAncStateShowAll.Enabled;
  MenuItem103.Enabled := ActionAncStateExportTextExport.Enabled;
  MenuItem102.Enabled := ActionAncStateExportPredictLivingSequence.Enabled;
  Exportmostprobablesequenceforcurrentlyselectednode1.Enabled := ActionActStateExportMostProbableSequence.Enabled;
  MenuItem101.Enabled := ActionActStateExportChangesList.Enabled;
end;
...
begin
  try
    ...
    on E:Exception do
      ShowMessage('An application error occurred when preparing the Calibration Wizard: ' + E.Message);
  end;
end;
...
begin
  ...
  FCalibrationDlg.HideWelcomeMsg;
  FCalibrationDlg.Hide;
...
begin
  ...
  if MAI.MyUsrOperation <> dtdoGeneDupInference then
  begin
...
var
  ...
  aLabels: TStringList = nil;
  aData: TTreeData = nil;
  SessionUsesExtended: Boolean = False;
begin
  ...
  try
    try
      ...
      begin
        ...
        SessionUsesExtended := SessionPlatformIsExtendedCompatible(i);
      ...
      begin
        ...
        begin
          InitCustomTimetree;
          if not TCustomTimetreeBox(CustomTimeTree).ReadFromFile(data, version, SessionUsesExtended) then
      ...
      begin
        ...
        begin
          ...
          MLAnalyzer.ReadFromFile(data, version, SessionUsesExtended, MAI.MyOutgroupMembersArray);
          if Assigned(MAI) then
      ...
      if version >= 702 then
        FReltimeAnalyzer.ReadFromFile(data, version, SessionUsesExtended);
      if FIsRelTimeTreeSession then
      begin
...
begin
  ...
  try
    try
      ...
      begin
        OriTree.AllowNegativeNodeStats := True;
        AllowNegativeStatsAction.Checked := True;
      end;
...
procedure TTreeViewForm.SetUpRelTimeCalibrations(aCalibType: TCalibrationTarget);
begin
  ...
  if aCalibType = ctLeafNode then
    ...
    FCalibrationDlg.ShowWelcomeMsg;
end;
...
procedure TTreeViewForm.TreeDblClickNotify(Sender: TObject);
begin
  if Tree.FocusedIndex < 1 then
    Exit;
  if not ActionInfo.Checked then
...
procedure TTreeViewForm.UpdateMacClientBounds;
begin
  {$IFDEF DARWIN}
  // hack to trigger TabControl bounds updates since MacOS widgetset doesn't
  // always update them at the right times
  TreeTabControl.Height := TreeTabControl.Height + 1;
  TreeTabControl.Height := TreeTabControl.Height - 1;
  {$ENDIF}
end;
...
begin
  try
    ...
    begin
      ...
      TreeTabControlChange(nil);
      FDivTimesFrame.Enabled := True;
      FDivTimesFrame.ToggleCollapseBodyPanel;
    end;
