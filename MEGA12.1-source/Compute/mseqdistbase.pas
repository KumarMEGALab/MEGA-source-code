...
uses
  ...
  MegaUtils, Math, MNucDist, ErrorMessages_HC, munique_seqs_counter;
...
var
  i, j, gI, gJ: Integer;
  TempDouble : Double;
  uniqueSeqsCounter: TUniqueSequenceCounter = nil;
  numUniqueSeqs: Integer = -1;
  tempGps: TIntArray = nil;
  ...
  function CalculateMeanOfWithinGroupMeans: Double;
  var
    gI: Integer;
  begin
    Result := 0;
  ...
  case ComputationType of
    ...
    gdAvgDiversityWithinSubPops:
      begin
        CalculateWithinGpMeans;
        try
          { per Sudhir, just use within group means and multiply by numUniqueSeqs/(numUniqueSeqs - 1) for each group's mean}
          uniqueSeqsCounter := TUniqueSequenceCounter.Create(FSequences, FNoOfSites);
          uniqueSeqsCounter.Initialize(FGpId);
          for i := 0 to FNoOfGps - 1 do
          begin
            numUniqueSeqs := uniqueSeqsCounter.NumUniqueSeqsInGroup(i);
            if numUniqueSeqs > 1 then
              CurMeanGpD[i] := (numUniqueSeqs/(numUniqueSeqs - 1))*CurMeanGpD[i]
            else
              CurMeanGpD[i] := 0;
          end;
        finally
          if Assigned(uniqueSeqsCounter) then
            uniqueSeqsCounter.Free;
        end;
      end;
    gdAvgDiversityForEntirePop:
      begin
        ...
        try
          { per Sudhir, just use overall mean and multiply by numUniqueSeqs/(numUniqueSeqs - 1) for each group's mean}
          SetLength(tempGps, FNoOfSeqs);
          for i := Low(tempGps) to High(tempGps) do
            tempGps[i] := 0;
          uniqueSeqsCounter := TUniqueSequenceCounter.Create(FSequences, FNoOfSites);
          uniqueSeqsCounter.Initialize(tempGps);
          numUniqueSeqs := uniqueSeqsCounter.NumUniqueSeqsInGroup(0);
          if numUniqueSeqs > 1 then
            CurMeanD := (numUniqueSeqs/(numUniqueSeqs - 1))*CurMeanD
          else
            CurMeanD := 0;
        finally
          if Assigned(uniqueSeqsCounter) then
    ...
    gdInterPopDiversity:
      begin
        CalculateWithinGpMeans;
        TempDouble := CalculateMeanOfWithinGroupMeans; // this update CurDiversity
    ...
    gdPropOfInterPopDiversity:
      begin
        CalculateWithinGpMeans;
        TempDouble := CalculateMeanOfWithinGroupMeans; //
        CalculateOverallMean;               // this updates CurMeanD
...
begin
  ...
  begin
    case ComputationType of
      ...
      gdAvgDiversityWithinSubPops, gdWithinGroupMean:
...
begin
  ...
  if VarType = gdBootstrapVar then
    case ComputationType of
      ...
      gdAvgDiversityWithinSubPops, gdWithinGroupMean:   SetLength(MeanGpDrep, FNoOfGps);
    end;
end;
