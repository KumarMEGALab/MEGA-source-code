...
type
  ...
  private
    ...
    procedure SetMyEslEnabled(value: Boolean);
  public
...
begin
  try
    ...
    if EnableSubsamplingAnalyses then
      EnableSubsamplingComboBx.ItemIndex := 0
    else
      EnableSubsamplingComboBx.ItemIndex := 1;
  finally
    FIgnoreEvents := False;
  end;
end;
...
begin
  ...
  if MyEslComboBx.ItemIndex = MyEslComboBx.Items.IndexOf('True') then
    SetMyEslEnabled(True)
  else
    SetMyEslEnabled(False);
end;
...
procedure TDeveloperSettingsDlg.SetMyEslEnabled(value: Boolean);
begin
  MegaForm.MyEslAction.Enabled := value;
  MegaForm.MyEslAction.Visible := value;
end;
