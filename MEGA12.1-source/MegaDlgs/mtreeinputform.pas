...
type
  ...
  private
    ...
    procedure LaunchNewickRecoveryFileThread;
    ...
    procedure NameEditedNotify(Sender: TObject);
    {$IFDEF DEBUG}
...
uses
  ...
  dateutils, mega_main, MMegaWindowInfo, StringUtils, mnewick_export_thread;
...
begin
  ...
  try
    if FShowAsTopologyEditor then
      DeleteRecoveryFile(GetPrivateOutputFile(NewickRecoveryFile));
  except
    on E:Exception do
    begin
      {$IFDEF DEBUG}
      ShowMessage('Failed to delete the newick recovery file: ' + E.Message);
      {$ENDIF}
    end;
  end;
end;
...
begin
  //AutosizeActive := not AutosizeActive; { turned off AutosizeActive for now until an unresolved crash bug when adding new taxa is fixed. }
...
procedure TTreeInputForm.NameEditedNotify(Sender: TObject);
begin
  if ShowAsTopologyEditor then
    LaunchNewickRecoveryFileThread;
end;
...
begin
  ...
  begin
    if FileExists(GetPrivateOutputFile(NewickRecoveryFile)) and FShowAsTopologyEditor then
      CustomRes := QuestionDlg('Tree Editor', 'A previous tree editing session can be restored. Would you like to open that tree for editing?', mtCustom, [CR_RECOVER_FILE, 'Restore Previous Session', CR_CANCEL, 'Skip', 'IsCancel'], 0);
    if CustomRes <> CR_RECOVER_FILE then
    ...
    if CustomRes = CR_RECOVER_FILE then
      OpenTree(GetPrivateOutputFile(NewickRecoveryFile))
    else if CustomRes = CR_FILE then
    begin
...
begin
  ...
  if (not FShowAsTopologyEditor) and (missingTaxaCount > 0) then
  begin
...
begin
  ...
  TreeToActiveMatchLBx.ItemHeight := TreeToActiveMatchLBx.Canvas.TextHeight('ABCXYZ');
  TreeToActiveMatchLBx.AutoSize := False;
  TreeToActiveMatchLBx.Height := Min(TreeToActiveMatchLBx.ItemHeight*TreeToActiveMatchLBx.Items.Count + {$IFDEF DARWIN}20{$ELSE}4{$ENDIF}, Self.Height - top - OKCancelPanel.Height);
  SetupTTAChkStates;
  SetStateOfTTAChkArray;
  TreeToActiveMatchLBx.Show
end;
...
begin
  ...
  begin
    ...
    begin
      if (MessageDlg('This tree name is already associated with another MEGA name, would you like to change the name association?', mtWarning, [mbYes, mbNo], 0) = mrYes) then
...
begin
  try
    ...
    on E:Exception do
      ShowMessage('Application Error when adding a new OTU: ' + E.Message);
  end;
end;
...
begin
  ...
  TreeEditBox.NameEditedNotify := NameEditedNotify;
end;
...
procedure TTreeInputForm.LaunchNewickRecoveryFileThread;
var
  filename: String = '';
  newick: String = '';
begin
  try
    newick := TreeEditBox.NewickTree;
    filename := GetPrivateOutputFile(NewickRecoveryFile);
    SaveNewickRecoveryFile(newick, filename);
  except
    on E:Exception do
    begin
    {$IFDEF DEBUG}
    ShowMessage('error in LaunchNewickRecoveryFileThread: ' + E.Message);
    {$ENDIF}
    end;
  end;
end;
