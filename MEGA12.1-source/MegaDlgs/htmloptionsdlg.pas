...
type
  ...
  THtmlOptionsDialog = class(TForm)
    ...
    procedure Chromium1BeforeDownload(Sender: TObject; const browser: ICefBrowser; const downloadItem: ICefDownloadItem; const suggestedName: ustring; const callback: ICefBeforeDownloadCallback{$IFDEF LINUX}; var aResult : boolean{$ENDIF});
...
procedure THtmlOptionsDialog.Chromium1BeforeDownload(Sender: TObject; const Browser: ICefBrowser; const downloadItem: ICefDownloadItem; const suggestedName: ustring; const callback: ICefBeforeDownloadCallback{$IFDEF LINUX}; var aResult : boolean{$ENDIF});
begin
...
{$IFDEF LINUX}
procedure THtmlOptionsDialog.Chromium1FileDialog(Sender: TObject; const browser: ICefBrowser; mode: TCefFileDialogMode; const title, defaultFilePath: ustring; const acceptFilters, accept_extensions, accept_descriptions: TStrings; const callback: ICefFileDialogCallback; var Result: Boolean);
{$ELSE}
procedure THtmlOptionsDialog.Chromium1FileDialog(Sender: TObject; const browser: ICefBrowser; mode: Cardinal; const title, defaultFilePath: ustring; const acceptFilters: TStrings; const callback: ICefFileDialogCallback; var Result: Boolean);
{$ENDIF}
var
  ...
  begin
    try
      try
        ...
        on E:Exception do
           ShowBrowserMessage('An application error has occurred: ' + E.Message);
      end;
      finally
