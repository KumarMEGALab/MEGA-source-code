...
type
  ...
  TCalibrationParamsDlg = class(TForm)
    ...
    procedure SingleTimeRadioBtnChange(Sender: TObject);
  ...
  private
    ...
    FTempCalibration: TCalibrationTime;
    ...
    function ActivePageToCalibrationCategory: TCalibrationCategory;
    procedure HideTabs;
  public
    procedure Initialize(aCalib: TCalibrationTime; taxaNames: TStringList);
...
procedure TCalibrationParamsDlg.SingleTimeRadioBtnChange(Sender: TObject);
begin
  if FixedTimeRadioBtn.Checked then
  begin
    SingleTimeLabel.Caption := 'Fixed Time';
    FTempCalibration.CalibrationCategory := ccFixedTime;
  end
  else if MinTimeRadioBtn.Checked then
  begin
    SingleTimeLabel.Caption := 'Min Time';
    FTempCalibration.CalibrationCategory := ccMinTimeOnly;
  end
  else if MaxTimeRadioBtn.Checked then
  begin
    SingleTimeLabel.Caption := 'Max Time';
    FTempCalibration.CalibrationCategory := ccMaxTimeOnly;
  end
  else
    raise Exception.Create('Application error: missing handler for radio button');
end;
...
begin
  FIsUpdating := False;
  RateTab.TabVisible := False; { disabled starting in M12 per Sudhir}
...
begin
  if Assigned(FDensity) then
    FDensity.Free;
  if Assigned(FTempCalibration) then
    FTempCalibration.Free;
end;
...
procedure TCalibrationParamsDlg.TaxonAComboBoxChange(Sender: TObject);
begin
  MrcaLabel.Caption := 'Target node is MRCA of:';
  MrcaLabel.Font.Color := clDefault;
  if Assigned(FTempCalibration) then
  begin
    FTempCalibration.NodeA := TaxonAComboBox.Items[TaxonAComboBox.ItemIndex];
    FTempCalibration.CalibrationName := EmptyStr;
    FTempCalibration.NodeName := EmptyStr;
    FTempCalibration.MrcaName := EmptyStr;
  end;
end;

procedure TCalibrationParamsDlg.TaxonBComboBoxChange(Sender: TObject);
begin
  MrcaLabel.Caption := 'Target node is MRCA of:';
  MrcaLabel.Font.Color := clDefault;
  if Assigned(FTempCalibration) then
  begin
    FTempCalibration.NodeB := TaxonBComboBox.Items[TaxonBComboBox.ItemIndex];
    FTempCalibration.CalibrationName := EmptyStr;
    FTempCalibration.NodeName := EmptyStr;
    FTempCalibration.MrcaName := EmptyStr;
  end;
...
begin
  ...
  else if PageControl1.ActivePage = UniformDistTab then
    UpdateAreaSeriesUniform;

  if PageControl1.ActivePage = SingleTimeTab then
...
begin
  ...
  case AValue of
    ...
    ClientHeight := PageControl1.Height + PageControl1.TabHeight + DistributionChart.Height + Panel1.height + MrcaLabel.Height + TaxonAComboBox.Height*2 + 20
end;
...
begin
  Result := False;
  if (TaxonAComboBox.ItemIndex < 0) or (TaxonBComboBox.ItemIndex < 0) then
  begin
    MrcaLabel.Caption := 'Please specify 2 taxa whose MRCA is the target node:';
    MrcaLabel.Font.Color := clRed;
    Exit(False);
  end;

  if TaxonAComboBox.Items[TaxonAComboBox.ItemIndex] = TaxonBComboBox.Items[TaxonBComboBox.ItemIndex] then
  begin
    MrcaLabel.Caption := 'Taxon B cannot be the same as Taxon A';
    Exit;
  end;

  FTempCalibration.CalibrationCategory := ActivePageToCalibrationCategory;
  FCalibrationCategory := FTempCalibration.CalibrationCategory;
  case FTempCalibration.CalibrationCategory of
    ccDensityNormal:
      begin
        ...
        FTempCalibration.SetCalibrationDensity(TCalibrationDensity.CreateNormalDist(NormalDistMeanEdit.Value, NormalDistStdDevEdit.Value));
      end;
    ccDensityLogNormal:
      begin
        ...
        FTempCalibration.SetCalibrationDensity(TCalibrationDensity.CreateLogNormalDist(LogNormalOffsetSpinEdit.Value, LogNormalMeanSpinEdit.Value, LogNormalStdDevSpinEdit.Value));
      end;
    ccDensityExponential:
      begin
        ...
        FTempCalibration.SetCalibrationDensity(TCalibrationDensity.CreateExponentialDistUsingLambda(ExpDistTimeEdit.Value, ExpDistRateEdit.Value));
      end;
    ccDensityUniform:
      begin
        ...
        FTempCalibration.SetCalibrationDensity(TCalibrationDensity.CreateUniformDist(UniformDistMinTimeEdit.Value, UniformDistMaxTimeEdit.Value));
      end;
    ccMinTimeOnly:
      begin
        FTempCalibration.MinTime := SingleTimeEdit.Value;
        FTempCalibration.MaxTime := -1;
      end;
    ccMaxTimeOnly:
      begin
        FTempCalibration.MinTime := -1;
        FTempCalibration.MaxTime := SingleTimeEdit.Value;
      end;
    ccMinMaxTime:
      begin
        FTempCalibration.MinTime := UniformDistMinTimeEdit.Value;
        FTempCalibration.MaxTime := UniformDistMaxTimeEdit.Value;
      end;
    ccFixedTime:
      begin
        FTempCalibration.MinTime := SingleTimeEdit.Value;
        FTempCalibration.MaxTime := FTempCalibration.MinTime;
      end;
    ccFixedRate:
      begin
        ...
        begin
          ...
          FTempCalibration.MinTime := FixedRateEdit.Value;
      end
    else
  ...
  FTempCalibration.NodeA := TaxonAComboBox.Items[TaxonAComboBox.ItemIndex];
  FTempCalibration.NodeB := TaxonBComboBox.Items[TaxonBComboBox.ItemIndex];
  FCalibration.Assign(FTempCalibration);
  Result := True;
end;

function TCalibrationParamsDlg.ActivePageToCalibrationCategory: TCalibrationCategory;
begin
  if PageControl1.ActivePage = UniformDistTab then
    Result := ccDensityUniform
  else if PageControl1.ActivePage = ExponentialDistTab then
    Result := ccDensityExponential
  else if PageControl1.ActivePage = NormalDistTab then
    Result := ccDensityNormal
  else if PageControl1.ActivePage = LogNormalDistTab then
    Result := ccDensityLogNormal
  else if PageControl1.ActivePage = SingleTimeTab then
  begin
    if MinTimeRadioBtn.Checked then
...
procedure TCalibrationParamsDlg.Initialize(aCalib: TCalibrationTime; taxaNames: TStringList);
var
  i: Integer = -1;
begin
  FTempCalibration := aCalib.Clone;
  FCalibration := aCalib;
  ...
  begin
    TaxonAComboBox.Items.Add(taxaNames[i]);
    TaxonBComboBox.Items.Add(taxaNames[i]);
  end;
  if FTempCalibration.NodeA <> EmptyStr then
    TaxonAComboBox.ItemIndex := TaxonAComboBox.Items.IndexOf(FTempCalibration.NodeA)
  else
    TaxonAComboBox.ItemIndex := -1;

  if FTempCalibration.NodeB <> EmptyStr then
    TaxonBComboBox.ItemIndex := TaxonBComboBox.Items.IndexOf(FTempCalibration.NodeB)
  else
    TaxonBComboBox.ItemIndex := -1;

  SetCalibrationCategory(FTempCalibration.CalibrationCategory);
  case FTempCalibration.CalibrationCategory of
    ccDensityNormal, ccDensityLogNormal, ccDensityExponential, ccDensityUniform:
      begin
        SetParamsFromDensity(FTempCalibration.GetCalibrationDensity);
      end;
    ccMinTimeOnly:
      begin
        MinTimeRadioBtn.Checked := True;
        PageControl1.ActivePage := SingleTimeTab;
        if FTempCalibration.MinTime > 0 then
          SingleTimeEdit.Value := FTempCalibration.MinTime;
      end;
    ccMaxTimeOnly:
      begin
        MaxTimeRadioBtn.Checked := True;
        PageControl1.ActivePage := SingleTimeTab;
        if FTempCalibration.MaxTime > 0 then
          SingleTimeEdit.Value := FTempCalibration.MaxTime;
      end;
    ccMinMaxTime:
      begin
        PageControl1.ActivePage := UniformDistTab;
        UniformDistMinTimeEdit.Value := FTempCalibration.MinTime;
        UniformDistMaxTimeEdit.Value := FTempCalibration.MaxTime;
      end;
    ccFixedTime:
      begin
