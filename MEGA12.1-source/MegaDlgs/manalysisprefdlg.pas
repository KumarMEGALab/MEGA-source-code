...
type
  ...
  private
    ...
    procedure SaveMaoFile(data: PtrInt);
    procedure SiteLabelsChechboxClick(Sender: TObject);
    procedure ClearSiteLabelCheckboxes;
    ...
    function  GetIsSiteLabelTypeUsed(aLabel: Char): Boolean;
    procedure SetIsSiteLabelTypeUsed(aLabel: Char; isUsed: Boolean);
  ...
  public
    ...
    function GetBooleanPickListSetting(settingName: String): Boolean;
...
begin
  FSiteLabelCheckboxes := TList.Create;
...
begin
  ...
  if Assigned(FSiteLabelCheckboxes) then
  begin
    ClearSiteLabelCheckboxes;
    FSiteLabelCheckboxes.Free;
  end;
end;
  ...
  begin
    try
      try
        ...
        begin
          Application.QueueAsyncCall(@SaveMaoFile, placeholder);
        end;
      except
        on E : Exception do
...
procedure TAnalysisPrefDlg.SaveMaoFile(data: PtrInt);
var
...
begin
  try
    try
      if GetMaoFileSaveLocation(MySettingsFileName) then
      begin
        maoStrings := generateMAOFile;
        maoStrings.SaveToFile(MySettingsFileName);
        CancelBtnClick(nil); // We want to close the APD after saving settings.
        StoreAllOptions;
      end
      else
        CancelBtnClick(nil);
      ...
      on E:Exception do
        ShowMessage('An error occurred when saving the mao file: ' + E.Message);
    end;
  finally
    if Assigned(maoStrings) then
      maoStrings.Free;
...
procedure TAnalysisPrefDlg.SiteLabelsChechboxClick(Sender: TObject);
var
  i: Integer = 0;
  MyCount: Integer = 0;
  cb: TCheckBox = nil;
begin
  for i := 0 to FSiteLabelCheckboxes.Count - 1 do
  begin
    cb := TCheckBox(FSiteLabelCheckboxes[i]);
    if cb.Checked then
      inc(MyCount);
  end;
  ...
  StoreOption(opsSelectLabelsPanel2, BoolToStr(cb.Checked, True), cb.Caption);
end;

procedure TAnalysisPrefDlg.ClearSiteLabelCheckboxes;
var
  i: Integer = 0;
begin
  if FSiteLabelCheckboxes.Count > 0 then
    for i := 0 to FSiteLabelCheckboxes.Count - 1 do
      TCheckBox(FSiteLabelCheckboxes[i]).Free;
  FSiteLabelCheckboxes.Clear;
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SelectLabelsPanel.Enabled := False;
          end
          else
          begin
            ASetting.IsReadOnly := False;
            ...
            SelectLabelsPanel.Enabled := True;
          end;
        end;
      end
...
begin
  ...
  Assert(Assigned(ProcessPackTemp));
...
begin
  ...
  if FOperation in [dtdoRelTimeML] then
  begin
    if HasSetting(opsSubsamplingVerifyResult) then
    begin
      aSetting := GetSetting(opsSubsamplingVerifyResult);
      if HasSetting(opsEstimateVar2) and (CurrentOptionStr(opsEstimateVar2) = opsSubsampling2) then
        aSetting.SetState(False, True)
      else
        aSetting.SetState(True, False);
    end;
    ...
    dtdoMLModelTest, dtdoMLModelTamer:
      begin
        ...
        begin
          ...
          if FOperation = dtdoMLInferSiteBySiteRates then
            AddPickListSetting(opsIsTimetree2, YesNoPickList, 'YesNoPickList', 'If times are given, they will be used to calculate absolute site-by-site rates');
        end;
      ...
      dtdoMLModelTest, dtdoMLModelTamer:
        begin
          ...
          begin
            ...
            CurrentOption(opsPickUserTree2).SetState(True, False);
            if HasSetting(opsIsTimetree2) then
              CurrentOption(opsIsTimetree2).SetState(True, False);
          end
          else
          begin
            if HasSetting(opsIsTimetree2) then
              CurrentOption(opsIsTimetree2).SetState(False, True);
            ...
            if (not IsPrototyper) then
            begin
              CurrentOption(opsPickUserTree2).SetState(False, True);
            end
            else
            begin
...
begin
  ...
  if FHasSiteLabelTypes and (FSiteLabelCheckboxes.Count > 0) then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      SelectLabelsPanel.Enabled := (GetPickListSettingValue(opsLabelledSites2) = OnlyLabeledSitesStr);
    end;
    PositionEllipsisPanel(SelectLabelsPanel);
...
function TAnalysisPrefDlg.GetIsSiteLabelTypeUsed(aLabel: Char): Boolean;
var
  i: Integer = 0;
  cb: TCheckBox = nil;
begin
...
procedure TAnalysisPrefDlg.SetIsSiteLabelTypeUsed(aLabel: Char; isUsed: Boolean);
var
  i: Integer = 0;
  cb: TCheckBox = nil;
begin
...
function TAnalysisPrefDlg.GetBooleanPickListSetting(settingName: String): Boolean;
var
  s: TAnalysisSetting = nil;
  temp: String = '';
begin
  ...
  begin
    ...
    if s is TPickListAnalysisSetting then
      ...
      begin
        ...
        if (temp = 'yes') or (temp = 'true') then
          Result := True
        else if (temp = 'no') or (temp = 'false') then
          Result := False
        else
          raise Exception.Create(Format('invalid boolean pick list setting: "%s"', [temp]));
      end;
  end;
end;
...
var
  newCheckbox: TCheckBox = nil;
  lastCheckbox: TCheckBox;
begin
  newCheckbox := TCheckBox.Create(nil);
  newCheckbox.Parent := SelectLabelsPanel;
  newCheckbox.AutoSize := False;
  newCheckbox.Width := 30;
  newCheckbox.Caption := Value;
  newCheckbox.Checked := True;
  newCheckbox.OnClick := @SiteLabelsChechboxClick;
  newCheckbox.Top := 0;
  if FSiteLabelCheckboxes.Count > 0 then
  begin
    lastCheckbox:= TCheckBox(FSiteLabelCheckBoxes[FSiteLabelCheckBoxes.Count - 1]);
    newCheckbox.Left := lastCheckbox.Left + lastCheckbox.Width + 1;
  end
  else
    newCheckbox.Left := 8;

  FSiteLabelCheckboxes.Add(newCheckbox);
end;
