...
begin
  ...
  try try
     ...
     begin
       ...
       // We don't actually need to check platform here
       // MSDX doesn't utilize Extended type
       // so should be aligned the same on all platforms
...
begin
  ...
  try
    try
      ...
      if (not IsPrototyper) and (not IsDeveloper) and (seqs.MaxNoOfSites*seqs.Count > SDE_MAX_TOTAL_SITES) then
      begin
...
begin
  ...
  try
    try
      ...
      if (not IsPrototyper) and (not IsDeveloper) and (Parser.NoOfOtus*Parser.NoOfSites > SDE_MAX_TOTAL_SITES) then
      begin
        if not Assigned(MegaInfoForm) then
          MegaInfoForm := TMegaInfoForm.Create(MegaForm);
        MegaInfoForm.SetInfoText(MessageStringForDisabledSeqDataExplorer(GetDataTypeString, ANoOfOtus, ANoOfSites));
...
{$ENDIF}
  ...
  try
    ...
    if (not IsPrototyper) and (not IsDeveloper) and (ANoOfOtus*ANoOfSites > SDE_MAX_TOTAL_SITES) then
    begin
      if not Assigned(MegaInfoForm) then
        MegaInfoForm := TMegaInfoForm.Create(MegaForm);
      MegaInfoForm.SetInfoText(MessageStringForDisabledSeqDataExplorer(GetDataTypeString, ANoOfOtus, ANoOfSites));
...
{$ENDIF}
  ...
  {$ENDIF}
    ...
    VS_SeqDataExplorer := TVS_SeqDataExplorer.Create;
    {$IFDEF VISUAL_BUILD}
    if (not IsPrototyper) and (not IsDeveloper) and (NoOfOtus*NoOfSites > SDE_MAX_TOTAL_SITES) then
    begin
      if not Assigned(MegaInfoForm) then
        MegaInfoForm := TMegaInfoForm.Create(MegaForm);
      MegaInfoForm.SetInfoText(MessageStringForDisabledSeqDataExplorer(GetDataTypeString, ANoOfOtus, ANoOfSites));
