...
uses
  ...
  MegaAnalysisPrefStrings, ml_thread_terminator, MAnalysisSummary;
...
//                                            MAI.MyNoOfSeqs, MAI.MyNoOfSites,  MAI.MyLabelsUsed, MAI.RecodeScheme,MAI.SiteCoverage);
    ...
    if MAI.MyTreePack.DoesContain(ttUserTree) and (UsrOperation in [dtdoMLInferSiteBySiteRates]) then
    begin
      aNewick := MAI.MyUserNewickTree;
      aSummary.AddCalculatedValue('timetree', aNewick);
      {$IFDEF VISUAL_BUILD}
      if MAI.CalculateAbsoluteRates then
      begin
        if not TreeIsUltrametric(aNewick, MAI.MyOtuNames) then
        begin
          ShowMessage('User tree is not ultrametric. Absolute site-by-site rates will NOT be calculated.');
          MAI.CalculateAbsoluteRates := False;
        end
        else
          MAI.UserTimeTreeRootNodeHeight := GetRootNodeHeight(aNewick, MAI.MyOtuNames);
      end;
      {$ELSE}
      if MAI.CalculateAbsoluteRates or TreeIsUltrametric(aNewick, nil) then
      begin
        MAI.CalculateAbsoluteRates := True;
        MAI.UserTimeTreeRootNodeHeight := GetRootNodeHeight(aNewick, nil);
      end;
      {$ENDIF}
    end;
    ...
    if (MAI.MyUsrOperation = dtdoMLTree) and (MAI.MyNoOfSeqs < 4) then
      RaiseErrorMessage(HC_Not_Enough_Taxa_Selected, 'At least four sequences are needed for likelihood tree construction');
...
var
  ClockTestThread: TMLClockTestThread = nil;
  Terminator: TMLThreadTerminator = nil;
  Response: Integer = -1;
  aMsg: String = '';
