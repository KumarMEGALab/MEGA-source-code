...
uses
  ...
  ComCtrls, ExtCtrls, Buttons, MegaUtils, MegaConsts,
...
begin
  ...
  try
    try
      ...
      begin
        {$IFDEF VISUAL_BUILD}
        ShowMessage('An error occurred when finalizing the calculation: ' + E.Message);
        {$ELSE}
        error_nv('An error occurred when finalizing the calculation: ' + E.Message, E);
        {$ENDIF}
      end;
    end;
  finally
...
var
  ctestThread: TCorrTestMLThread = nil;
  ctestBlenThread: TCorrTestBlenThread = nil;
  MAI: TAnalysisInfo = nil;
begin
...
procedure ComputeRelTimeLSTree(MAI: TAnalysisInfo; IsUpdateARP: Boolean);
var
  RelTimeThread: TRelTimeLSThread = nil;
...
begin
  ...
  try
    ...
    {$IFDEF VISUAL_BUILD} Assert(MAI.ARP.Visible, 'progress display must be made visible from the context of the main gui thread'); {$ENDIF}
    ACPTestThread.Start;
...
begin
  ...
  try
    ...
    {$IFDEF VISUAL_BUILD} Assert(MAI.ARP.Visible, 'Progress display must be made visible from within the main gui thread'); {$ENDIF}
    ANJThread.Start;
    ANJThread := nil;
    ANJThreadTerminator := nil;
  finally
