...
uses
  ...
  cthreads,
  {$ENDIF}{$ENDIF}
  uCEFApplicationCore, mbrowserutils, uCEFInterfaces, uCEFTypes,
  SysUtils, uCEFProcessMessage, uCEFDomVisitor, StringUtils;
  ...
  try
    try
      ...
      begin
        ...
        debugFile := ExtractFilePath(ParamStr(0)) + 'subprocess-render.log';
        aTxt := Format('%s   MEGA RenderProcess: %s', [aTxt, aMsg])
      end
      else if sourceProcess = PID_BROWSER then
      begin
        debugFile := ExtractFilePath(ParamStr(0)) + 'subprocess-browser.log';
        aTxt := Format('%s   MEGA BrowserProcess: %s', [aTxt, aMsg])
      end
      else
      begin
        debugFile := ExtractFilePath(ParamStr(0)) + 'subprocess-unknown.log';
...
begin
  try
    try
      WriteToLog('fetching tree options');
    ...
    WriteToLog('sending response from renderer');
    frame.SendProcessMessage(PID_BROWSER, message);
  end;
end;
...
procedure MyProcessMessageHandler(const browser: ICefBrowser; const frame: ICefFrame; sourceProcess: TCefProcessId; const message: ICefProcessMessage; var aHandled: Boolean);
var
  debug : String;
  ...
  // and user data.
{
  ...
  GlobalCEFApp.OnProcessMessageReceived := @MyProcessMessageHandler;
