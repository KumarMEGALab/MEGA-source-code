...
type
  ...
  TMegaBrowserFrm = class(TForm)
    ...
    {$IFDEF VISUAL_BUILD}
    procedure crmBeforeDownload(Sender: TObject; const Browser: ICefBrowser; const downloadItem: ICefDownloadItem; const suggestedName: ustring; const callback: ICefBeforeDownloadCallback{$IFDEF LINUX}; var aResult : boolean{$ENDIF});
...
var
  aDlg: TFindDialog = nil;
begin
  aDlg := TFindDialog.Create(Self);
  aDlg.Options := [frDisableWholeWord, frDown, frHideWholeWord];
  aDlg.OnFind := FindDialog1Find;
  aDlg.Execute;
end;
...
procedure TMegaBrowserFrm.crmBeforeDownload(Sender: TObject; const Browser: ICefBrowser; const downloadItem: ICefDownloadItem; const suggestedName: ustring; const callback: ICefBeforeDownloadCallback{$IFDEF LINUX}; var aResult : boolean{$ENDIF});
var
  downloadLoc: String;
begin
...
var
  ...
  aForwards: Boolean = False;
  aFindNext: Boolean = False;
  aDlg: TFindDialog = nil;
begin
  aDlg := TFindDialog(Sender);
  if Assigned(activeTabChromium) then
  begin
    if frMatchCase in aDlg.Options then
      aCaseSensitive := True;
    if frDown in aDlg.Options then
      aForwards := True;
    if frFindNext in aDlg.Options then
      aFindNext := True;
    aDlg.Options := aDlg.Options +  [frFindNext];
    activeTabChromium.Browser.Host.Find(aDlg.FindText, aForwards, aCaseSensitive, aFindNext);
...
begin
  ...
  try
    ...
    case FBrowserMode of
      bmBrowser: ABrowser.RunInWebBrowserMode;
      bmCaption: ABrowser.RunInCaptionMode;
      bmResults: ABrowser.RunInExcelExportMode;
      bmHelpBrowser: ABrowser.RunInHelpMode;
      bmCitation: ABrowser.RunInCaptionMode;
      bmFileViewer: ABrowser.RunInFileViewerMode;
    end;
    url := Trim(FLinkUrl);
