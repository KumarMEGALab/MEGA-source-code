...
type
  ...
  TMegaAlignmentFileLoader = class(TAlignmentFileLoader)
    private
      ...
      function PromptUserForInfo: Boolean;
...
var
  ...
  SymbolStr: String = '';
  {$ENDIF}
  MyDataFileFormat: TDataFileFormat = dfUnknown;
  ADataType: String = '';
  AGapSym: String = '';
  AMissingSym: String = '';
  AIdentSym: String = '';
...
function TMegaAlignmentFileLoader.PromptUserForInfo: Boolean;
  {$IFNDEF VISUAL_BUILD}
var
  ProcessPack: TProcessPack = nil;
  ContainsCoding: String = '';
...
begin
  {$IFDEF VISUAL_BUILD}
  Result := PromptUserForDataFileInfo(FDataFileInfo, False);
  if Result then
  begin
    FDataFileInfo.SetUserSpecifiedSymbols(true);
...
begin
  ...
  try
    ...
    On E: Exception do
    begin
      if FShowProgress and Assigned(FProgress) then
        FProgress.Hide;
      FLog.Add(E.Message);
      aRow := FParser.Row;
      aCol := FParser.Col;
