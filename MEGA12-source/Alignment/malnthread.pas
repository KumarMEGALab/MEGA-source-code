  ...
  try
    try
     ...
     begin
       FCanceled := true;
       {$IFDEF DEBUG}
         ErrMessage := 'Error in ClustalThread.Execute: ' + E.Message;
         Synchronize(DoOnFatalError);
       {$ENDIF}
     end;
    end;
  finally
...
procedure TClustalWThread.DoOnFatalError;
begin
  {$IFDEF VISUAL_BUILD}
...
begin
  ...
  if not FProgressDlg.HasProgressDelegate then
    FProgressDlg.Show;
  {$ENDIF}
end;
...
{$IFDEF VISUAL_BUILD}
  if FProgressDlg.PairwiseProgressBar.Position < FProgressDlg.PairwiseProgressBar.Max then
    FProgressDlg.StepPairwiseProgress;
{$ELSE}
...
{$IFDEF VISUAL_BUILD}
  if FProgressDlg.MultipleProgressBar.Position < FProgressDlg.MultipleProgressBar.Max then
    FProgressDlg.StepMultipleProgress;
{$ELSE}
  ShowProgressIncrementStatic;
{$ENDIF}
end;
...
//    wind_gap      := prot_wind_gap;
  ...
  begin
    ...
    begin
      if i = -1 then
      begin
        ErrMessage := 'Internal error occurred in Clustal. Alignment aborted.';
        Synchronize(DoOnFatalError);
      end;
      exit;
    end;
