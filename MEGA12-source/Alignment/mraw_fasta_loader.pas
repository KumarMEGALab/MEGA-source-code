...
begin
  ...
  TrimTaxaName(aName);
  aInfo := Trim(aInfo);
  Result := (aName <> EmptyStr);
  if (aName = EmptyStr) then
    Raise Exception.Create('Fasta parse error.  Expected a aName to follow ''>'', but none found');
  ...
  begin
    ...
    SetLength(FInfo, FNoOfTaxa + MEM_INCREMENT);
  end;
  GetMem(PName, (sizeof(AnsiChar)*length(aName))+1);
  StrCopy(PName, PAnsiChar(aName));
  FTaxa[FNoOfTaxa-1] := PName;

  if aInfo <> EmptyStr then
  begin
    GetMem(PInfo, (SizeOf(AnsiChar)*Length(aInfo)) + 1);
    StrCopy(PInfo, PAnsiChar(aInfo));
    FInfo[FNoOfTaxa - 1] := PInfo;
  end;
end;
    ...
    try
      ...
      if FNoOfTaxa > 0 then
        ...
        begin
          ...
          if Assigned(FInfo[i]) then
            aSeq.SeqInfo := FInfo[i];
          FSeqs.Add(aSeq);
        end;
      CheckIfLooksLikeDna;
