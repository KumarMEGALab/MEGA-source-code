...
uses
  ...
  SysUtils, MegaUtils_NV, MRuntimeProgressDlg, MTreePack, MAnalysisSummary,
  Controls, MTreeViewForm, MFileUtils, Forms, MegaUtils, KeywordConsts;
...
begin
  ...
  begin
    if (AnalysisInfo.MyProcessPack = ProcessPack) then
...
//  try
//    try
//      ATreeViewer := TTreeViewForm.Create(MegaForm, True);
//      ATreeViewer.JustRootingAATree := True;
//      ATreeViewer.Hide;
...
//      ATreeViewer.JustRootingATree := False; { so the temp info will get freed}
//    except
//      On E: Exception do
...
begin
  ...
  try
    try
      ...
      ATreeExplorer := TTreeViewForm.Create(Owner, True); { cannot use the existing TreeViewForm because it will be closed after ShowModal }
      ATreeExplorer.JustRootingATree := True; { necessary so that the launchgenedupsaction does not get disabled in SetData}
...
procedure TGeneDuplicationsWizard.UpdateAnalysisSummary;
var
  s: TAnalysisSummary = nil;
begin
  {$IFDEF VISUAL_BUILD}
  s := AnalysisInfo.AnalysisSummary;
  {$ELSE}
  s := D_MegaMain.AnalysisSummary;
  {$ENDIF}
  s.AnalysisName := OperationTypeString(dtdoGeneDupInference);
  s.NumTaxa := FGeneTreeList[0].NoOfOTUs;
  s.DataType := snTree;
  s.AnalysisOptions.Add(Format('Gene Tree is Rooted=%s', [BoolToStr(FGeneTreeIsRooted, True)]));
  s.AnalysisOptions.Add(Format('Using Species Tree=%s', [BoolToStr(UsingSpeciesTree, True)]));
end;
