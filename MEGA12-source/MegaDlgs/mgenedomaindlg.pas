...
uses
  ...
  MegaConsts, mimageform, mdomaininfo_search;
...
type
  ...
  TGeneDomainDlg = class(TForm)
    ...
    SearchActions: TActionList;
    SearchEdit: TEdit;
    HelpAction: TAction;
    Label2: TLabel;
    NumDomainsLabel: TLabel;
    OkAction: TAction;
    ...
    CheckBoxImages32: TImageList;
    GridImages64: TImageList;
    EditBtnImage100: TImageList;
    Label1: TLabel;
    Panel2: TPanel;
    Panel3: TPanel;
    SiteNumEdit: TEdit;
  ...
  private
    FDomainInfoSearch: TDomainInfoSearch;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(FDomainInfoSearch) and FDomainInfoSearch.IsMatch(aRow - 1) and ((aCol = GENE_NAME_INDEX) or (aCol = DOMAIN_NAME_INDEX)) then
...
procedure TGeneDomainDlg.FindFirstActionExecute(Sender: TObject);
begin
  if Trim(SearchEdit.Text) = EmptyStr then
  begin
    SearchResultsLabel.Caption := 'No search query provided';
    Exit;
  end;
  ClearSearchResults;
  FDomainInfoSearch := TDomainInfoSearch.Create(FGenesAndDomains, SearchEdit.Text);
  if FDomainInfoSearch.FindFirst <> nil then
    UpdateSearchResults
  else
  begin
    ClearSearchResults;
    SearchResultsLabel.Caption := 'No matches found';
  end;
end;

procedure TGeneDomainDlg.FindNextActionExecute(Sender: TObject);
begin
  if Trim(SearchEdit.Text) = EmptyStr then
  begin
    SearchResultsLabel.Caption := 'No search query provided';
  ...
  if not Assigned(FDomainInfoSearch) then
    FindFirstActionExecute(Sender)
  else
  begin
    if FDomainInfoSearch.FindNext <> nil then
      UpdateSearchResults
    else
      SearchResultsLabel.Caption := 'No matches found';
  end;
...
procedure TGeneDomainDlg.FindPreviousActionExecute(Sender: TObject);
begin
  if Trim(SearchEdit.Text) = EmptyStr then
  begin
    SearchResultsLabel.Caption := 'No search query provided';
  ...
  begin
    FDomainInfoSearch := TDomainInfoSearch.Create(FGenesAndDomains, SearchEdit.Text);
    if FDomainInfoSearch.FindLast <> nil then
      UpdateSearchResults
    else
    begin
      ClearSearchResults;
  ...
  begin
    if FDomainInfoSearch.FindPrevious <> nil then
      UpdateSearchResults
    else
      SearchResultsLabel.Caption := 'No matches found';
  end;
...
begin
  ...
  if Panel1.Width > ToolBar2.Width then
    ToolBar2.Left := Round((Panel1.Width - ToolBar2.Width)/2);
...
begin
  ...
  if Assigned(FColumnSizer) then
    FColumnSizer.Free;
  if Assigned(FDomainInfoSearch) then
    FDomainInfoSearch.Free;
  ClearGridImages;
end;
...
procedure TGeneDomainDlg.SearchEditChange(Sender: TObject);
begin
  ClearSearchResults;
end;

procedure TGeneDomainDlg.SearchEditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TGeneDomainDlg.ClearSearchResults;
begin
  if Assigned(FDomainInfoSearch) then
  begin
    FreeAndNil(FDomainInfoSearch);
    GenesDomainsDrawGrid.Invalidate;
  end;
  SearchResultsLabel.Caption := EmptyStr;
end;

procedure TGeneDomainDlg.UpdateSearchResults;
begin
  if not Assigned(FDomainInfoSearch) then
    SearchResultsLabel.Caption := EmptyStr
  else
  begin
    SearchResultsLabel.Caption := Format('match %d of %d', [FDomainInfoSearch.CurrentMatchNumber, FDomainInfoSearch.NumMatches]);
    GenesDomainsDrawGrid.Row := FDomainInfoSearch.CurrentMatchRow + 1;
    GenesDomainsDrawGrid.Invalidate;
  end;
end;

procedure TGeneDomainDlg.UpdateSummaryLabel;
begin
  if Assigned(FGenesAndDomains) and (FGenesAndDomains.Count > 0) then
  begin
    if FGenesAndDomains.Count = 1 then
      NumDomainsLabel.Caption := '1 gene/domain'
    else
      NumDomainsLabel.Caption := Format('%d genes/domains', [FGenesAndDomains.Count]);
  end;
end;
...
begin
  ...
  aInfo.Free;
  D_InputSeqData.FDomainMarks.IsDirty := True;
  i := D_InputSeqData.DomainMarks.IndependentCount; // this forces IndependentCount to be updated in DomainMarks
...
procedure TGeneDomainDlg.UpdateGridImages;
var
...
begin
  ...
  begin
    FDomainCheckedImg.SetSize(32, 64);
    FDomainPartialImg.SetSize(32, 64);
    FDomainUnCheckedImg.SetSize(32, 64);
    FGeneCheckedImg.SetSize(32, 64);
    FGenePartialImg.SetSize(32, 64);
    FGeneUnCheckedImg.SetSize(32, 64);
    FEditBtnImg.SetSize(100, 36);
    GridImages64.GetBitmap(0, FDomainCheckedImg);
    GridImages64.GetBitmap(1, FDomainPartialImg);
    GridImages64.GetBitmap(2, FDomainUncheckedImg);
    GridImages64.GetBitmap(3, FGeneCheckedImg);
    GridImages64.GetBitmap(4, FGenePartialImg);
    GridImages64.GetBitmap(5, FGeneUncheckedImg);
    EditBtnImage100.GetBitmap(0, FEditBtnImg);

    FCheckedImg.SetSize(32, 32);
    FUncheckedImg.SetSize(32, 32);
    FPartialCheckedImg.SetSize(32, 32);
    CheckboxImages32.GetBitmap(0, FCheckedImg);
    CheckboxImages32.GetBitmap(1, FUncheckedImg);
    CheckboxImages32.GetBitmap(2, FPartialCheckedImg);
  end
  else
  begin
...
begin
  ...
  SiteNumEdit.Text := Format('Site %.0n', [(NextSite + 1)*1.0]);
