...
uses
  ...
  StdCtrls, Spin, Buttons, IniPropStorage, ActnList, ComCtrls,
  mseqexportoptions, mimageform;
...
type
  ...
  TSeqDataExportOptionsDlg = class(TForm)
    KeepFullCodonsCbox: TCheckBox;
    InterleavedChkBx: TCheckBox;
    Label5: TLabel;
    IncludeCodonPositionsLabel: TLabel;
  ...
  private
    ...
    procedure SetIsChooseBases(AValue: Boolean);
  ...
  public
    ...
    property IsChooseBases: Boolean read FIsChooseBases write SetIsChooseBases;
...
procedure TSeqDataExportOptionsDlg.FormResize(Sender: TObject);
begin
  if Panel3.Width > ToolBar1.Width then
    ToolBar1.Left := Round((Panel3.Width - ToolBar1.Width)/2);
end;
...
procedure TSeqDataExportOptionsDlg.FormActivate(Sender: TObject);
begin
...
begin
  ...
  KeepFullCodonsCbox.Enabled := Value;
end;

procedure TSeqDataExportOptionsDlg.SetIsChooseBases(AValue: Boolean);
begin
  FIsChooseBases := AValue;
  EnableCodPosSiteGpBx(AValue);
end;
...
begin
   ...
   Result.ExportFullCodons := (IsChooseBases and KeepFullCodonsCbox.Checked);
   ...
   Result.IsCodonByCodonWriting := (IsCodonByCodonWriting and (not Result.IsFASTAFormat));
