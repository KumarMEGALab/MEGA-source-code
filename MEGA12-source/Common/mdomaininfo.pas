...
type
  ...
  public
    ...
    function FullInfosStringList: TStringList;
    procedure Assign(Source: TAllDomainInfo);
...
begin
  Result := nil;
  if (Index < 0) or (Index >= FNoOfSites) then
    Exit;
  Result := FArray[Index];
end;
...
begin
  if FIsUsed then
    ...
    Result := 'inactive ';
  if FIsCoding then
    Result := Result + 'Coding; CodonStart = ' + IntToStr(FCodonStart+1) +';';
end;
...
begin
  ...
  if IsGene then
    ...
    Result := 'Domain: ';
  Result := Format('%s %-20s %20s %s', [Result, FName, GetSiteRangeString, GetInfoString]);
end;
...
procedure TAllDomainInfo.Clear;
var
  i: Integer = -1;
begin
  for i:= 0 to FArray.Count - 1 do
    if Assigned(FArray[i]) then
      TDomainInfo(FArray[i]).Free;
  FArray.Clear;
end;
...
procedure TAllDomainInfo.UnselectAll;
var
  i: Integer = -1;
begin
  if FArray.Count > 0 then
    ...
    begin
      if Assigned(FArray[i]) then
        TDomainInfo(FArray[i]).IsUsed := False;
    end;
end;
...
function TAllDomainInfo.SummaryString: String;
var
  i: Integer = 0;
  d: Integer = 0;
  g: Integer = 0;
...
begin
  if FArray.Count > 0 then
    ...
    begin
      inc(d);
      aInfo := TDomainInfo(FArray[i]);
      if Assigned(aInfo) and aInfo.IsGene then
        inc(g);
    end;
  if (d <> 1) and (g <> 1) then
    Result := Format('%d genes and %d domains defined', [g, d])
  else if (d = 1) and (g = 1) then
    Result := '1 gene and 1 domain defined'
  else if d = 1 then
    Result := Format('%d genes and 1 domain defined', [g])
  else if g = 1 then
    Result := Format('1 gene and %d domains defined', [d])
end;

function TAllDomainInfo.FullInfosStringList: TStringList;
var
  i: Integer = -1;
  aInfo: TDomainInfo = nil;
begin
  ...
  if FArray.Count > 0 then
    ...
    begin
      ...
      begin
        aInfo := TDomainInfo(FArray[i]);
        Result.Add(aInfo.FullInfoString);
      end
      else
        Result.Add(Format('domain info #%d is nil', [i + 1]));
    end;
end;

procedure TAllDomainInfo.Assign(Source: TAllDomainInfo);
var
  i: Integer = -1;
  aInfo: TDomainInfo = nil;
begin
  Clear;
  if Source.NoOfDomains > 0 then
    for i := 0 to Source.NoOfDomains - 1 do
    begin
