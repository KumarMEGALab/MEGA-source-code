...
type
  ...
  public
    ...
    procedure AddValues(values: TLongintList);
    procedure Delete(Index: Longint);
    function FindFirst(aValue: LongInt): Integer;
    function  First: Longint;
    function  Last: Longint;
    function Find(aValue: LongInt): Integer;
    function HasSameValues(otherList: TLongintList): Boolean;
    function HasValue(aValue: LongInt): Boolean;
...
begin
  inherited Insert(Index, TMegaLongInt.Create(Item));
end;

procedure TLongintList.AddValues(values: TLongintList);
var
  i: Integer = -1;
begin
...
function TLongintList.FindFirst(aValue: LongInt): Integer;
var
  i: Integer = -1;
begin
  Result := -1;
...
function TLongintList.HasSameValues(otherList: TLongintList): Boolean;
var
  i: Integer = -1;
begin
  if otherList.Count = 0 then
    raise Exception.Create('Application Error: comparing empty list');
  Result := False;
  if otherList.Count <> Self.Count then
    Exit;
  for i := 0 to otherList.Count - 1 do
    if Find(otherList[i]) = -1 then
      Exit;
  Result := True;
end;

function TLongintList.HasValue(aValue: LongInt): Boolean;
begin
  if Find(aValue) >= 0 then
    Result := True
  else
    Result := False;
end;

function TLongintList.ToCsvString: String;
var
  i: Integer = -1;
begin
  Result := EmptyStr;
  if Count > 0 then
    for i := 0 to Count - 1 do
      Result := Result + Format('%d,', [Get(i)]);
end;

function TLongintList.MaxValue: LongInt;
var
  i: Integer = -1;
begin
  Result := -1;
...
function TLongintList.ToArray: TIntArray;
var
  i: Integer = -1;
begin
