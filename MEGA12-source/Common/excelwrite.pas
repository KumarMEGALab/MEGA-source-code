...
type
  ...
  TExcelWrite = class
    ...
    public
      ...
      procedure WriteLine(Worksheet: Integer = 0; Start: AnsiChar = 'A'; aFormat: String = '';
        MergeRow: Boolean = False); overload;
...
begin
  Exit(True); { assume that everyone has some application to read excel files as the registry check above has been known to fail somehow}
  //Result := False;
  //{$IFDEF MSWINDOWS}
...
begin
  case FileFormat of
    ...
    ExportMega: Result := GetSaveLocation(EXmega);
    ExportFasta: Result := GetSaveLocation(EXfasta);
    ExportNone, ExportInvalid: Result := GetSaveLocation(EXnone);
    else
...
procedure TExcelWrite.WriteLine(Worksheet: Integer = 0; Start: AnsiChar = 'A';
  aFormat: String = ''; MergeRow: Boolean = False); // Main output method.  This writes the array to which we have been adding elements out to the next line of Excel
 var
