...
function TGeographicalInfo.HasAnyInfo: Boolean;
begin
  if FGroup <> EmptyStr then Exit(True);
  if FPopulation <> EmptyStr then Exit(True);
  if FSpecies <> EmptyStr then Exit(True);
  if FYear <> -1 then Exit(True);
  if FMonth <> NoMonth then Exit(True);
  if FDay <> 0 then Exit(True);
  if FContinent <> EmptyStr then Exit(True);
  if FCountry <> EmptyStr then Exit(True);
  if FCity <> EmptyStr then Exit(True);
  if CompareValue(FReferenceTime, 0.0, FP_CUTOFF) <> 0 then Exit(True);
  Result := False;
end;
