...
type
  ...
  public
    ...
    function XCoordInView(xCoord: Integer): Boolean;
    function YCoordInView(yCoord: Integer): Boolean;
  ...
  public
    ...
    procedure ScrollToYCoord(newY: Integer);
...
function TVirtualScrollBox.XCoordInView(xCoord: Integer): Boolean;
begin
  Result := (xCoord >= FScrollLeft) and (xCoord <= (FScrollLeft + Width));
end;

function TVirtualScrollBox.YCoordInView(yCoord: Integer): Boolean;
begin
  Result := (yCoord >= FScrollTop) and (yCoord <= (FScrollTop + Height));
end;
...
begin
  try
    FBitmap.PixelFormat := GetOptimalPixelFormat(ARect);
    FBitmap.SetSize(ARect.Right, ARect.Bottom);
    FBitmap.Canvas.CopyRect(ARect, aCanvas, aRect);
    Invalidate;
  Except
    on E: EOutOfResources do
      ShowMessage('Application limits exceeded! The tree is too large to be drawn as formatted: ' + PixelFormatToString(FBitmap.PixelFormat));
    on E:Exception do
      ShowMessage('Application error! An error occurred when loading the drawing canvas: ' + E.Message);
  end;
end;
...
begin
  ...
  if (DeltaX = 0) and (DeltaY = 0) then
    exit;
  FScrollLeft := min(NewX, FScrollWidth);
  FScrollTop  := min(newY, FScrollHeight);
  // Scroll the window
  ScrollWindow(Handle, -DeltaX, -DeltaY, nil, nil);
...
procedure TEmfScrollbox.ScrollToYCoord(newY: Integer);
begin
  ScrollTo(FScrollLeft, newY);
end;
