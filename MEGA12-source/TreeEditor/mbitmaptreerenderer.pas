...
type
  ...
  TBitmapTreeRenderer = class(TAbstractTreeRenderer)
    private
...
constructor TBitmapTreeRenderer.Create(aCanvas: TCanvas);
begin
  inherited Create(aCanvas);

end;

procedure TBitmapTreeRenderer.MTextOut(const x: Integer; const y: Integer; const str: String; const rotation: Integer = 0);
begin
  FCanvas.TextOut(x, y, str);
end;

procedure TBitmapTreeRenderer.MPolyline(const p: array of TPoint);
begin
  FCanvas.Polyline(p);
end;

procedure TBitmapTreeRenderer.MPolygon(const p: array of TPoint);
begin
  FCanvas.Polygon(p);
end;

procedure TBitmapTreeRenderer.MRectangle(const p: array of TPoint; const Filled: Boolean);
begin
  MPolygon(p);
end;

procedure TBitmapTreeRenderer.MRoundRectangle(const aRect: TRect; const Filled: Boolean; const rx, ry: Integer);
begin
  if Filled then
    FCanvas.Brush.Style := bsSolid
  else
    FCanvas.Brush.Style := bsClear;
  FCanvas.RoundRect(aRect, rx, ry);
end;

procedure TBitmapTreeRenderer.MPolyBezier(const p: array of TPoint; const n: Integer; Filled: Boolean; Continuous: Boolean);
begin
  FCanvas.PolyBezier(p, n, Filled, Continuous);
end;

procedure TBitmapTreeRenderer.MDrawImage(const x: Integer; const y: Integer; const aBitmap: TBitmap);
begin
  FCanvas.Draw(x, y, aBitmap);
end;

procedure TBitmapTreeRenderer.MDrawArc(const aLeft: Integer;
...
procedure TBitmapTreeRenderer.MDrawEllipse(const aRect: TRect);
begin
  FCanvas.Ellipse(aRect);
end;

procedure TBitmapTreeRenderer.MDrawSquare(const x: Integer; const y: Integer; const r: Integer; const a: Double; const fill: Boolean);
begin
