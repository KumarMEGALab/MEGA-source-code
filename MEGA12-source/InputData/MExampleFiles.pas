...
type
  ...
  TExampleFiles = class(TObject)
    ...
    public
      ...
      {$IFDEF DEBUG}
      procedure UpdateRecentlyUsedFilesList;
      {$ENDIF}
...
uses
  ...
  MegaPrivateFiles,
  {$ENDIF}
  MegaUtils, Dialogs, Forms, FileUtil, MegaVerConsts;

function GetExampleFilesFolderName: String;
begin
  {$IFDEF DEBUG}
    {$IFDEF DARWIN}
     Result := GetHomeDirectory + 'MEGA' + IntToStr(MAJOR_VERSION) + PathDelim + 'Examples';
    {$ELSE}
    Result := ExtractFileDir(Application.ExeName) + PathDelim + '..' + PathDelim + 'MEGA7_Install' + PathDelim + 'Private' + PathDelim + 'Examples';
    Result := ExpandFileName(Result);
    {$ENDIF}
  {$ELSE}
  Result := GetHomeDirectory + 'MEGA_' + IntToStr(MAJOR_VERSION) + PathDelim + 'Examples';
  {$ENDIF}
end;
...
{$IFDEF DEBUG}
procedure TExampleFiles.UpdateRecentlyUsedFilesList;
var
  aList: TStringList = nil;
  filename: String = '';
  i: Integer = -1;

  procedure AddIfExists(aFile: String);
  begin
    if not FileExists(aFile) then
      Exit;
    if aList.IndexOf(aFile) < 0 then
      aList.Add(aFile);
  end;
...
begin
  try
    try
      filename := GetPrivateFile(mfMegaMainMruFiles, False);
      aList := TStringList.Create;
      if FileExists(filename) then
        aList.LoadFromFile(filename);
      if ExampleFilesList.Count > 0 then
        for i := 0 to ExampleFilesList.Count - 1 do
        begin
          if ExtractFileExt(ExampleFilesList[i]) <> '.meg' then
            continue;
          if aList.IndexOf(SystemExampleFilename(ExampleFilesList[i])) < 0 then
            aList.Add(SystemExampleFilename(ExampleFilesList[i]));
        end;
      AddIfExists('C:\Users\gstecher\Documents\iGEM\MEGA\testData\DNA\aligned_rubisco.meg');
      AddIfExists('C:\Users\gstecher\Documents\iGEM\MEGA\testData\AA\3x2048_AA.meg');
      aList.SaveToFile(filename);
    except
      on E:Exception do
        ShowMessage('DEBUG error when updating the recently used files list: ' + E.Message);
    end;
  finally
