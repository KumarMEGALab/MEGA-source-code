...
uses
  ...
  StdCtrls, Dialogs, ExtCtrls, MTreeBox;
...
uses
  MegaConsts, MAlignGrid, MTraceEdit, MClustalW;
  ...
  try
    try
      ...
      begin
        ...
        begin
          if (aForm.Components[i].ClassName='TAlignGrid') or
             (aForm.Components[i].ClassName='TClustalW') or
             (aForm.Components[i].ClassName='TTraceEdit') then
            begin
    ...
    try
      ...
      on  E:Exception do
        if not IsSessionTest then
          ShowMessage('Application Error: failed to write components to stream: ' + E.Message);
    end;
  finally
    if Assigned(aStream) then
      aStream.Free;
