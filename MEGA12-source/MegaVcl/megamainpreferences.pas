...
function  TryToGetPreferencesMainMenu(caption: String; var che: Boolean; var option: String): Boolean;
procedure LoadMainMenuPreferencesOnCreate(AIniFileName: String);
procedure SaveMainMenuPreferencesOnDestroy;
procedure SaveDataCollectPrefs(IsAllowed: Boolean);
procedure SaveUserType(userType: String);
...
var
  ...
  checked: TStringList = nil;
  menuToAdd: TMenuItem = nil;
  ABool: Boolean = False;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        {$IFDEF VISUAL_BUILD}
        if menuToAdd.Caption = UserPref_AllowCollectUsageData then
          menuToAdd.OnClick := @MegaForm.DataCollectPrefsItemClicked
        else
...
begin
  ...
  begin
    ...
    if Pos('&', ButtonCaption) > 0 then
      Delete(ButtonCaption, pos('&', ButtonCaption), 1);
...
function TryToGetPreferencesMainMenu(caption: String; var che: Boolean; var option: String): Boolean; // returns true if the caption is in the options list.
var
  foundMenuItem: TMenuItem;
begin
  try
    foundMenuItem := PrefMenu.Items.Find(caption);
    ...
    begin
      ...
      option := FDoNotPromptSettings.ReadString('settings', caption, option);
    end;
  except
    on E: Exception do
    begin
...
begin
  try
    try
      PrefsFileString := GetMegaGlobalFile('') + DATA_COLLECT_PREFS_FILE;
...
procedure SaveUserType(userType: String);
var
  userTypeFile: TextFile;
  userTypeFilePath: String;
begin
  try
    try
      userTypeFilePath := GetMegaGlobalFile('') + USER_TYPE_FILE;
      AssignFile(userTypeFile, userTypeFilePath);
      Rewrite(userTypeFile);
      WriteLn(userTypeFile, userType);
    except
      // eat the exception, we don't bother users with data collection problems
    end;
  finally
    CloseFile(userTypeFile);
  end;
end;
...
begin
  ...
  try
    try
      userTypeFilePath := GetMegaGlobalFile('') + USER_TYPE_FILE;
      if FileExists(userTypeFilePath) then
      begin
        AssignFile(userTypeFile, userTypeFilePath);
        Reset(userTypeFile);
        ReadLn(userTypeFile, userType);
        if userType.StartsWith('Other') then
        begin
          tokens := userType.Split(['=']);
          if length(tokens) = 2 then
            USER_TYPE := Trim(tokens[1]) // for the case of 'Other', the file contents will be something like 'Other = SuperCoolUser'.
          else
            USER_TYPE := OTHER_STR // failsafe to at least acknowledge that 'Other' was selected
        end
        else
          USER_TYPE := userType;
      end
