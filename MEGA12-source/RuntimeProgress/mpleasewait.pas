...
uses
 ...
 {$ENDIF}
 SysUtils, Classes, MegaConsts, Forms;
...
constructor TPleaseWait.CreateWithoutGui;
begin
  {$IFDEF VISUAL_BUILD}
  PleaseWaitDlg := nil;
  {$ENDIF}
...
begin
  {$IFDEF VISUAL_BUILD}
  if not Assigned(PleaseWaitDlg) then
    Exit;
...
procedure TPleaseWait.SetPosition(aPos: TPosition);
begin
{$IFDEF VISUAL_BUILD}
  if Assigned(PleaseWaitDlg) then
    PleaseWaitDlg.Position := aPos;
{$ENDIF}
end;
