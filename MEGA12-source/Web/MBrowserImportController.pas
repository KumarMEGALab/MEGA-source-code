...
uses
  ...
  Dialogs, FileUtil, Forms, Graphics, MegaPrivateFiles, MegaConsts,
  htmloptionsdlg, mbrowserutils, AlnBuilder_HC,
...
begin
  ...
  begin
    ...
    if SeqInfoString.isEmpty then
      ...
      FSequence.SeqInfo := SeqInfoString;
...
begin
  ...
  if AlignEditMainForm.SourceAlignGrid.Empty then // tell AlignEditMainForm what type of data to expect
  begin
  ...
  begin
    {$IFNDEF DARWIN}
    if (FDataType = snProtein) and AlignEditMainForm.SourceAlignGrid.IsDNA then
    begin
    ...
    else if (FDataType = snNucleotide) and not AlignEditMainForm.SourceAlignGrid.IsDNA then
  ...
  Assert((FSequenceList <> nil) and (FSequenceList.Count > 0));
  try
    AlignEditMainForm.SourceAlignGrid.SkipHeaderUpdates := True;
    for i := 0 to FSequenceList.Count - 1 do
      AlignEditMainForm.SourceAlignGrid.AppendSequence(FSequenceList[i]);
  finally
    AlignEditMainForm.SourceAlignGrid.SkipHeaderUpdates := False;
  end;
