...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Buttons, ActnList, ComCtrls,
  MD_Sequences;

type

  { TSeqNameOptionDlgForm }
...
uses
  MAlignEditMainForm, MAlignGrid, MegaVerConsts, mimageform;
...
procedure TSeqNameOptionDlgForm.CancelActionExecute(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TSeqNameOptionDlgForm.FormActivate(Sender: TObject);
begin
...
procedure TSeqNameOptionDlgForm.FormResize(Sender: TObject);
begin
  if ClientWidth > ToolBar1.Width then
    ToolBar1.Left := Round((ClientWidth - ToolBar1.Width)/2);
end;
...
begin
  ...
  if result and (AlignEditMainForm <> nil) then
    for i := 0 to AlignEditMainForm.SourceAlignGrid.NoOfSeqs-1 do
      if str = AlignEditMainForm.SourceAlignGrid.Sequence[i].SeqName then
      begin
        result := false;
        break;
      end;
