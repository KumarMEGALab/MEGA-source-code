...
type
  ...
  TStyleSheet = class(TObject)
    private
      FFontSizesScaled: Boolean;
      FbodyFontSize: Integer;
      FMaxBodyFontSize: Integer;
      FStyles: TList;
      FFontName: String;
      FReferenceFontSize: Integer;
      FDisclaimerFontSize: Integer;
      FFigureTitleFontSize: Integer;
      FNoteFontSize: Integer;
      procedure SetDisclaimerFontSize(AValue: Integer);
    ...
    public
      ...
      procedure ScaleUpFontSizes(scalingFactor: Double);
      ...
      property DisclaimerFontSize: Integer read FDisclaimerFontSize write SetDisclaimerFontSize;
      ...
      property MaxBodyFontSize: Integer read FMaxBodyFontSize write FMaxBodyFontSize;
...
begin
  FFontSizesScaled := False;
  FStyles := TList.Create;
  FMaxBodyFontSize := 200;
  FbodyFontSize := 12;
  FNoteFontSize := 14;
  FFigureTitleFontSize := 16;
  FReferenceFontSize := 12;
  FDisclaimerFontSize := 10;
...
begin
  ...
  dec(FDisclaimerFontSize);
  RefreshStyleSheet;
end;

procedure TStyleSheet.ScaleUpFontSizes(scalingFactor: Double);
var
  fs: Integer = 0;
begin
  if CompareValue(scalingFactor, 1, FP_CUTOFF) <= 0 then
    Exit;
  if FFontSizesScaled then
    Exit;
  FFontSizesScaled := True;
  fs := Round(FbodyFontSize*scalingFactor);
  if fs > FMaxBodyFontSize then
    fs := FMaxBodyFontSize;

  while FBodyFontSize < fs do
  begin
    inc(FbodyFontSize);
    inc(FReferenceFontSize);
    inc(FNoteFontSize);
    inc(FFigureTitleFontSize);
    inc(FDisclaimerFontSize);
  end;
  RefreshStyleSheet;
end;
...
function TStyleSheet.GetStyleSheetString: String;
var
...
begin
  if FBodyFontSize = FMaxBodyFontSize then
    Exit;
  inc(FbodyFontSize);
  inc(FReferenceFontSize);
  inc(FNoteFontSize);
...
begin
  ...
  aStyle := TStyle.Create('disclaimer', stId);
  aStyle.Add('font-size: ' + IntToStr(FDisclaimerFontSize) + 'px;');
  aStyle.Add('margin-top: 15px;');
  //aStyle.Add('text-indent: -1em;');
  //aStyle.Add('padding-left: 1em;');
  FStyles.Add(aStyle);
...
procedure TStyleSheet.SetDisclaimerFontSize(AValue: Integer);
begin
  FDisclaimerFontSize := AValue;
end;
