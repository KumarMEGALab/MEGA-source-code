...
type
  ...
  public
    ...
    SearchStr: String;
    property NoOfBases: Integer read GetNoOfBases;
  end;
...
uses
  MAlignEditMainForm, uMegaBrowser, MAlignGrid, mhelpfiles, mhelpkeywords,
  MMegaWindowInfo, AlnBuilder_HC, MegaVerConsts, mega_main,
  MD_Sequences, LCLProc, mshortcutshelper, MegaConsts;
...
begin
  Result := 'TraceEditor (' + ExtractFileName(filename) + ')';
end;
...
begin
  ...
  {$IFDEF LINUX}
  HorzTrackBar.Height := ToolBar1.ButtonHeight;
  HorzTrackBar.ScalePos := trLeft;
  HorzTrackBar.ShowSelRange := False;
  HorzTrackBar.TickStyle := tsNone;
  VertTrackBar.TickStyle := tsNone;
  {$ENDIF}
end;

procedure TTraceEditForm.FormActivate(Sender: TObject);
begin
...
begin
  ...
  Caption := 'TraceEdit (' + ExtractFileName(SaveDlg.FileName) + ')';
end;
...
function TTraceEditForm.GetNoOfBases: Integer;
begin
  if Assigned(TraceEdit) and Assigned(TraceEdit.Data) then
    Result := TraceEdit.Data.NoOfBases
  else
    Result := 0;
end;
...
begin
  ...
  begin
    ...
    if (not SourceAlignGrid.IsDNA) and
    ...
    try
      SourceAlignGrid.SkipHeaderUpdates := True;
      SourceAlignGrid.AppendSequence(seq);
    finally
      SourceAlignGrid.SkipHeaderUpdates := False;
    end;
    InitForm;
...
begin
  ...
  if TraceEdit.SelLength > 1 then
    aChBrowser.BLAST(TraceEdit.SelectedSequence, true)
  else
    aChBrowser.BLAST(TraceEdit.Data.UnmaskedBases, true);
end;
