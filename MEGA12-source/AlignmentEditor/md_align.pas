...
uses
  ...
  RegExpr, MAnalysisInfo, Dialogs, Forms, Controls, MUsageStatistics, fpjson;
...
type
  ...
  TD_Align = class(TObject)
  private
    FAlignCodons: Boolean;
...
//                  ADataType, ADataFormat: TSnTokenCode; AMissSym, AIdenSym, AGapSym: AnsiChar): Boolean;
    ...
    procedure UpdateUsageStatistics(settings: TJSONObject);
    procedure WorkflowMuscleThreadFinished(aThread: TObject);
  public
    MySeqList : TSequenceList;
    ...
    function GetUsageDataAsJsonString(settings: TJSONObject): String;
  published
...
function MuscleOptionsToJson(aOptions: TStringList): TJSONObject;
...
uses
  ...
  {$ENDIF}
  MAlnThread, MDistPack, ProcessInputData, MFormatConvertToMega, mmemutils,
  MRuntimeProgressDlg, MegaAnalysisPrefStrings,
  DateUtils;

procedure SetGeneticCode(GeneticCodeTable : AnsiString);
begin
  D_Align.MySeqList.CodeTable := GeneticCodeTable;
end;
...
begin
  ...
  try
    ...
    else if MySeqList.Count > 0 then
    begin
      WriteMEGAFile(MySeqList, filename);
      Result := FileExists(filename);
    end;
  finally
...
begin
  ...
  NumRefSeqs := D_Align.MySeqList.Count;
  ...
  TargetSequence.SeqData := D_Align.MySeqList[D_Align.MySeqList.Count - 1].SeqData;
  TargetSequence.SeqName := D_Align.MySeqList[D_Align.MySeqList.Count - 1].SeqName;
  ...
  for i := 0 to D_Align.MySeqList.Count - 2 do
  begin
    AlignedSequences.Items[2*i].SeqData := TargetSequence.SeqData;
    AlignedSequences.Items[2*i].SeqName := TargetSequence.SeqName;
    AlignedSequences.Items[(2*i) + 1].SeqData := D_Align.MySeqList.Items[i].SeqData;
    AlignedSequences.Items[(2*i) + 1].SeqName := D_Align.MySeqList.Items[i].SeqName;
...
begin
  ...
  TempSequences.Assign(D_Align.MySeqList);
  AlignedSequences := TSequenceList.Create;
  TargetSequence := TSequence.Create;
  ...
  D_Align.MySeqList.Clear;
  D_Align.MySeqList.add(TargetSequence);
  D_Align.MySeqList.Add(TempSequences.Items[0]);
  ShowRunStatusInfoStatic('status', 'Performing pairwise alignments using MUSCLE');
  for i := 0 to TempSequences.Count - 1 do
  begin
    D_Align.MySeqList.Items[0] := TargetSequence;
    D_Align.MySeqList.Items[1] := TempSequences.Items[i];
    ...
    AlignedSequences.Items[2*i].SeqData := D_Align.MySeqList.Items[0].SeqData;
    AlignedSequences.Items[2*i].SeqName := D_Align.MySeqList.Items[0].SeqName;
    AlignedSequences.Items[(2*i) + 1].SeqData := D_Align.MySeqList.Items[1].SeqData;
    AlignedSequences.Items[(2*i) + 1].SeqName := D_Align.MySeqList.Items[1].SeqName;
  end;
  ShowRunStatusInfoStatic('status', ' ');
  Result := TSequenceList.Create;
  Result.Assign(AlignedSequences);
...
begin
  try try
    ...
    FAlignCodons := MuscleAlignCodons;
    ...
    TempDir := GetPrivateOutputFile('Private' + PathDelim + 'temp' + PathDelim);
    ...
    {$IFNDEF VISUAL_BUILD}
    if (D_MegaMain.FDataType = snNucleotide) and (not MySeqList.IsDNA) then
      Error_nv('Tried to align protein sequence with nucleotide options.');
    if (D_MegaMain.FDataType = snProtein) and MySeqList.IsDNA and (not MySeqList.IsProteinCoding) then
    ...
    D_MegaMain.AnalysisSummary.NumTaxa := MySeqList.Count;
    for i := 0 to ProcessPack.TextualSettingsList.Count - 1 do
    ...
    MuscleLink.SetSequenceData(MySeqList);
...
var
  ...
  Row: Integer = -1;
  Col: Integer = -1;
  isCodonAlignment: Boolean = False;
  {$ENDIF}
  i: integer;
  SequenceList : TStringList;
  SequenceNames : TStringList;
  settingsJson: TJSONObject = nil;
begin
  if MySeqList.Count < 2 then
  begin
    Raise Exception.Create('At least two sequences need to be provided for alignment.');
  ...
  settingsJson := ClustalW.GetSettingsAsJson;
  {$IFNDEF VISUAL_BUILD}
  isCodonAlignment := ProcessPack.SettingValueIs(ContainsCodingNucStr, 'true');
  settingsJson.Add('Align by codons', BoolToStr(isCodonAlignment, True));
  {$ENDIF}
  try
    for i := 0 to MySeqList.Count-1 do
    begin
      SequenceList.Add(MySeqList[i].SeqData);
      SequenceNames.Add(MySeqList[i].SeqName);
    end;
    ClustalW.Sequences := SequenceList;
    ClustalW.SeqNames := SequenceNames;
    {$IFNDEF VISUAL_BUILD}
    if MegaUtils.CheckStopCodons(MySeqList, Row, Col) then
      warn_nv('Stop codon(s) are found in the translated sequences. Is the correct genetic code selected?');
    {$ENDIF}
    ClustalW.Align;
    {$IFNDEF VISUAL_BUILD}
    D_MegaMain.AnalysisSummary.WriteToFile(ChangeFileExt(NextAvailableFileNameNV('.txt'), '_summary.txt'), False);
    {$ENDIF}
    UpdateUsageStatistics(settingsJson);
    SequenceList.Free;
    SequenceNames.Free;
...
function TD_Align.GetUsageDataAsJsonString(settings: TJSONObject): String;
var
...
begin
  try
    ...
    dStr := MySeqList.DataTypeString;
    if dStr = 'protein' then
      dStr := 'amino acid'; // for consistency with usage stats because protein and amino acid are used ambiguously throughout MEGA
    generalInfo.Add(DATA_TYPE_STR, dStr);
    generalInfo.Add(NUM_TAXA_STR, MySeqList.Count);
    generalInfo.Add(NUM_SITES_STR, MySeqList.GetMaxNoOfSites);
    json.Add(GENERAL_INFO, generalInfo);
    json.Add(ANALYSIS_SETTINGS, settings);
    ...
    statsInfo.Add('Execution Time (seconds)', SecondSpan(FEndTime, FStartTime));
    {$IFNDEF LINUX}
    statsInfo.Add('Peak Memory Used(Working Set - MB)', GetPeakWorkingSetSizeMB);
    {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      MySeqList.CodeTable := ProcessPack.TextualSettingsList.Values['GeneticCodeTable'];
      MySeqList.Translate;
...
procedure TD_Align.UpdateUsageStatistics(settings: TJSONObject);
var
...
begin
  try
    try
      FEndTime := Now;
      FPeakMemory := GetPeakWorkingSetSizeMB;
      aJson := GetUsageDataAsJsonString(settings);
      usageStats := TStatisticsReporter.Create(True);
      usageStats.UsageStatistics := aJson;
      usageStats.Start;
      usageStats := nil;
      {$IFNDEF VISUAL_BUILD}
      if ExportUsageData then { ExportUsageData is used for the test harness}
      begin
        try
          filename := NextAvailableFilenameNV('_summary.json');
...
begin
  try
    ...
    aLink := t.MuscleLink;
    jsonOptions := MuscleOptionsToJson(aLink.MuscleOptions);
    jsonOptions.Add('Align by codons', BoolToStr(FAlignCodons, True));
    UpdateUsageStatistics(jsonOptions);
    jsonOptions := nil; // will be freed downstream
  except
    on E:Exception do
    begin
      if Assigned(t) then
...
begin
 ...
 for i := 0 to ClustalW.Sequences.Count-1 do    //Place the aligned sequences into the seqlist overwriting unaligned seqs
   MySeqList[i].SeqData := ClustalW.Sequences[i];
  if (D_MegaMain.OutputFormat = ExportFasta) then
    WriteFastaFile(MySeqList, NextAvailableFilenameNV('.fasta'))
  else
   WriteMEGAFile(MySeqList, NextAvailableFilenameNV('.meg'));    //Write the aligned sequences into a MEGA file
  try
...
{$IFNDEF VISUAL_BUILD}
   ...
   for i := 0 to ClustalW.Sequences.Count-1 do //Place the aligned sequences back into the seqlist overwriting unaligned seqs
     MySeqList[i].SeqData := ClustalW.Sequences[i];
   MySeqList.UnTranslate; //Untranslate the cDNA sequences
   if (D_MegaMain.OutputFormat = ExportFasta) then
     WriteFastaFile(MySeqList, NextAvailableFilenameNV('.fasta'))
   else
     WriteMEGAFile(MySeqList, NextAvailableFilenameNV('.meg')); //Write the untranslated aligned sequences into a MEGA file
   try
     NumActiveThreadsCS.Acquire;
     RunningThreadCount := RunningThreadCount - 1;
   finally
...
begin
  ...
  try
    ...
    MySeqList := D_InputSeqData.GenerateTSequenceList;
...
begin
  ...
  try
    ...
    MySeqList := TSequenceList.Create;
    ...
    begin
      ...
      if MySeqList.Count > 0 then
        for i := 0 to MySeqList.Count-1 do
          for j := 1 to MySeqList[i].NoOfSites do
            if Upcase(MySeqList[i].Base[j]) in DNAbases then
              inc(nn)
            else if Upcase(MySeqList[i].Base[j]) in AABases then
              inc(na);
...
//        MySeqList.IsProteinCoding := (FDataType = snCoding);
        {$IFNDEF VISUAL_BUILD}
        MySeqList.IsProteinCoding := D_MegaMain.FFileShouldContainCoding;
        {$ENDIF}
      end
      else
      begin
        ...
        MySeqList.IsProteinCoding := D_MegaMain.FFileShouldContainCoding;
        {$ELSE}
        MySeqList.isProteinCoding := isDNA;
        {$ENDIF}
      end;
      MySeqList.isDNA := isDNA;  // MySeqList wasn't having it's isDNA properly set, it always defaulted to true.
    ...
    if Assigned(Converter) then
      Converter.Free;
    {if Assigned(MySeqList) then  //we will use this
      MySeqList.Free; }
      ...
      case MuscleLink.ErrorCode of
         noError:
           begin
             ...
             seq := D_Align.MySeqList; // Might have to create the seq object first before assigning.
            ...
            if XSites <> EmptyStr then
             ...
             begin
               ...
               begin
                 D_Align.MySeqList[i].SeqData := ResultList.Strings[i];
                 D_Align.MySeqList[i].SeqName := NameOrder.Strings[i]; // GS - added for profile alignments so that the extra sequence name gets included
               end;
               ...
               D_Align.MySeqList.Add(PhyloQSequence);
             end
             else
             begin
               for I := 0 to ResultList.Count-1 do
             ...
             begin
               ...
               {$IFNDEF VISUAL_BUILD}
               if (D_MegaMain.OutputFormat = ExportFasta) then
                 D_Align.WriteFastaFile(D_Align.MySeqList, NextAvailableFilenameNV('.fasta'))
               else
                 D_Align.WriteMEGAFile(D_Align.MySeqList, NextAvailableFilenameNV('.meg'));
               {$ENDIF}
               AppLink.MAI.ARP.SetProgress(100);
...
begin
  ...
  Result.Add(opsPhylo2, 'MUSCLE');

  if aOptions.Count > 0 then
    for i := 0 to aOptions.Count - 1 do
    begin
      aKey := Trim(aOptions.Names[i]);
      if SameText(aKey, '-log') then
        continue; // don't save the log file path because it has identifiable info, such as user name
      aValue := Trim(aOptions.ValueFromIndex[i]);
      if aKey[1] = '-' then
