...
uses
  ...
  MGlobalSettings, MegaUtils, mmemutils, MAnalysisInfo, msitecoverage;
...
begin
  try
    ...
    inherited;
    {$IFDEF VISUAL_BUILD}
    if FCanComputeSiteCoverage and Assigned(Info) then
    begin
...
begin
    ...
    ParsimTrees.AddParsimonyTree(tree);
...
constructor TBranchBoundSearchThread.Create(Tree: TTreeList; OriSeqs: TList;
  NoOfSites: integer; NoOfBits: integer);
begin
  inherited Create(Tree, OriSeqs, NoOfSites, NoOFBits);
  FCanComputeSiteCoverage := True;
end;
...
begin
  ...
  try
    ...
    while OriTreeList.NoOfTrees > 0 do
      ParsimTrees.AddParsimonyTree(OriTreeList.Remove(0));
