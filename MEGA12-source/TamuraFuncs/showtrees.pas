...
function CheckTreePassesSanityCheck(aTree: TTreeData; isRooted: Boolean; otuNames: TStringList): Boolean;
...
uses
  ...
  {$ENDIF}
  SysUtils, MegaErrUtils, Graphics, MLegendGenerator,
  MTreeDataAdapter, mtree_display_setup, MegaVerConsts;
...
{$IFDEF VISUAL_BUILD}
var
  TreeExpl : TTreeViewForm = nil;
  {$ENDIF}
begin
  {$IFDEF VISUAL_BUILD}
   if not CheckTreePassesSanityCheck(AList[0], AList.isRooted, aList.OTUNameList) then
     exit;
  ...
  try
    try
      ...
      Result := TreeExpl;
        TreeExpl.Show;
        AList.AddInformation('User Tree File', ExtractFileName(filename));
      ...
      On E: Exception do
        ShowMessage('An application error occurred when loading the newick tree: ' + E.Message);
    end;
  finally
    if TreeExpl <> nil then
      TreeExpl.Free;
...
begin
  ...
  try
    try
      ...
      on E:Exception do
      begin
        ShowMessage('The input tree failed sanity checks and therefore failed to load: ' + E.Message);
        Result := False;
      end;
    end;
  finally
