  ...
  begin
    ...
    begin
      for j := 0 to max  do
        for k := 0 to max do
        if j <> k  then
          FMatrixExport.SavedExcelWrite.Add(FloatToPrecision(Matrix[j][k], 3), RowColor);
    end;
    FMatrixExport.SavedExcelWrite.WriteLine();
...
begin
    ...
    if FAnalysisInfo.MyMLAnalysisPack.IsInitialTree then
      FCaptionExpert.AssignData('TreeUsedString','The ML analyses were conducted using a user-specified topology.')
    else
      FCaptionExpert.AssignData('TreeUsedString', Format('The ML analyses were conducted using an automatically generated tree topology [{%s:Mega5Citation}].', [POST_PROCESS_CITATION]));
    if FAnalysisInfo.isAminoAcid then
      listofentries  := 'ARNDCQEGHILKMFPSTWYV'
    else
      listofentries := 'ATCG';
