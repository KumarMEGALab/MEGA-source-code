...
begin
  try
    try
      ...
      if not TempCalibs.DoAllValidations(ErrorMsg, False) then
      begin
      ...
      FCalibrationIndices := FCalibrations.CalibrationIndicesForTE;
      Tree.CalibratedNodes := FCalibrationIndices;
      Tree.ShowCalibratedNodeMarker := True;   { without this line, markers do not show up}
      Tree.DrawCalibratedNodeMarkers;
      if FCalibrationType = ctInternalNode then
...
begin
  ...
  try
    ...
    begin
      Assert(False, 'not implemented - need to handle the case when using tree explorer');
...
var
  ErrorMsg: String = '';
begin
  if (not Visible) or (not OkBtn.Enabled) or FIsClosing then
    Exit;
  ...
  try
    try
      ...
      begin
        if not FCalibrations.DoAllValidations(ErrorMsg, True) then
        begin
          FIsClosing := False;
        ...
        begin
          ...
          FCalibrations.RemoveUnselected;
          if Assigned(CalibrationWizard) then
          begin
...
begin
  ...
  begin
    try
      AnchoredTimesCS.Acquire;
      CalibrationsCancelled := True;
      if CancelledByTreeViewer then
        TreeViewerLives := False // so CalibrationWizard knows not to destroy the tree viewer as it was destroyed when the user closed it
      else
...
procedure TCalibrationDlg.HelpBtnMouseEnter(Sender: TObject);
begin
  HelpImages.GetBitmap(1, HelpBtn.Picture.Bitmap);
end;

procedure TCalibrationDlg.HelpBtnMouseLeave(Sender: TObject);
begin
  HelpImages.GetBitmap(1, HelpBtn.Picture.Bitmap);
end;
...
procedure TCalibrationDlg.OkBtnMouseEnter(Sender: TObject);
begin
  Buttons.GetBitmap(1, OkBtn.Picture.Bitmap);
end;

procedure TCalibrationDlg.OkBtnMouseLeave(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    if (TreeHeight - TopMargin) < newHeight then
      PixelsPerOtu := PPOTU;
    TreeWidth := TreePanel.Width - GetSystemMetrics(SM_CXVSCROLL) - LeftMargin - 15  - LargestWidthOfOTUNames;  // 15 for the tree's padding on each side.
    SetAttrIndex;
    Refresh;
  end;
end;
...
procedure TCalibrationDlg.CancelBtnMouseEnter(Sender: TObject);
begin
  Buttons.GetBitmap(3, CancelBtn.Picture.Bitmap);
end;

procedure TCalibrationDlg.CancelBtnMouseLeave(Sender: TObject);
begin
...
begin
  case aCalib.CalibrationCategory of
    ...
    ccMinTimeOnly:
      begin
        DivergenceTimeEdit.EditLabel.Caption := 'MIN TIME';
      end;
    ccMaxTimeOnly:
      begin
        MaxDivTimeEdit.EditLabel.Caption := 'MAX TIME';
      end;
  end;
end;
...
begin
  if (not Assigned(StatusBar)) or (not Assigned(ProgressBar)) then
    Exit;
  AWidth := Max(150, StatusBar.Canvas.TextWidth(StatusBar.Panels[1].Text)) + 4;
  StatusBar.Panels[1].Width := Max(200, AWidth);
  StatusBar.Width := BottomBottomPanel.Width - ProgressBar.Width - 6;
  StatusBar.Panels[0].Width := StatusBar.Width - StatusBar.Panels[1].Width;
  ProgressBar.Left := StatusBar.Left + StatusBar.Width + 2;
end;
...
begin
  ...
  AStatusBar.Align := alLeft;
  AProgressBar.Parent := BottomBottomPanel;
  AStatusBar.Width := BottomBottomPanel.Width - AProgressBar.Width - 6;
...
begin
  ...
  ClientHeight := Max(730, TreeHeight + Toolbar4.Height + BottomPanel.Height);
end;
...
begin
  if ShowHelpBtn.Caption = '<< less' then
  begin
    ShowHelpBtn.Caption := 'more >>';
...
begin
  if ShowHelpBtn2.Caption = '<< less' then
  begin
    ShowHelpBtn2.Caption := 'more >>';
