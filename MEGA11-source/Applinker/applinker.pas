...
uses
  ...
  MAnalysisInfo, MegaConsts, Pipes{$IFDEF VISUAL_BUILD}, AppOptionsDlg, Menus{$ELSE}, MegaUtils_NV{$ENDIF};
...
type
  ...
  TAppLinkThread = class abstract({$IFDEF VISUAL_BUILD}TThread{$ELSE}TMegaThread{$ENDIF})
    protected
      FIsSuccess: Boolean;
      ...
      function NewLineCheckCancel(NewLine: String): Boolean; virtual;
...
uses
  {$IFDEF VISUAL_BUILD}
  MAlignEditMainForm, mega_main, MEditorForm, MTreeViewForm, Dialogs, MWebBrowser,
  {$ELSE}
...
function TAppLinkThread.NewLineCheckCancel(NewLine: String): Boolean;
begin
  FNewLine := NewLine;
...
begin
  if Assigned(NewLineCheckCancelFunc) then
    FIsCancelled := NewLineCheckCancelFunc(FNewLine);
end;
...
begin
  ...
  try
    ...
    FMuscleLink.NewLineCheckCancelFunc := NewLineCheckCancel;
    ...
    begin
      Result := False;
      FMessagesLog.Add('MUSCLE failed with error: ' + FMuscleLink.ErrorCodeMessage);
    end
    else
      Result := True;
  except
    on E:Exception do
      FMessagesLog.Add('Failed to run MUSCLE: ' + E.Message);
  end;
end;
...
begin
  ...
  begin
    FIsSuccess := False;
    FMessagesLog.Add('MUSCLE application linker not initialized');
    Terminate;
  end
  else
...
begin
  ...
  try
    try
      ...
      //{$IFDEF DEBUG}WriteToGlensLogFile(FCommandLine);{$ENDIF}
      Process.Execute;
...
begin
  ...
  begin
    ...
    LogFilename := GetAppConfigDir(False) + 'temp' + PathDelim + 'MuscleRunLog_' + IntToStr(GetCurrentProcessId) + '.txt';
