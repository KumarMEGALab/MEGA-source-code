...
type
  ...
  private
    ...
    function CleanOtuNameForExport(aName: AnsiString): AnsiString;
  ...
  public
    ...
    function RemoveOtu(aName: String): Boolean;
    ...
    function OutputAllNewickTrees(Options: TNewickExportOptions; Cutoff: Double): TStringList;
    ...
    procedure UpdateNodeLabels(Calibs: TCalibrations);
...
function TTreeList.GetInternalNodeLbl(index: integer):AnsiString;
begin
  if (not Assigned(FInternalNodeLbls)) or (index >= FInternalNodeLbls.Count) then
    result := ''
  else
    result := FInternalNodeLbls[index];
end;

procedure TTreeList.SetInternalNodeLbl(index: integer; value: AnsiString);
begin
...
function TTreeList.GetHasInternalNodeLbls: Boolean;
var
  i: Integer;
begin
  ...
  begin
    ...
    begin
      if Trim(FInternalNodeLbls[i]) <> EmptyStr then
        exit;
    end;
    Result := False;
  end;
...
function TTreeList.RemoveOtu(aName: String): Boolean;
var
  index: Integer;
  i: Integer;
begin
...
begin
  ...
  begin
    ...
    TrimTaxaName2(Calibration.NodeName);
    Exit;
  end;

  if Calibration.NodeName <> EmptyStr then // the user has specified a NodeName given in the newick file
  begin
    ...
    SetInternalNodeLbl(Calibration.NodeID, Calibration.NodeName);
  ...
  begin
    ...
    if (TempLbl <> EmptyStr) and (TempLbl <> Calibration.MRCAName) then
    begin
      {$IFDEF VISUAL_BUILD}
//      raise Exception.Create('Conflicting labels given for the same node: the calibration input has ' + #39 + Calibration.MRCAName + #39 + ' and the newick file has '+ #39  + TempLbl + #39 + '.');
    ...
    if Trim(Calibration.MRCAName) <> EmptyStr then
      SetInternalNodeLbl(Calibration.NodeId, Calibration.MRCAName);
  end
...
function TTreeList.CleanOtuNameForExport(aName: AnsiString): AnsiString;
var
  i: integer;
begin
  Result := trim(aName);
...
//            str[Pos('_', str)] := ' ';
        ...
        {$IFNDEF VISUAL_BUILD}
        if result = -1 then
          Error_NV('MEGA Error: Could not find entry in NameList = ' + str + '.  Ensure that the names in your tree exactly match the names in your analysis file.');
        {$ENDIF}
      end;
    end;
  ...
  try
    try
      ...
      begin
        ...
        MaxStats := 0;
        if not HasGeneDupsOrSpeciationMarkers(filebuffer) then
        begin
      ...
      begin
        {$IFDEF DEBUG}
          {$IFDEF VISUAL_BUILD}
           LogErrorMsg('Failure at DecodeNodeData' + E.Message);
          {$ELSE}
           warn_nv('Failure at DecodeNodeData');
          {$ENDIF}
        {$ENDIF}
...
var
  ...
  begin
    ...
    begin
      ...
      if FNewickExportOptions.NodeLabels and (Trim(InternalNodeLbl[TreeData[nodeindex].des1-NoOfOTUs]) <> EmptyStr) then
        Result := Result +')' + #39 + InternalNodeLbl[TreeData[nodeindex].des1-NoOfOTUs] + #39
      else
        Result := Result + ')';
      if TreeData[nodeIndex].des1 >= NoOfOtus then
    ...
    begin
      if WriteStats and (TreeData[nodeindex].des1 >= NoOfOTUs) then
        if abs(MaxStats) < 0.000000000001 then
          ...
          result := result +FloatToStrF(TreeData.Stats[TreeData[nodeindex].des1]/MaxStats,ffFixed,8,4);  // Tamura change Jan / 15
      if WriteBranchLengths then
        result := result +':'+FloatToStrF(TreeData.BLen[TreeData[nodeindex].des1],ffFixed,15,8);
    end;
    ...
    begin
      ...
      if FNewickExportOptions.NodeLabels and (Trim(InternalNodeLbl[TreeData[nodeindex].des2-NoOfOTUs]) <> EmptyStr) then
        result := result +')' + #39 + InternalNodeLbl[TreeData[nodeindex].des2-NoOfOTUs] + #39
      else
        result := result + ')';
      if TreeData[nodeindex].des2 >= NoOfOTUs then
    ...
    begin
      if WriteStats and (TreeData[nodeindex].des2 >= NoOfOTUs) then
        if abs(MaxStats) < 0.000000000001 then
          ...
          result := result +FloatToStrF(TreeData.Stats[TreeData[nodeindex].des2]/MaxStats,ffFixed,8,4);  // Tamura change Jan / 15
      if WriteBranchLengths then
        result := result +':'+FloatToStrF(TreeData.BLen[TreeData[nodeindex].des2],ffFixed,15,8);
    end;
  end;
...
begin
  ...
  try
    ...
    if IsRooted then
      ExportNode(root)
    else if TreeData[root].des1 < NoOfOTUs then
    begin
      Result := Result + CleanOtuNameForExport(OTUName[TreeData[root].des1]);
      if WriteBranchLengths then
        Result := Result + ':' + FloatToStrF(TreeData.BLen[TreeData[root].des1] + TreeData.BLen[TreeData[root].des2],ffFixed,15,8);
      Result := Result + ',';

      ExportNode(TreeData[root].des2-NoOfOTUs);
    end
    else if TreeData[root].des2 < NoOfOTUs then
    begin
      ...
      begin
        ...
        if WriteStats then
          if abs(MaxStats) < 0.000000000001 then
            ...
            Result := Result + FloatToStrF(TreeData.Stats[TreeData[root].des2]/MaxStats,ffFixed,8,4);  // Tamura change Jan / 15
        if WriteBranchLengths then
          Result := Result + ':' +FloatToStrF(TreeData.BLen[TreeData[root].des1]+TreeData.BLen[TreeData[root].des2],ffFixed,15,8);
      end
      else
...
function TTreeList.OutputAllNewickTrees(Options: TNewickExportOptions; Cutoff: Double): TStringList;
var
  i: Integer;
begin
  Result := TStringList.Create;
  if NoOfTrees > 0 then
    for i := 0 to NoOfTrees - 1 do
      Result.Add(OutputNewickTree(i, Options, Cutoff));
end;
...
function TTreeList.ExportATreeToNewickFile(index: integer; filename :String; bl,st:boolean; CutOff: double): boolean;
var
  f : TextFile;
begin
  try
...
function TTreeList.ToStringList: TStringList;
var
  i: Integer;
  j: Integer;
...
begin
  ...
  for i := 0 to NoOfTrees - 1 do
  begin
    TempStringList := TTreeData(Items[i]).StateToStringList('');
...
procedure TTreeList.UpdateNodeLabels(Calibs: TCalibrations);
var
  i: Integer;
  NodeId: Integer;
begin
  ...
  begin
    if Calibs.GetCalibration(i).IsSamplingTime then
      continue;
    NodeId := Calibs.GetCalibration(i).NodeID;
    if Trim(Calibs.GetCalibration(i).NodeLabel) <> EmptyStr then { don't replace any labels specified in the newick file with an empty string}
      InternalNodeLbl[NodeId] := Calibs.GetCalibration(i).NodeLabel;
  end;
end;
