...
uses
  ...
  MegaPrivateFiles, MegaErrUtils, RegExpr;
{$M+}
...
const
  ...
  validOperators: array [0 .. 3] of String = (tcoEquals, tcoNotEquals,
    tcoLessThan, tcoGreaterThan);
...
type
  ...
  public
    ...
    procedure AssignData(TplVarName, Value: String);
...
uses
  {$IFDEF VISUAL_BUILD}htmlview, {$ENDIF}
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if SameText(TagRecord.ConditionalValue, TagRecord.Value) then
                TagRecord.ConditionalResult := True;
            end
            else if TagRecord.ConditionalOperator = tcoNotEquals then
            begin
              // if TagRecord.ConditionalValue <> TagRecord.Value then
              if StrIComp(PChar(TagRecord.ConditionalValue), PChar(TagRecord.Value)) <> 0 then
                TagRecord.ConditionalResult := True;
            end
            ...
            begin
              if TryStrToInt(TagRecord.ConditionalValue, testValueInteger) and
                TryStrToInt(TagRecord.Value, testValueInteger) then
              begin
                // we have an integer of some sort
                if StrToInt(TagRecord.Value) >
                  StrToInt(TagRecord.ConditionalValue) then
                  TagRecord.ConditionalResult := True;
              end
              else if TryStrToFloat(TagRecord.ConditionalValue, testValueDouble)
                and TryStrToFloat(TagRecord.Value, testValueDouble) then
              begin
                if StrToFloat(TagRecord.Value) >
              ...
              begin
                ...
                if StrToInt(TagRecord.Value) > GetEnumValue(PropInfo.PropType, TagRecord.ConditionalValue) then
                  TagRecord.ConditionalResult := True;
              end;
            end
...
var
  ...
  IntermediaryStream: TStringStream;
...
begin
  ...
  try
    ...
    begin
      ...
      Parser := TParser.Create(IntermediaryStream);  // Using the undocumented TParser class
      ParseState := psNext;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                ParseState := psNext;
                CurTag.PositionEnd := Parser.SourcePos + 1; // why do I need to add 1?
                if TagStack.Count > 0 then
                begin
              ...
              begin
                ...
                begin
                  ...
                  Parser.NextToken;
                  CurTag.PositionEnd := Parser.SourcePos + 1;
                  ParseState := psNext;
                  // Continue;
                end
                else
      ...
      on E:Exception do
      {$IFDEF VISUAL_BUILD}
      ShowMessage('Oh no! An error has occurred while generating a caption: ' + E.Message);
      {$ELSE}
...
var
  ExternFileImport: TStringList;
  ExternFile: String;
  TempTag: String;
  AFile: String;
  posExternalFileTag: Integer;
begin
  ExternFileImport := nil;
  Result := False;
  try
    try
      ...
      begin
        ...
        begin
          ...
          TemplateText.Text := ReplaceStr(TemplateText.Text, TempTag, ExternFileImport.Text);
          posExternalFileTag := Pos('IncludeExternalFile', TemplateText.Text);
        end;
        Result := True;
      end
...
procedure TLegendGenerator.AssignData(TplVarName, Value: String);
var
  i: Integer;
  CurTag: PTag;
begin
  ...
  begin
    CurTag := TemplateTags.Items[i];
    if (CurTag.Namespace = ClassName) and (CurTag.ValueField = TplVarName) then
    begin
      CurTag.Value := Value;
      ...
      if CurTag.IsConditional then
        if CurTag.ConditionalValue = CurTag.Value then
          CurTag.ConditionalResult := True
        else if (SameText(TplVarName, DISP_DIV_TIMES_STR)) or
          (SameText(TplVarName, DISP_ERR_BARS_STR)) then
        begin
...
function TLegendGenerator.LoadCitationFromFile(HelpTopic: String)
  : String;
const
  // move this out of here eventually
  mfCitationDir = 'Private' + PathDelim + 'Citations' + PathDelim;
  sRoboHelpHTMLExtension = '.htm';
var
  CitationHTML: TStringList;
  ReferenceHTML, FullCitationFilePath: String;
  PosBegin, PosEnd, i, dupePos: Integer;
  isDupe: Boolean;
begin
  // Loads citation information from HTML help output produced by RoboHelp
  ReferenceHTML := '';
  Result := '';
  CitationHTML := TStringList.Create;
  FullCitationFilePath := GetPrivateFile(mfCitationDir + HelpTopic +
    sRoboHelpHTMLExtension);
  try
    if FileExists(FullCitationFilePath) then
    begin
      CitationHTML.LoadFromFile(FullCitationFilePath);
      // all of the other parsers suck, parse it myself
      ...
      ReferenceHTML := '<P class="reference" >$refNumber. </i> ' +
        ReferenceHTML + '</P>';
      // what if this citation already exists in the list?
      isDupe := False;
      for i := 0 to CitationList.Count - 1 do
      begin
        if CitationList.Strings[i] = ReferenceHTML then
        begin
          isDupe := True;
          dupePos := i;
          Break;
    ...
    begin
      if ExtractFileName(FullCitationFilePath) = '.htm' then
        Raise (Exception.Create
          ('Your MEGA installation is missing files required to properly generate citations for captions.  Please report this bug to the MEGA authors.')
          )
      else
        Raise (Exception.Create
          ('Your computer appears to be missing files required to properly generate citations for captions.  Missing file is '
          + FullCitationFilePath +
          '. Please report this bug to the MEGA authors.'));
    end;
...
var
  testInteger: Integer;
begin
  // break this out into a function when we have more
  if Tag.FunctionName = 'ExpandCitation' then
  begin
    // Maybe not  the best place to check this but...
...
var
  ConstData: TStringList; // TIniFile would be overkill
  Filename: String;
  i: Integer;
begin
  Result := False;
  try
    try
      ...
      ConstData := TStringList.Create;
      FileName := GetPrivateFile(mfTemplateConstants);
      if FileExists(FileName) then
      begin
        ConstData.LoadFromFile(Filename);
        for i := 0 to ConstData.Count - 1 do
          AssignData(ConstData.Names[i], ConstData.Values[ConstData.Names[i]]);
