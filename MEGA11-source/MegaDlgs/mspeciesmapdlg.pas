...
uses
  ...
  ExtCtrls, Menus, StdCtrls, Grids, Buttons, ClipBrd,
...
begin
  ...
  begin
    ...
    aCaptions[1] := 'All';
    aMsgDlg := CreateMessageDialog('Edit species names only for selected rows or all rows', mtConfirmation, mbYesNo);
...
procedure TSpeciesMapDlg.SaveBtnMouseEnter(Sender: TObject);
begin
  Buttons.GetBitmap(1, SaveBtn.Picture.Bitmap);
end;

procedure TSpeciesMapDlg.SaveBtnMouseLeave(Sender: TObject);
begin
...
procedure TSpeciesMapDlg.CancelBtnMouseEnter(Sender: TObject);
begin
  Buttons.GetBitmap(3, CancelBtn.Picture.Bitmap);
end;

procedure TSpeciesMapDlg.CancelBtnMouseLeave(Sender: TObject);
begin
...
procedure TSpeciesMapDlg.InitImages;
begin
  {$IFDEF DARWIN}
