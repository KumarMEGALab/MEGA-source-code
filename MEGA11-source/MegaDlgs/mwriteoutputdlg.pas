...
uses
  ...
  StdCtrls, Buttons, EditBtn, MegaConsts, mimageform;
...
begin
  ...
  case Result of
    EXtext: ;
    EXtextSave, EXcsvSave, EXexcelSave, EXexcelXmlSave, EXodsSave: SaveLocation := WriteOutputDlg.OutputFile;
...
procedure TWriteOutputDlg.CancelBtnMouseEnter(Sender: TObject);
begin
  ButtonImages.GetBitmap(3, CancelBtn.Picture.Bitmap);
end;

procedure TWriteOutputDlg.CancelBtnMouseLeave(Sender: TObject);
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if FileExists(ExtractFileName(FullFileName)) then
...
procedure TWriteOutputDlg.HelpBtnMouseEnter(Sender: TObject);
begin
  HelpImages.GetBitmap(1, HelpBtn.Picture.Bitmap);
end;

procedure TWriteOutputDlg.HelpBtnMouseLeave(Sender: TObject);
begin
  HelpImages.GetBitmap(0, HelpBtn.Picture.Bitmap);
end;

procedure TWriteOutputDlg.OkBtnMouseEnter(Sender: TObject);
begin
  ButtonImages.GetBitmap(1, OkBtn.Picture.Bitmap);
end;

procedure TWriteOutputDlg.OkBtnMouseLeave(Sender: TObject);
begin
...
begin
  ...
  //UpdateSaveOptions;
  case AValue of
    EXtextSave, EXcsvSave, EXexcelSave, EXexcelXmlSave, EXodsSave: SaveToDiskRadioBtn.Checked := True;
...
begin
  Result := -1;
  //If this function is changed make sure to change GetExportType
  case Choice of
    EXInvalid, EXnone, EXfasta: raise Exception.Create('invalid export type');
