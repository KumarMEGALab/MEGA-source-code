...
uses
  ...
  MD_InputSeqData, MD_InputDistData, MTreeData, MTreeList, MPartitionList,
  MTreePack, MTreeSearchThread, ShowTrees, ErrorMessages_HC, MD_Align, mreltimethreads, MReltimeComputer,
  MTreeDataAdapter, msitecoverage, MSimpleTreeNode, mcrossprojectutils,
  mcheck_abort_reltime, mdist_command_threads;
...
begin
  ...
  try
    try
      ...
      begin
        if not (Thread as TTreeSearchThread).Canceled then
        begin
          {$IFDEF VISUAL_BUILD}
...
const
  ValidOps: set of TDistTreeDlgOption = [dtdoRelTimeML, dtdoRelTimeBLens, dtdoRelTimeLS, dtdoRtdtML, dtdoRtdtLS, dtdoRtdtBlens, dtdoCalTest, dtdoCorrTestML, dtdoCorrTestBlens];
begin
...
begin
  ...
  try
    try
      ...
      MyExceptionName := ReltimeThread.MyExceptionName;
      if MyExceptionName <> 'none' then
      begin
        MyExceptionMessage := ReltimeThread.MyExceptionMessage;
        if Assigned(AnalysisInfo.ARP) then
          AnalysisInfo.ARP.Free;
        AnalysisInfo.Free;
        if MyExceptionName = 'EOutOfMemory' then
        ...
        begin
          ...
          raise Exception.Create(MyExceptionMessage);
          {$ELSE}
          Error_NV('Error: ' + Trim(MyExceptionMessage));
          {$ENDIF}
        end;
      end
      else
...
begin
  ...
  try try
    ...
    ARP.AddAnalysisOptions('No. of Sites', Format('%.0f', [MAI.MyNoOfSites*1.0]));
...
procedure BootstrapNJ(MAI: TAnalysisInfo; DoFullProgress: Boolean);
var
  ABootstrapNJThread: TBootstrapNJThread;
  ABootstrapNJTerminator: TTreeThreadTerminator;
  ATree: TTreeData;
begin
...
//    ABootstrapNJThread.BootstrapDistanceProc := BootstrapSampleD;
    ...
    ABootstrapNJThread.OnTerminate := ABootstrapNJTerminator.OnThreadDone;
...
procedure BootstrapUPGMA(MAI: TAnalysisInfo; DoFullProgress: Boolean);
var
  ABootstrapUPGMAThread: TBootstrapUPGMAThread;
  ABootstrapUPGMATerminator: TTreeThreadTerminator;
  ATree: TTreeData;
begin
...
//    ABootstrapUPGMAThread.BootstrapDistanceProc := BootstrapSampleD;
    ...
    ABootstrapUPGMAThread.OnTerminate           := ABootstrapUPGMATerminator.OnThreadDone;
...
procedure BootstrapME(MAI: TAnalysisInfo);
var ABootstrapMEThread: TBootstrapMEThread;
    ABootstrapMETerminator: TTreeThreadTerminator;
    ATree: TTreeData;
begin
...
//    ABootstrapMEThread.MaxNoOfTrees := MAI.MyTreePack.MaxTrees;       // Added by KT on 001206

//    ABootstrapMEThread.Threshold := ???; // add it to the Compute Options Dlg in 2.2
...
//    ABootstrapMEThread.BootstrapDistanceProc := BootstrapSampleD;
    ...
    ABootstrapMEThread.OnTerminate := ABootstrapMETerminator.OnThreadDone;
...
begin
  ...
  try try
    ...
    D_InputSeqData.PrepareDataForDistAnalysis(MyAnalysisInfo.MySubsetOpt, MyAnalysisInfo.MyMappedData, MyAnalysisInfo.MyUsedOtuInfos, MyAnalysisInfo.MyNoOfSeqs, MyAnalysisInfo.MyNoOfSites,MyAnalysisInfo.MyLabelsUsed, MyAnalysisInfo.RecodeScheme, MyAnalysisInfo.SiteCoverage);
...
procedure RunCorrTest(MAI: TAnalysisInfo);
var
  ctestThread: TCorrTestBlenThread;
  Terminator: TRelTimeTerminator;
  i: Integer;
begin
  ctestThread := nil;
