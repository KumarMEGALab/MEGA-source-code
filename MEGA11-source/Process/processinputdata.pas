...
uses
  ...
  MLexSeq, MRuntimeProgressDlg, MLexDist;
...
function PromptUserForDataFileInfoOld(aInfo: TDataFileInfo; OpenInSDE: Boolean=True): Boolean;
...
uses
  ...
  MegaPrivateFiles,  MDomainInfo, MOtuInfo,
...
var
  ...
  js: String;
  {$IFDEF DARWIN}
  MyInputDataOptionsDlg: TInputDataOptionsDlg;
  {$ENDIF}
  {$ELSE}
  ProcessPack: TProcessPack;
  ContainsCoding: String;
...
begin
  ...
  {$IFDEF VISUAL_BUILD}
    {$IFNDEF DARWIN}
    try
      CurrentDataFileInfo := aInfo;
      CurrentDataFileInfo.OpenInSDE := OpenInSDE;
      js := 'var saved_link = ''' + InputDataOptionsFiletype + ''';';
      HtmlOptionsDialog.Show;
      if OpenInSDE then
        HtmlOptionsDialog.ProcessMessageReceivedProc := aInfo.ProcessInputOptionsMessage;
...
function PromptUserForDataFileInfoOld(aInfo: TDataFileInfo; OpenInSDE: Boolean): Boolean;
var
  {$IFDEF VISUAL_BUILD}
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if not AFoundFormat then
        begin
          if not PromptUserForDataFileInfoOld(Result, False) then
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        {$ELSE}
        PromptUserForDataFileInfo(Result, OpenInSDE);
    ...
    begin
      ...
      begin
        ...
        if not PromptUserForDataFileInfo(Result) then
          raise EAbort.Create('user aborted');
        {$ELSE}
        PromptUserForDataFileInfo(Result);
        raise EAbort.Create('Aborting normal process execution in order to prompt user for needed information');
        {$ENDIF}
      end;
    end
  ...
  begin
    ...
    begin
      ...
      Result.SetUserSpecifiedSymbols(false);
      {$IFNDEF VISUAL_BUILD}
      if (not AnsiContainsText(ADataType, 'distance')) and (not (D_MegaMain.MegaAction = maFindTipDates))  then
      begin
...
begin
  ...
  Application.ProcessMessages;
  SetupSequenceInformation(MyParser, ARP, OpenInSDE and (MyParser.NoOfSites < SDE_MAX_SITES));
  ARP.Hide;
  Result := True;
end;
...
begin
  ...
  SetupFastaSequenceInformation(DataFileInfo, FNoOfTaxa, FNoOfSites, FTaxa, FSequence, ARP, OpenInSDE and (FNoOfSites < SDE_MAX_SITES));
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SetupNexusSequenceInformation(ANexusFile, ARP, OpenInSDE and (ANexusFile.NumSites < SDE_MAX_SITES));
        Result := True;
      end
      else
      begin
