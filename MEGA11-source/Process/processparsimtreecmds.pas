...
uses
  ...
  MTreeSearchThread, ShowTrees, MTreeDataAdapter, msitecoverage;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                bootstrapVals := TBootstrapMPTreeSearchThread(Thread).GetBootstrapValues;
                for i := Low(bootstrapVals) to High(bootstrapVals) do
                  aData.Stats[i] := bootstrapVals[i];
              end;
...
begin
  try
    ...
    D_InputSeqData.PrepareDataForParsimAnalysis(MAI.MySubsetOpt, MAI.MyInfoSites, MAI.MyUsedOtuInfos,
...
begin
  ...
  if AnalysisInfo.MyUsrOperation = dtdoMPInferAncSeq then
      ...
      begin
        ...
        begin
          try
            ...
            data := MyOriTreeList[0];
            tda.GuessOutgroup(data);
            MAI.TransferTreeOutgroupInfoToOtuInfos(0);
          finally
        ...
        if MAI.MyOriTreeList.isRooted then
          ...
          error_nv('ancestral sequence inference requires that an outgroup be defined');
        {$ENDIF}
      end;
      MAI.GroupInfo.SetTreeOutgroupFromOtuInfos;
      {$IFNDEF VISUAL_BUILD}
      if not MAI.IngroupDefined then
        error_nv('An ingroup must be defined for the ancestral sequence inference analysis');
      {$ENDIF}
      RootOnOutgroup(MAI);
      MAI.MyOriTreeList.isRooted := True;
      InferAncestralSeq(MAI);  // Show the tree, set the Ancestral states flag as on.
    end;
  end;
  Result := True;
end;
