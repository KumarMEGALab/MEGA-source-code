...
uses
  ...
  mtajimaneutralitytestthread, mpartitions_blens_summary, mextendedlist,
  mmlcalculatedvalues, MTreeData
  {$IFDEF DEBUG}LazLogger,{$ENDIF}
  mcheck_abort_reltime, mtipdatesfinder, mparse_mao_file,
  mgeographical_info, mega_citation, mcompare_bootstrap_trees, mtabular_tree_export;
...
begin
  ...
  try
    ...
    ExitCode := 0;
    WriteLn();
    WriteLn('MEGA has completed the requested action');
    D_MegaMain.Free;
  except
    on E: Exception do
