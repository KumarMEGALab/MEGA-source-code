...
type
  ...
  protected
    ...
    function ParseResponse: Boolean;
...
uses
  ...
  StringUtils, fpjson, jsonparser, MegaConsts;
...
function TUpdateThread.ParseResponse: Boolean;
var
  AParser: TJSONParser;
  AJson: TJSONObject;
  AData: TJSONData;
 begin
  ...
  AData := nil;
  Result := False;
  if Trim(FServerResponse) = EmptyStr then
    Exit;
  {$IFDEF DEBUG}
    {$IFDEF UNIX}
    Exit;
  ...
  try
    try
      AParser := TJSONParser.Create(FServerResponse, []);
      AData := AParser.Parse;
      if (not Assigned(AData)) then
        raise Exception.Create('failed to parse JSON');
      AJson := TJSONObject(AData);

      AData := AJson.Find('current_major', jtNumber);
      if Assigned(AData) then
        FMajorVersion := AData.Value;

      AData := AJson.Find('current_minor', jtNumber);
      if Assigned(AData) then
        FMinorVersion := AData.Value;

      AData := AJson.Find('current_release', jtNumber);
      if Assigned(AData) then
        FReleaseNumber := AData.Value;

      AData := AJson.Find('current_build_code', jtNumber);
      if Assigned(AData) then
        FBuildCode := AData.Value;

      AData := AJson.Find('has_update', jtBoolean);
      if Assigned(AData) then
        FHasUpdate := AData.Value;

      AData := AJson.Find('has_new_version', jtBoolean);
      if Assigned(AData) then
        FHasNewVersion := AData.Value;

      AData := AJson.Find('message', jtString);
      if Assigned(AData) then
        FMessage := AData.Value;

      AData := AJson.Find('url', jtString);
      if Assigned(AData) then
        FDownloadUrl := AData.Value;

      AData := AJson.Find('release_type', jtString);
      if Assigned(AData) then
        FReleaseType := AData.Value;

      AData := AJson.Find('version_id', jtNumber);
      if Assigned(AData) then
        FVersionId := AData.Value;
   ...
   try
     ...
     begin
       ...
       begin
         ...
         if currentVersionInfo.IsCompatible(NewestVersionInfo) then
         begin
