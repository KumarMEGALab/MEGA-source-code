...
uses
  ...
  MegaConsts, genedomainpropertiesdlg, htmloptionsdlg, cef3intf,
...
type
  ...
  private
    ...
    //procedure drawDomainArrow(site: Integer; aCanvas: TCanvas);
...
uses
  {$IFDEF DARWIN}mseqdataexportoptionsdlg,{$ENDIF}
  LCLIntf, MD_InputSeqData, MVS_SeqDataExplorer, math, DataExplorerHelp_HC,
  ...
  mbrowserutils, MegaPrivateFiles, StringUtils, MutationExplorer,
...
begin
  ...
  begin
    aRect := DataGrid.Selection;
    if (Key = VK_LEFT) then
       aRect.Left := Max(1, aRect.Left - 1)
    else if Key = VK_RIGHT then
      aRect.Left := Min(DataGrid.ColCount - 1, aRect.Left + 1);
    DataGrid.Selection := aRect;
  end;
  inherited;
end;
...
begin
  ...
  begin
    ...
    if ((X - mouseRect.Left) < 15) and (ACol > 0) then
      ACol := ACol - 1;
...
begin
  ...
  try
    ...
    begin
      ...
      if X > 15 then // Can't have a col smaller than 15 pixels
      begin
    ...
    if (not showSplit) and (not ((Col = 0) and (X <= 15))) then
    begin
    ...
    if (Col <> 0) or (Row <> 0) or  (X > 15) then
...
begin
  ...
  try
    ...
    begin
      ...
      if (ACol >= Cols.VisibleCount) and (ARow > 0) and (Button = mbRight) then // right click in the sequence data
      begin
        AddEditGeneDomains.Popup(xyFromDataGrid.X, xyFromDataGrid.Y);
      end;
...
begin
  ...
  ActionDispTaxaNamesExecute(Self);
  {$IFNDEF DARWIN}
  InitMainMenu;
  InitPopupMenus;
  {$ENDIF}
...
begin
  try
    ...
    on E:Exception do
      ShowMessage('Oh no! An error occurred when exporting sequence data: ' + E.Message);
  end;
end;

procedure TV_SeqDataExplorer.ProcessWebOptionsMessage(const Browser: ICefBrowser; const message: ICefProcessMessage; out Result: Boolean);
var
  ExportOptions : TSeqExportOptions = nil;
  siteNumbers, gapTreatment, includedSites: String;
  fileType, aDestination: String;
  firstPos, secondPos, thirdPos, noncodingPos: Boolean;
...
begin
  try
    ...
    ExportOptions := TSeqExportOptions.Create;
    ExportOptions.FileTitle := message.ArgumentList.GetString(SDEO_SEQDATA_TITLE_INDEX);
    ExportOptions.FileDescription := message.ArgumentList.GetString(SDEO_SEQDATA_DESCRIPTION_INDEX);

    fileType := message.ArgumentList.GetString(SDEO_FILE_FORMAT_INDEX);
    ExportOptions.ExportFormat := StringToMegaExportFormat(fileType);
    temp := message.ArgumentList.GetString(SDEO_SITES_PER_LINE_INDEX);
    ExportOptions.SitesPerLine := StrToInt(temp);
    ExportOptions.IsInterleaved:= StrToBool(message.ArgumentList.GetString(SDEO_IS_INTERLEAVED_INDEX));
    if ExportOptions.ExportFormat = mefFasta then
      ExportOptions.IsInterleaved := False;
    siteNumbers := message.ArgumentList.GetString(SDEO_WRITING_SITE_NUMBERS_INDEX);
    if siteNumbers = SDEO_FOR_EACH_SITE then
      ExportOptions.IsWriteSiteNumAtTop := True
    else if siteNumbers = SDEO_AT_END_OF_LINE then
      ExportOptions.IsWriteSiteNumAtEnd := True;

    firstPos := StrToBool(message.ArgumentList.GetString(SDEO_INCLUDE_FIRST_POSITION_INDEX));
    secondPos := StrToBool(message.ArgumentList.GetString(SDEO_INCLUDE_SECOND_POSITION_INDEX));
    thirdPos := StrToBool(message.ArgumentList.GetString(SDEO_INCLUDE_THIRD_POSITION_INDEX));
    noncodingPos := StrToBool(message.ArgumentList.GetString(SDEO_INCLUDE_NONCODING_POSITIONS_INDEX));
    ExportOptions.IsRemove1stBase := not firstPos;
    ExportOptions.IsRemove2ndBase := not secondPos;
    ExportOptions.IsRemove3rdBase := not thirdPos;
    ExportOptions.IsRemove0thBase := not noncodingPos;

    gapTreatment := message.ArgumentList.GetString(SDEO_GAP_TREATMENT_INDEX);
    if SameText(gapTreatment, SDEO_EXC_MISSING_AMBIGUOUS_GAP_SITES) then
    begin
      ExportOptions.IsRemoveGapData := True;
      ExportOptions.IsRemoveMissData := True;
    end
    else if SameText(gapTreatment, SDEO_EXC_MISSING_AMBIGUOUS_ONLY) then
    begin
      ExportOptions.IsRemoveGapData := False;
      ExportOptions.IsRemoveMissData := True;
    end
    else if SameText(gapTreatment, SDEO_EXC_GAPS_ONLY) then
    begin
      ExportOptions.IsRemoveGapData := True;
      ExportOptions.IsRemoveMissData := False;
    end
    else
    begin
      ExportOptions.IsRemoveGapData := False;
    ...
    includedSites := message.ArgumentList.GetString(SDEO_INCLUDED_SITES_INDEX);
    if SameText(includedSites, SDEO_ALL_SITES) then
    begin
      ExportOptions.IsExportHighlightedSites := True;
      ExportOptions.IsExportUnHighlightedSites := True;
    end
    else if SameText(includedSites, SDEO_ONLY_HIGHLIGHTED_SITES) then
    begin
      ExportOptions.IsExportHighlightedSites := True;
      ExportOptions.IsExportUnHighlightedSites := False;
    end
    else if SameText(includedSites, SDEO_ONLY_UNHIGHLIGHTED_SITES) then
    begin
      ExportOptions.IsExportHighlightedSites := False;
      ExportOptions.IsExportUnHighlightedSites := True;
    end;
    ExportOptions.isNucData := (D_InputSeqData.FIsNuc and (not VS_SeqDataExplorer.IsTranslated));
    if D_InputSeqData.FIsNuc and D_InputSeqData.FIsCoding and (not VS_SeqDataExplorer.IsTranslated) then
      ExportOptions.IsChooseBases := True;
    ExportOptions.IsCodonByCodonWriting := IsCodonByCodonWriting(ExportOptions.IsRemove1stBase,
                                                                 ExportOptions.IsRemove2ndBase,
                                                                 ExportOptions.IsRemove3rdBase,
                                                                 SameText(includedSites, SDEO_ALL_SITES));
    if D_InputSeqData.IsCoding then
    begin
      temp := message.ArgumentList.GetString(SDEO_FULL_CODONS_INDEX);
      ExportOptions.ExportFullCodons := StrToBool(temp);
      if ExportOptions.ExportFullCodons then
        ExportOptions.IsCodonByCodonWriting := True;
    end;
...
begin
  ExportDataDlg := nil;
  try try
    ExportDataDlg := TSeqDataExportOptionsDlg.Create(Self);  //create dialog for users to select output options
    //Setup the dialog for the users to select export options
    if D_InputSeqData.FIsNuc and D_InputSeqData.FIsCoding and (not VS_SeqDataExplorer.IsTranslated) then
    begin
      ExportDataDlg.IsNucData     := True;
      ExportDataDlg.IsChooseBases := True;
    end
    else if D_InputSeqData.FIsNuc and D_InputSeqData.FIsCoding and VS_SeqDataExplorer.IsTranslated then
      ExportDataDlg.IsNucData := False
    else if D_InputSeqData.FIsNuc then
    begin
      ExportDataDlg.IsNucData := True;
      ExportDataDlg.IsChooseBases := False;
    end
    else
      ExportDataDlg.IsNucData := False;
    ...
    //Actually show the form and make sure the user clicks OK, otherwise don't continue the export
    if ExportDataDlg.ShowModal <> mrOK then
      Exit;
    //Since if it is excel format we need to handle it differently than the other formats
    ...
    //The results holds the information for every export type besides excel
    Results := TStringList.Create;
  ...
  Except
    ...
    //FreeAndNil(Results);
    FreeAndNil(ExportDataDlg);
  end;
{$ELSE}
var
  js: String;
  isNucData: Boolean = False;
  isChooseBases: Boolean = False;
  desc: String;
begin
  try
    HtmlOptionsDialog.ProcessMessageReceivedProc := ProcessWebOptionsMessage;
    HtmlOptionsDialog.DomProcedureName := VISITDOMPROC_SEQDATAEXPORTOPTIONS;
    HtmlOptionsDialog.Show;
    js := Format('$("#%s").val("%s");', [SDEO_SEQDATA_TITLE_ID, MegaForm.DataTitle]) + LineEnding;
    desc := SanitizeForJavaScript(MegaForm.DataDescription);
    js := js + Format('$("#%s").val("%s");', [SDEO_SEQDATA_DESCRIPTION_ID, desc]) + LineEnding;
    if D_InputSeqData.FIsNuc and D_InputSeqData.FIsCoding and (not VS_SeqDataExplorer.IsTranslated) then
    begin
      IsNucData := True;
      IsChooseBases := True;
    ...
    begin
      ...
      IsNucData := False;
    if IsNucData then
      js := js + Format('$("#%s").val("true")', [SDEO_IS_NUC_DATA]) + LineEnding;
    if IsChooseBases then
      js := js + Format('$("#%s").val("true")', [SDEO_IS_CHOOSE_BASES]) + LineEnding;
    if VS_SeqDataExplorer.CurAttrDisp <> megNone then
      js := js + Format('$("#%s").val("true")', [SDEO_HAS_HIGHLIGHTED_SITES]) + LineEnding;
    if (not IsNucData) or (not IsChooseBases) then
    begin
      js := js + Format('$("#%s, #%s, #%s, #%s").attr("disabled", "disabled");', [SDEO_INCLUDE_FIRST_POSITION_ID, SDEO_INCLUDE_SECOND_POSITION_ID, SDEO_INCLUDE_THIRD_POSITION_ID, SDEO_INCLUDE_NONCODING_POSITIONS_ID]) + LineEnding;
      js := js + Format('$("#%s").attr("disabled", "disabled");', [SDEO_FULL_CODONS_ID]) + LineEnding;
    end
    else
    begin
      js := js +  Format('$("#%s, #%s, #%s, #%s").removeAttr("disabled");', [SDEO_INCLUDE_FIRST_POSITION_ID, SDEO_INCLUDE_SECOND_POSITION_ID, SDEO_INCLUDE_THIRD_POSITION_ID, SDEO_INCLUDE_NONCODING_POSITIONS_ID]) + LineEnding;
      js := js + Format('$("#%s").removeAttr("disabled");', [SDEO_FULL_CODONS_ID]) + LineEnding;
    end;

    if Sender = ExportCSVTBtn then
      js := js + Format('$("#%s").val("%s").trigger("change");', [SDEO_FILE_FORMAT_ID, MEF_CSV]) + LineEnding
    else if Sender = ExportXLTBtn then
      js := js + Format('$("#%s").val("%s").trigger("change");', [SDEO_FILE_FORMAT_ID, MEF_EXCEL_XML]) + LineEnding
    else if Sender = ExportMEGATBtn then
      js := js + Format('$("#%s").val("%s").trigger("change");', [SDEO_FILE_FORMAT_ID, MEF_MEGA]) + LineEnding;
    if VS_SeqDataExplorer.CurAttrDisp = megNone then
      js := js + Format('$("#%s").hide()', [SITES_TO_INCLUDE_BOX]);
    HtmlOptionsDialog.LoadOptionsFile(wofWebSeqDataExportFile, js, 'Sequence Data Export Options', 380, 510, HC_Export_Data_in_Sequence_Data_Explorer);
  except
    on E:Exception do
      ShowMessage('Oh no! An error has occurred: ' + E.Message);
  end;
...
begin
  try
    try
      ...
      ActionDispTaxaNames.Checked := True;
      ActionDispGroupNames.Checked := True;
