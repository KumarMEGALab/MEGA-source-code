...
begin
  try
    if not FSelectionHasChanged then
      V_SeqDataExplorer.Show
    else
      ActivateAlignment(True);
  except
    on E: Exception do
      ShowMessage('Oh no! An error occured when retrieving the 47 species alignment: ' + E.Message);
  end;
end;
...
var
  ...
  TempFile: String;
  Nmid: String;
  Query: TEvodSearchResult;
begin
  Result := False;
  if (not FSelectionHasChanged) or (Assigned(V_SeqDataExplorer) and (V_SeqDataExplorer.EvoDPeptideId = FPeptideId)) then // no need to retrieve the same alignment
  begin
    V_SeqDataExplorer.Show;
    Exit;
  end;
