...
uses
  {$IFDEF VISUAL_BUILD}
  MWebBrowser, cef3intf, PrintersDlgs,
  {$ENDIF}
  Classes, SysUtils, FileUtil, Forms, Controls, Graphics, Dialogs,
  Menus, ExtCtrls, Grids, Spin, ComCtrls, Buttons, ActnList, MLegendGenerator,
  ...
  LCLType, IniPropStorage, mworkflow_element;
...
type
  ...
  {$ENDIF}
   ...
   public
     ...
     public
       CurSet: PArrayOfInt; // Sorted set of sequences
       FAllowNeg :Boolean;
  ...
  private
    ...
    function  ConstructCol0RowName(Index: LongInt): String;
    {$ENDIF}
  private
    DispStr: String;
...
uses
  ...
  mbrowserutils, MegaPrivateFiles, ContextHelp_HC;
...
procedure TD_DisplayMatrix.SaveToFileItemClick;
{$IFDEF VISUAL_BUILD}
var
...
{$IFDEF VISUAL_BUILD}
 try try
     //Setup the export dialog that will get the user's selected export options
    OutputMatrixDlg                := TOutputMatrixDlg.Create(nil);
    OutputMatrixDlg.NoOfRows       := NoOfRows;
    OutputMatrixDlg.NoOfCols       := NoOfCols;
    OutputMatrixDlg.HelpContext    := HC_Dist_Exp_Options_dialog_box;
    OutputMatrixDlg.AllowStandardError := (not FOnlyDistance);
    OutputMatrixDlg.AllowMEGAFormat := True;

    //disable MEGA export in certian situations
    case FComputationType of
      gdPairwise:     if FNoOfTaxa < 3 then
            OutputMatrixDlg.AllowMEGAFormat := False;
      gdBetweenGroupMean,
      gdNetGroupMean: if FNoOfGps < 3 then  OutputMatrixDlg.AllowMEGAFormat := False;
      gdWithinGroupMean: begin OutputMatrixDlg.AllowMEGAFormat := False; OutputMatrixDlg.IsColumnar := True; end;
    end;


    with OutputMatrixDlg do
    begin
      AllowNegative := FAllowNeg;
      Precision := VS_DisplayMatrix.FPrecision;

      //If clicked from a button then select that as the default export type


      //Show the options dialog so users can choose their options
      if ShowModal = mrOk then //If the user clicks okay then go ahead with the export
      begin
        ExportOptions := OutputMatrixDlg.GetExportOptions;  //Queries the options the user set and puts it into a type TDistExportOptions object to pass the export options
        Results := WriteExportToFile(ExportOptions); //Write the file (if excel) or return text to display (all not excel)
        if (ExportOptions.OutputFormat = ExportExcel) AND (ExcelWrite.hasExcel) then  //If the user is exporting to excel and has excel open the file we wrote for them in it's temporary location, this file will be deleted when mega closes if it can
          //ShellExecute(0, 'open', PChar(ExportOptions.SaveExcelFileTo),nil, nil, SW_SHOW)
          RunAProgram(ExportOptions.SaveExcelFileTo)
        else if ExportOptions.OutputFormat <> ExportExcel then    //If it is not exporting excel then we need to show it to them in the text editor.  If it is excel and they can not read excel files we do nothing more
   ...
   FreeAndNil(OutputMatrixDlg);
   FreeAndNil(ExportOptions);
   FreeAndNil(Results);
   // if MyD <> nil then FreeDistMatrix(MyD, VS_DistDataExplorer.NoOfSelTaxa);
  end;
 {$ENDIF}
end;
...
begin
  ...
  //if HasPrevFont then
  //begin
  //  DataGrid.Font.Name := PrevFontName;
  //  DataGrid.Font.Size := PrevFontSize;
  //  DataGrid.Canvas.Font.Name := PrevFontName;
  //  DataGrid.Canvas.Font.Size := PrevFontSize;
  //end;
  ...
  DataGrid.DefaultRowHeight := DataGrid.Canvas.TextHeight('W') + 6;
  DataGrid.RowCount := 1;
...
begin
  ...
  LowerLeftBtn.Down := True; // display is lowerleft
  ...
  else if SpinEdit1.Value = 10 then
    IncPrecisionAction.Enabled := False;
  //if VS_DisplayMatrix.FPrecision < 0 then
    VS_DisplayMatrix.FPrecision := Trunc(SpinEdit1.Value);
...
function TDisplayMatrixDlg.ConstructCol0RowName(Index: LongInt): String;
begin
  Result := IntToStr(Index)+'. ';   //Add a period to seperate the names and groups from the number

 if ShowTaxaNamesItem.Checked and ShowGpnamesItem.Checked then
  begin
    Result := Result + D_DisplayMatrix.RowName[Index-1];
    if Length(D_DisplayMatrix.RowGpname[Index-1]) > 0 then
      Result := Result + ' {'+D_DisplayMatrix.RowGpName[Index-1]+'}';
  end
...
begin
  try
    ...
    If Sender = SortOriginalItem Then
      Begin
         ...
         End; //For
      End
    Else // sort by Name or Group Name
      Begin
        SortedList := TStringList.Create;
        OriginalList := TStringList.Create;
        If Sender = SortByNameItem Then   //Sorting by Name
          Begin
            for i := 0 to nRows-1 do
              Begin
                OriginalList.Add(D_DisplayMatrix.GetRowName(i));
                SortedList.Add(D_DisplayMatrix.GetRowName(i));
                {OriginalList.Add(D_DisplayMatrix.TaxonName[i]);
                SortedList.Add(D_DisplayMatrix.TaxonName[i]);}
              End;
              SortedList.Sort();
           End
        Else //Sorting by Group Name
         ...
         Begin
            ...
            OldIndx := OriginalList.IndexOf(TheName);
            D_DisplayMatrix.CurSet[i] := OldIndx;
         End; //For
      End; //If Original Order
...
begin
  ...
  begin
    ...
    //DefaultRowHeight := Canvas.TextHeight('W') + 6;
    if OtherCol then
      //DefaultColWidth := Canvas.TextWidth('0') *(FPrecision + floor(abs(MaxD)) +3);
      DefaultColWidth := Canvas.TextWidth('0')*(VS_DisplayMatrix.FPrecision + 1 + Length(IntToStr(floor(D_DisplayMatrix.MaxD))) + 6);
    ...
    begin
      ...
      if Canvas.TextWidth(AStr) > VS_DisplayMatrix.MaxRowNamePixelLen then
        VS_DisplayMatrix.MaxRowNamePixelLen := Canvas.TextWidth(AStr);
    end;
...
procedure TDisplayMatrixDlg.DataGridSelectCell(Sender: TObject; aCol,
  ...
  begin
    ...
    begin
      ...
      begin
        FillRect(aRect);
        DispStr := EmptyStr;
        //TextRect(aRect, aRect.Left+2, aRect.Top+2, DispStr);
      end
...
procedure TDisplayMatrixDlg.ProcessWebOptionsMessage(const Browser: ICefBrowser; const message: ICefProcessMessage; out Result: Boolean);
var
  ExportOptions : TDistExportOptions = nil;
  fileType: String;
  Results: TStringList = nil;
...
begin
  try
    try
      ...
      ExportOptions.AllowNegative := D_DisplayMatrix.FAllowNeg;
      ...
      ExportOptions.FIsColumnar := ((D_DisplayMatrix.ComputationType = gdWithinGroupMean) or (D_DisplayMatrix.ComputationType = gdOverallMean));
      ...
      begin
        ...
        ExportOptions.PrevMatrixFormat := 0;
      end
      else
      begin
        ExportOptions.OppositeSides := False;
        ExportOptions.PrevMatrixFormat := 1;
      end;

      ExportOptions.NoOfRows := D_DisplayMatrix.NoOfRows;
      ...
      ExportOptions.WithStdErr := ExportOptions.AllowStandardError;
...
begin
  try
    ...
    js := Trim(js + LineEnding + Format('$("#%s").val("%d");', [DMEO_PRECISION_ID, VS_DisplayMatrix.FPrecision]));
...
begin
  FontDialog.Font := DataGrid.Font;
  if FontDialog.Execute then
    with DataGrid do
    begin
      DataGrid.Font := FontDialog.Font;
      DataGrid.Canvas.Font := FontDialog.Font;
      UpdateColumnWidths(True,True);
      DataGrid.Invalidate;
    end;
end;
...
begin
  ...
  DataGrid.Invalidate;
  SpinEdit1.Value := VS_DisplayMatrix.FPrecision;
end;
...
begin
  ...
  ChromiumBrowser.LoadHtmlFromString(Caption);
  {$ENDIF}
  {$ENDIF}
end;

procedure TDisplayMatrixDlg.DataGridMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
